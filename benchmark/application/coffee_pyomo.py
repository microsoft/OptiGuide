...
# Variables
model.x = Var(shipping_cost_from_supplier_to_roastery.keys(), domain=Integers, bounds=(0, None))
model.y_light = Var(shipping_cost_from_roastery_to_cafe.keys(), domain=Integers, bounds=(0, None))
model.y_dark = Var(shipping_cost_from_roastery_to_cafe.keys(), domain=Integers, bounds=(0, None))

# Objective
model.obj = Objective(expr=sum(model.x[i] * shipping_cost_from_supplier_to_roastery[i]
                            ...
                            sum(roasting_cost_light[r] * model.y_light[r, c] +
                                ...
                                for r, c in shipping_cost_from_roastery_to_cafe) +
                            sum((model.y_light[j] + model.y_dark[j]) * shipping_cost_from_roastery_to_cafe[j]
                                for j in shipping_cost_from_roastery_to_cafe),
                      sense=minimize)

# Constraints
def flow_constraint(model, r):
    return sum(model.x[i] for i in shipping_cost_from_supplier_to_roastery if i[1] == r) == \
           sum(model.y_light[j] + model.y_dark[j] for j in shipping_cost_from_roastery_to_cafe if j[0] == r)

def supply_constraint(model, s):
    return sum(model.x[i] for i in shipping_cost_from_supplier_to_roastery if i[0] == s) <= capacity_in_supplier[s]

def light_demand_constraint(model, c):
    return sum(model.y_light[j] for j in shipping_cost_from_roastery_to_cafe if j[1] == c) >= light_coffee_needed_for_cafe[c]

def dark_demand_constraint(model, c):
    return sum(model.y_dark[j] for j in shipping_cost_from_roastery_to_cafe if j[1] == c) >= dark_coffee_needed_for_cafe[c]
...
# Solve
solver = SolverFactory('glpk')  # You can change the solver as per your requirement
solver.solve(model)
