...
class WarehouseLayoutOptimization:
    ...
    def solve(self, instance):
        ...
        flow_vars = {(f, u): model.addVar(vtype="C", name=f"Flow_{f}_{u}") for f in range(n_facilities) for u in range(n_units)}
        ...
        model.setObjective(
            ...
            quicksum((premium_transport_costs[f, u] if premium_status[f] == 1 else transport_costs[f, u]) * flow_vars[f, u] for f in range(n_facilities) for u in range(n_units)), 
            "minimize"
        )

        # Network Flow Constraints: Flow conservation
        for u in range(n_units):
            model.addCons(quicksum(flow_vars[f, u] for f in range(n_facilities)) == demands[u], f"Unit_{u}_Demand_Flow")

        for f in range(n_facilities):
            for u in range(n_units):
                model.addCons(flow_vars[f, u] <= link_capacities[f, u], f"Link_{f}_{u}_Capacity")
                model.addCons(flow_vars[f, u] <= facility_vars[f] * link_capacities[f, u], f"Flow_{f}_{u}_Facility_Open")
            
            model.addCons(quicksum(flow_vars[f, u] for u in range(n_units)) <= spaces[f], f"Facility_{f}_Space")
        ...
        for f in range(n_facilities):
            if premium_status[f] == 1:
                ...
                for u in range(n_units):
                    model.addCons(flow_vars[f, u] <= M * premium_vars[f], f"Premium_transport_{f}_{u}")
