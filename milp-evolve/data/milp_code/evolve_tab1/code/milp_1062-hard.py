...
class PowerEfficientTaskDistribution:
    ...
    def get_instance(self):
        ...
        maintenance_cost = np.random.uniform(10, 50, self.n_data_centers).tolist()
        heat_dissipation = np.random.normal(20, 5, self.n_data_centers).tolist()
    ...
    def solve(self, instance):
        ...
        heat_dissipation = instance['heat_dissipation']
        ...
        maintenance_cost_vars = {dc: model.addVar(vtype="C", name=f"MaintenanceCost_{dc}", lb=0) for dc in range(n_data_centers)}
        heat_dissipation_vars = {dc: model.addVar(vtype="C", name=f"HeatDissipation_{dc}", lb=0) for dc in range(n_data_centers)}
        ...
        model.setObjective(
            ...
            quicksum(maintenance_cost_vars[dc] * maintenance_cost[dc] for dc in range(n_data_centers)) -
            quicksum(heat_dissipation_vars[dc] * heat_dissipation[dc] for dc in range(n_data_centers)),
            "maximize"
        )
        ...
        for dc in range(n_data_centers):
            model.addCons(power_vars[dc] == quicksum(task_vars[dc, t] * power_usage[dc] for t in range(n_tasks)), f"PowerUsage_{dc}")

        for dc in range(n_data_centers):
            model.addCons(maintenance_cost_vars[dc] <= maintenance_cost[dc], f"MaintenanceCost_{dc}")

        for dc in range(n_data_centers):
            model.addCons(heat_dissipation_vars[dc] <= heat_dissipation[dc], f"HeatDissipation_{dc}")
