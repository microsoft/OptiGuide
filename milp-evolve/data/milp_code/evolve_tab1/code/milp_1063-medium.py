...
class DisasterPreparednessOptimization:
    ...
    def solve(self, instance):
        ...
        model = Model("DisasterPreparednessOptimization")
        ...
        hazard_penalty_vars = {n: model.addVar(vtype="C", name=f"HazardPenalty_{n}") for n in range(n_neighborhoods)}
        revenue_vars = {n: model.addVar(vtype="C", name=f"Revenue_{n}") for n in range(n_neighborhoods)}
        ...
        # Hazard Penalty Constraints
        for n in range(n_neighborhoods):
            model.addCons(hazard_penalty_vars[n] == hazard_risk[n] * quicksum(allocation_vars[f, n] for f in range(n_facilities)), f"HazardPenalty_{n}")

        # Revenue from Secured Neighborhoods
        for n in range(n_neighborhoods):
            model.addCons(revenue_vars[n] == neighborhood_revenue[n] * quicksum(allocation_vars[f, n] for f in range(n_facilities)), f"Revenue_{n}")

        # Objective: Maximize total profitability while minimizing costs and penalties
        model.setObjective(
            quicksum(revenue_vars[n] for n in range(n_neighborhoods)) -
            quicksum(fixed_costs[f] * facility_vars[f] for f in range(n_facilities)) -
            quicksum(maintenance_costs[f] * maintenance_vars[f] for f in range(n_facilities)) -
            quicksum(transport_costs[f][n] * allocation_vars[f, n] for f in range(n_facilities) for n in range(n_neighborhoods)) -
            quicksum(hazard_penalty_vars[n] for n in range(n_neighborhoods)),
            "maximize"
        )
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'stddev_transport_cost': 126,
        'min_transport_cost': 56,
        'max_transport_cost': 2844,
        'min_capacity': 130,
        'max_capacity': 1012,
        'route_prob': 0.59,
        'traffic_delay_factor': 54.0,
        'max_total_cost': 100000,
        'min_maintenance_cost': 500,
        'max_maintenance_cost': 5000,
        'maintenance_multiplier': 1.5,
    }

    optimizer = DisasterPreparednessOptimization(parameters, seed=42)
