...
class CapacitatedHubLocation:
    ...
    def generate_instance(self):
        ...
        storage_capacities = np.random.randint(30, 100, size=graph.number_of_nodes)

        # Delivery time windows (start, end) periods for orders
        delivery_time_windows = [(np.random.randint(0, time_periods // 2), np.random.randint(time_periods // 2, time_periods)) 
    ...
    def solve(self, instance):
        ...
        delivery_vars = {(i, t): model.addVar(vtype="B", name=f"delivery_{i}_{t}") for i in graph.nodes for t in range(time_periods)}
        storage_vars = {(i, t): model.addVar(vtype="I", ub=storage_capacities[i], name=f"storage_{i}_{t}") for i in graph.nodes for t in range(time_periods)}
        ...
        for node in graph.nodes:
            delivery_start, delivery_end = delivery_time_windows[node]
            for t in range(time_periods):
                if t < delivery_start or t > delivery_end:
                    model.addCons(delivery_vars[node, t] == 0, name=f"DeliveryWindow_{node}_{t}")

        # Storage capacity constraints
        for t in range(time_periods):
            for node in graph.nodes:
                model.addCons(storage_vars[node, t] >= demands[node] - material_usage_vars[node, t], name=f"StorageCapacity_{node}_{t}")
