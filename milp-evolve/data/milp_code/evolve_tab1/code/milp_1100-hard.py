...
class UrbanParkPlanning:
    ...
    def solve(self, instance):
        ...
        # New binary variables for indicator constraints
        maintenance_indicator_vars = {p: model.addVar(vtype="B", name=f"Maint_Ind_{p}") for p in range(n_parks)}
        flow_indicator_vars = {(i, j): model.addVar(vtype="B", name=f"Flow_Ind_{i}_{j}") for i in range(n_parks) for j in range(n_parks)}
        ...
        # Indicator constraints for maintenance
        for p in range(n_parks):
            model.addCons(maintenance_vars[p] >= self.min_maintenance_frequency * maintenance_indicator_vars[p], f"Maintenance_{p}_MinFreq_Ind")
            model.addCons(maintenance_vars[p] <= self.max_maintenance_frequency * maintenance_indicator_vars[p], f"Maintenance_{p}_MaxFreq_Ind")
            model.addCons(maintenance_indicator_vars[p] == park_vars[p], f"Maintenance_Indicator_{p}")
        
        # Indicator constraints for flow
        for i in range(n_parks):
            for j in range(n_parks):
                model.addCons(flow_vars[i, j] >= flow_indicator_vars[i, j] * self.min_flow_amount, f"Flow_{i}_{j}_MinFlow_Ind")
                model.addCons(flow_vars[i, j] <= flow_indicator_vars[i, j] * self.max_flow_amount, f"Flow_{i}_{j}_MaxFlow_Ind")
                model.addCons(flow_indicator_vars[i, j] == park_vars[i] * park_vars[j], f"Flow_Indicator_{i}_{j}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'max_park_area': 1012,
        'affinity': 5,
        'flow_cost_range_min': 15,
        'flow_cost_range_max': 150,
        'min_maintenance_cost': 150,
        'max_maintenance_cost': 3000,
        'min_maintenance_frequency': 0,
        'max_maintenance_frequency': 45,
        'min_env_impact': 0.52,
        'max_env_impact': 150.0,
        'min_flow_amount': 0,
        'max_flow_amount': 500,
    }
