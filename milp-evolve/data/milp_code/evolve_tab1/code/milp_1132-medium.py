...
class ManufacturingJobScheduling:
    ...
    def generate_instance(self):
        ...
        junction_penalties = {node: np.random.uniform(10, 50) for node in G.nodes}
        ...
        return {
            ...
            'junction_penalties': junction_penalties
        }
    ...
    def solve(self, instance):
        ...
        junction_penalties = instance['junction_penalties']
        ...
        batch_vars = {b: model.addVar(vtype="B", name=f"Batch_{b}") for b in range(self.n_batches)}
        assigned_batch_vars = {(j, b): model.addVar(vtype="B", name=f"Job_{j}_Batch_{b}") for j in range(n_job_types) for b in range(self.n_batches)}
        flow_vars = {(u, v): model.addVar(vtype="B", name=f"Flow_{u}_{v}") for u, v in G.edges}
        capacity_vars = {m: model.addVar(vtype="C", name=f"Capacity_{m}", lb=0) for m in range(n_machines)}
        ...
        for b in range(self.n_batches):
            model.addCons(quicksum(assigned_batch_vars[j, b] for j in range(n_job_types)) <= batch_vars[b], name=f"Batch_{b}_UpperBound")
            model.addCons(quicksum(assigned_batch_vars[j, b] for j in range(n_job_types)) >= batch_vars[b] * self.min_batch_size, name=f"Batch_{b}_LowerBound")
            for j in range(n_job_types):
                model.addCons(batch_vars[b] >= assigned_batch_vars[j, b], name=f"BatchAssign_{b}_Job_{j}")
        
        for u, v in G.edges:
            model.addCons(flow_vars[(u, v)] <= machine_usage_vars[u], name=f"Flow_{u}_{v}_Start")
            model.addCons(flow_vars[(u, v)] <= machine_usage_vars[v], name=f"Flow_{u}_{v}_End")
        
        for b in range(self.n_batches):
            model.addCons(quicksum(assigned_batch_vars[j, b] for j in range(n_job_types)) <= capacity_vars[b], name=f"Capacity_Batch_{b}")

        junction_penalty_terms = quicksum(junction_penalties[node] * machine_usage_vars[node] for node in G.nodes)
