...
class DataCenterTaskAllocation:
    ...
    def generate_instance(self):
        ...
        # Generate more subsets using graph-based approach for complexity
        G = nx.erdos_renyi_graph(self.n_tasks, 0.1, seed=self.seed)
        cliques = list(nx.find_cliques(G))
        subsets = [random.sample(range(self.n_tasks), int(0.2 * self.n_tasks)) for _ in range(5)]
        all_subsets = subsets + cliques[:5]  # adding first 5 cliques
        min_coverage = np.random.randint(1, 5, len(all_subsets))
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_cost_allocation': 337,
        'max_cost_allocation': 600,
        'min_capacity_server': 750,
        'max_capacity_server': 1350,
        'min_task_demand': 700,
