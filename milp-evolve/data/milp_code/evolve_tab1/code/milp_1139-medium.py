...
class GISP:
    ...
    def generate_op_costs(self, G):
        op_costs = {node: np.random.uniform(self.min_op_cost, self.max_op_cost) for node in G.nodes}
        return op_costs
    ...
    def generate_instance(self):
        ...
        op_costs = self.generate_op_costs(G)
        
        return {'G': G, 'E2': E2, 'sensor_statuses': sensor_statuses, 'maintenance_costs': maintenance_costs, 'op_costs': op_costs}
    
    ################# PySCIPOpt Modeling #################
    def solve(self, instance):
        G, E2, sensor_statuses, maintenance_costs, op_costs = instance['G'], instance['E2'], instance['sensor_statuses'], instance['maintenance_costs'], instance['op_costs']
        ...
        op_cost_vars = {f"oc{node}": model.addVar(vtype="C", lb=0, ub=self.max_op_cost, name=f"oc{node}") for node in G.nodes}
        for node in G.nodes:
            model.addCons(
                (node_vars[f"x{node}"] * self.min_op_cost) <= op_cost_vars[f"oc{node}"],
                name=f"OpC_lb_{node}"
            )
            model.addCons(
                op_cost_vars[f"oc{node}"] <= (node_vars[f"x{node}"] * self.max_op_cost),
                name=f"OpC_ub_{node}"
            )
