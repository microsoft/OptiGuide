...
class DisasterResponseOptimization:
    ...
    def solve(self, instance):
        ...
        # Simplified Objective function: Minimize total cost (fixed + transport + penalty for unmet demand + holding + backlog + transshipment)
        model.setObjective(
        ...
        for f in range(n_factories):
            for t in range(n_periods):
                model.addCons(
                    ...
                    quicksum(transship_vars[f, j, t] for j in range(n_factories) if j != f) <= capacities[f] * factory_vars[f, t], 
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_cost_transport': 140,
        'max_cost_transport': 700,
        'min_cost_transship': 216,
        'max_cost_transship': 2250,
        'min_capacity_factory': 350,
        'max_capacity_factory': 3000,
        'min_demand': 5,
        'max_demand': 800,
    }

    supply_chain_optimizer = DisasterResponseOptimization(parameters, seed)
