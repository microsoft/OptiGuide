...
class UrbanParkPlanning:
    ...
    def solve(self, instance):
        ...
        M = max(park_costs) + 1  # Big M constant for the formulation
        # Constraints: Park Graph Cliques with Big M
        for count, group in enumerate(inequalities):
            if len(group) > 1:
                for u, v in combinations(group, 2):
                    model.addCons(park_vars[u] + park_vars[v] <= edge_vars[(u, v)] + 1, f"CliqueBigM_{u}_{v}")
                    model.addCons(edge_vars[(u, v)] <= park_vars[u], f"EdgeBigM_u_{u}_{v}")
                    model.addCons(edge_vars[(u, v)] <= park_vars[v], f"EdgeBigM_u_{v}_{u}")
