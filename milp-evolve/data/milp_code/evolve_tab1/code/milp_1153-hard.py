...
class CapacitatedHubLocation:
    ...
    def solve(self, instance):
        ...
        for hub in graph.nodes:
            ...
            model.addCons(quicksum(hub_vars[node] * node_weights[node] for node in graph.nodes) - knapsack_capacity + M * indic_var <= M, name=f"IndicKnapsack_{hub}_upper")
            model.addCons(indic_var <= hub_vars[hub], name=f"IndicKnapsack_{hub}_relation")
