...
class DroneDeliveryOptimization:
    ...
    def generate_instance(self):
        ...
        battery_life = np.random.randint(200, 500, self.n_drones) # in minutes
        range_limitations = np.random.randint(20, 50, self.n_drones) # in kilometers
        ...
        return {
            ...
            "range_limitations": range_limitations,
    ...
    def solve(self, instance):
        ...
        range_limitations = instance['range_limitations']
        ...
        charging_node_vars = {d: model.addVar(vtype="B", name=f"Charging_Node_{d}") for d in range(n_drones)}
        ...
        for d in range(n_drones):
            model.addCons(quicksum(task_assignment_vars[d, t] for t in range(n_tasks)) <= battery_life[d], f"Battery_Life_{d}")
        
        # Range limitations for each drone
        for d in range(n_drones):
            model.addCons(quicksum(task_assignment_vars[d, t] * 0.5 for t in range(n_tasks)) <= range_limitations[d], f"Range_Limitations_{d}")
        ...
        # Ensure charging node allocation for operational drones
        for d in range(n_drones):
            model.addCons(drone_vars[d] == charging_node_vars[d], f"Charging_Node_Allocation_{d}")
