...
class LogisticsNetworkDesign:
    ...
    def generate_instance(self):
        ...
        # Generate independent set inclusion probabilities
        independent_set_probs = np.random.uniform(0.8, 1, self.Number_of_Depots)

        # Generate mutual exclusivity pairs
        mutual_exclusivity_pairs = []
        for _ in range(self.Mutual_Exclusivity_Pairs):
            node1 = random.randint(0, self.Number_of_Depots - 1)
            node2 = random.randint(0, self.Number_of_Depots - 1)
            if node1 != node2:
        ...
        return {
            ...
            "independent_set_probs": independent_set_probs,
    ...
    def solve(self, instance):
        ...
        independent_set_probs = instance['independent_set_probs']
        ...
        independent_set_vars = {node: model.addVar(vtype="B", name=f"IndependentSet_{node}") for node in range(number_of_depots)}

        # Objective: minimize the total cost including distance costs and depot costs + maximize the independent set inclusion
        model.setObjective(
            ...
            quicksum(distance_cost_vars[d] for d in range(number_of_depots)) -
            quicksum(independent_set_probs[node] * independent_set_vars[node] for node in range(number_of_depots)), "minimize"
        )
        ...
        for node in range(number_of_depots):
            model.addCons(independent_set_vars[node] <= depot_vars[node], f"IndependentSet_{node}")

        # Mutual Exclusivity Constraints
        for index, (node1, node2) in enumerate(mutual_exclusivity_pairs):
            model.addCons(depot_vars[node1] + depot_vars[node2] <= 1, f"MutualExclusivity_{index}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'City_Cost_Lower_Bound': 21,
        'City_Cost_Upper_Bound': 3000,
        'Min_Depot_Cost': 421,
        'Max_Depot_Cost': 5000,
        'Min_Depot_Capacity': 87,
        'Max_Depot_Capacity': 157,
        'Affinity': 2,
        'Min_Distance_Cost': 0.1,
        'Max_Distance_Cost': 0.38,
        'Mutual_Exclusivity_Pairs': 150,
    }

    logistics_network_optimizer = LogisticsNetworkDesign(parameters, seed=seed)
