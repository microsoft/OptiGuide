...
class DeliveryOptimization:
    ...
    def solve(self, instance):
        ...
        M = 10000  # Big M value, sufficiently large
        ...
        for i in range(num_locations):
            ...
            # Big-M constraints for high population density favoring recycling centers
            model.addCons(recycling_center[i] >= (population_density[i] / (M + population_density[i])), f"HighDensityRecycle_{i}")
            
            # Big-M constraints to ensure no waste disposal near water sources
            model.addCons(waste_disposal[i] <= (M - (proximity_to_water[i] * M)) / M, f"WaterSourceNoDisposal_{i}")
