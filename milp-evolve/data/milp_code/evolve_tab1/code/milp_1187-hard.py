...
class WarehouseLayoutOptimization:
    ...
    def generate_instance(self):
        ...
        # Generate capacity and additional flow cost data
        ...
        # Generate new data for facility types (hub or distribution center)
        facility_types = np.random.randint(0, 2, self.n_facilities)  # 0 for distribution center, 1 for hub
        hub_penalty_costs = np.random.randint(self.min_hub_penalty, self.max_hub_penalty + 1, self.n_facilities)
        ...
        return {
            ...
            "hub_penalty_costs": hub_penalty_costs,
        }
    ...
    def solve(self, instance):
        ...
        hub_penalty_costs = instance["hub_penalty_costs"]
        ...
        hub_indicator_vars = {f: model.addVar(vtype="B", name=f"HubIndicator_{f}") for f in range(n_facilities)}
        ...
        model.setObjective(
            ...
            quicksum(hub_penalty_costs[f] * hub_indicator_vars[f] for f in range(n_facilities)), 
            "minimize"
        )
        ...
        for f in range(n_facilities):
            model.addCons(quicksum(flow_vars[f, u] for u in range(n_units)) >= quicksum(demands[u] * transport_vars[f, u] for u in range(n_units)), f"Flow_Balance_{f}")

        # New indicator constraints for facility types (hub or distribution center)
        for f in range(n_facilities):
            model.addCons(hub_indicator_vars[f] == (facility_types[f] == 1), f"Hub_Indicator_{f}")
            model.addCons(self.special_facility_types_weight * hub_indicator_vars[f] <= self.maximum_hub_penalty, f"Hub_Penalty_{f}")
...
if __name__ == "__main__":
    ...
    parameters = {
        ...
        'graph_density': 0.45,
        'bigM': 10000,
        'min_social_impact': 74,
        'max_social_impact': 750,
        'min_carbon_emission': 2.5,
        'max_carbon_emission': 562.5,
        'social_impact_weight': 10.08,
        'carbon_emission_weight': 10.5,
        'transport_distance_factor': 0.38,
        'min_flow_capacity': 2500,
        'max_flow_capacity': 3000,
        'min_flow_cost': 27.0,
        'max_flow_cost': 300.0,
        'min_hub_penalty': 75,
        'max_hub_penalty': 1000,
        'special_facility_types_weight': 30.0,
        'maximum_hub_penalty': 1500,
    }
