...
class GISP:
    ...
    def generate_instance(self):
        ...
        # Generate data for set covering
        coverage_requirements = np.random.randint(1, 5, G.number_of_nodes())
        res["coverage_requirements"] = coverage_requirements
    ...
    def solve(self, instance):
        ...
        coverage_requirements = instance['coverage_requirements']
        ...
        CoverageSet_vars = {f"cs{node}": model.addVar(vtype="B", name=f"cs{node}") for node in G.nodes}
        ...
        for node in G.nodes:
            model.addCons(CoverageSet_vars[f"cs{node}"] * coverage_requirements[node] <= 1, f"SetCover_{node}")
        
        model.setObjective(
            objective_expr + quicksum(campaign_costs[node] * MarketingCampaign_vars[f"mc{node}"] for node in G.nodes) -
            quicksum(ad_costs[node, zone] * NeighborhoodCoverage_vars[node, zone] for node in G.nodes for zone in range(self.n_zones)) +
            z * self.subsidiary_penalty, 
            "maximize"
        )
