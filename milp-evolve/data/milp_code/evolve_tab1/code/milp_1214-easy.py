...
class BeekeepingPlanning:
    ...
    def generate_instance(self):
        ...
        penalties = np.random.randint(self.min_penalty, self.max_penalty, self.n_tasks)
    ...
    def solve(self, instance):
        ...
        # New variables for penalty costs
        penalty_vars = {t: model.addVar(vtype="C", name=f"Penalty_{t}") for t in range(n_tasks)}

        # Objective: Maximize honey production while minimizing nutrient, honey processing costs, health impacts, and penalties
        model.setObjective(
            ...
            quicksum(penalty_vars[t] * penalties[t] for t in range(n_tasks)),
            "maximize"
        )
        ...
        # Penalty constraints for task coverage
        for t in range(n_tasks):
            model.addCons(quicksum(allocation_vars[w, t] for w in range(n_beekeeping_units)) + penalty_vars[t] >= 1, f"Penalty_Task_{t}")
