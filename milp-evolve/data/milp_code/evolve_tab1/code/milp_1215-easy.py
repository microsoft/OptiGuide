...
class CapacitatedHubLocation:
    ...
    def generate_instance(self):
        ...
        connection_costs = np.random.randint(5, 50, size=(graph.number_of_nodes, graph.number_of_nodes))
    ...
    def solve(self, instance):
        ...
        unused_capacity_penalty_vars = {hub: model.addVar(vtype="I", name=f"unused_capacity_penalty_{hub}") for hub in graph.nodes}
        ...
        for hub in graph.nodes:
            ...
            model.addCons(unused_capacity_penalty_vars[hub] >= (capacities[hub] - quicksum(demands[node] * routing_vars[node, hub] for node in graph.nodes)), name=f"UnusedCapacityPenalty_{hub}")
        ...
        for node in graph.nodes:
            for hub in graph.nodes:
                model.addCons(distances[node, hub] * routing_vars[node, hub] <= max_distance * routing_vars[node, hub], name=f"Distance_{node}_{hub}")

        # Objective: Minimize total costs
        hub_opening_cost = quicksum(hub_vars[node] * opening_costs[node] for node in graph.nodes)
        connection_total_cost = quicksum(routing_vars[i, j] * connection_costs[i, j] + distances[i, j] * routing_vars[i, j] for i in graph.nodes for j in graph.nodes)
        unused_capacity_penalty = quicksum(unused_capacity_penalty_vars[hub] for hub in graph.nodes)
        
        total_cost = hub_opening_cost + connection_total_cost + unused_capacity_penalty
