...
class DRRNetwork:
    ...
    def generate_instance(self):
        ...
        # New utility costs and capacities:
        utility_costs = np.random.uniform(self.min_utility_cost, self.max_utility_cost, (self.n_nodes, self.n_nodes))
        utility_capacities = np.random.uniform(self.min_utility_capacity, self.max_utility_capacity, (self.n_nodes, self.n_nodes))
    ...
    def solve(self, instance):
        ...
        utility_capacities = instance['utility_capacities']
        ...
        # New Utility load variables
        utility_load_vars = {f"UtilityLoad_{i+1}_{j+1}": model.addVar(vtype="C", name=f"UtilityLoad_{i+1}_{j+1}") for (i, j) in edge_list}

        # Update objective function to include maintenance and utility costs
        ...
        objective_expr += quicksum(
            utility_costs[i, j] * utility_load_vars[f"UtilityLoad_{i+1}_{j+1}"]
            for (i, j) in edge_list
        )
        ...
        for (i, j) in edge_list:
            ...
            # Utility constraints:
            model.addCons(utility_load_vars[f"UtilityLoad_{i+1}_{j+1}"] <= utility_capacities[i, j], f"utility_capacity_{i+1}_{j+1}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'max_utility_cost': 1000,
        'min_utility_capacity': 250,
        'max_utility_capacity': 1000,
    }
