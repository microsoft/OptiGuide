...
class NeighborhoodMaintenanceOptimization:
    ...
    def generate_instance(self):
        ...
        budget_limits = np.random.randint(5000, 10000, self.num_teams) # in cost units
        work_ranges = np.random.randint(50, 100, self.num_teams) # in task units
        priority_neighborhoods = np.random.choice([0, 1], self.num_neighborhoods, p=[0.7, 0.3]) # 30% high-priority neighborhoods

        # Generate environmental impact costs for each maintenance method in each neighborhood
        env_impact_costs = np.random.randint(self.min_env_impact_cost, self.max_env_impact_cost + 1, (self.num_neighborhoods, self.num_maintenance_methods))
        ...
        return {
            ...
            "env_impact_costs": env_impact_costs  # new data element
        }
    ...
    def solve(self, instance):
        ...
        work_ranges = instance["work_ranges"]
        priority_neighborhoods = instance["priority_neighborhoods"]
        env_impact_costs = instance["env_impact_costs"]  # new data element
        ...
        guard_assignment_vars = {t: model.addVar(vtype="B", name=f"GuardAssignment_{t}") for t in range(num_teams)}
        
        # New binary variables for selecting maintenance methods
        maintenance_method_vars = {(n, m): model.addVar(vtype="B", name=f"MaintenanceMethod_{n}_{m}") for n in range(num_neighborhoods) for m in range(self.num_maintenance_methods)}
        ...
        model.setObjective(
            ...
            quicksum(env_impact_costs[n][m] * maintenance_method_vars[n, m] for n in range(num_neighborhoods) for m in range(self.num_maintenance_methods)),  # new objective component
            "minimize"
        )
        ...
        for t in range(num_teams):
            model.addCons(quicksum(neighborhood_vars[t, n] for n in range(num_neighborhoods)) <= budget_limits[t], f"BudgetLimit_{t}")
        
        # Work range for each team
        for t in range(num_teams):
            model.addCons(quicksum(neighborhood_vars[t, n] * 1.2 for n in range(num_neighborhoods)) <= work_ranges[t], f"WorkRange_{t}")
        ...
        # Ensure guard assignment for active teams
        for t in range(num_teams):
            model.addCons(team_vars[t] == guard_assignment_vars[t], f"GuardCoverage_{t}")

        # Ensure only one maintenance method is selected per neighborhood
        for n in range(num_neighborhoods):
            model.addCons(quicksum(maintenance_method_vars[n, m] for m in range(self.num_maintenance_methods)) == 1, f"MethodSelection_{n}")
...
if __name__ == "__main__":
    ...
    parameters = {
        'num_teams': 200,
        'num_neighborhoods': 225,
        'min_maintenance_cost': 3000,
        'max_maintenance_cost': 3000,
        'min_security_cost': 112,
        'max_security_cost': 125,
        'min_task_capacity': 250,
        'max_task_capacity': 1500,
        'min_coverage_tasks': 105,
        'max_coverage_tasks': 1350,
        'num_maintenance_methods': 2,
        'min_env_impact_cost': 50,
        'max_env_impact_cost': 1000,
    }
