...
class SupplyChainOptimization:
    ...
    def solve(self, instance):
        ...
        M = max(factory_costs) + 1  # Big M constant for the formulation
        # Constraints: Only open factories can serve warehouses
        for f in range(n_factories):
            for w in range(n_warehouses):
                model.addCons(warehouse_vars[f, w] <= factory_vars[f], f"Factory_{f}_Opens_Warehouse_{w}")
        
        # Constraints: Each warehouse can be linked to only one factory
        for w in range(n_warehouses):
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'max_warehouse_cost': 2000,
        'min_transport_cost': 6,
        'max_transport_cost': 350,
        'min_factory_capacity': 300,
        'max_factory_capacity': 2000,
        'min_customer_demand': 30,
        'max_customer_demand': 300,
