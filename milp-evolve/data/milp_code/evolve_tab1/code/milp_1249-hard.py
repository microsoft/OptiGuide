...
class LogisticsNetworkDesign:
    ...
    def solve(self, instance):
        ...
        for s in range(self.No_of_Scenarios):
            for c in range(number_of_cities):
                model.addCons(
                    quicksum(transport_scenarios[s]['demand'][c] * city_vars[d, c] for d in range(number_of_depots)) == transport_scenarios[s]['demand'][c],
                    name=f"Scenario_{s}_City_{c}_Demand"
                )
