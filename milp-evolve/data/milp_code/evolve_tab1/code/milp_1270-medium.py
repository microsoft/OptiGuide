...
class NeighborhoodMaintenanceOptimization:
    ...
    def solve(self, instance):
        ...
        # Replace Big M formulation with Convex Hull formulation
        for t in range(num_teams):
            for n in range(num_neighborhoods):
                y = model.addVar(vtype="B", name=f"ConvexHull_{t}_{n}")
                model.addCons(neighborhood_vars[t, n] <= coverage_tasks[n] * y, f"ConvexHullUpper_{t}_{n}")
                model.addCons(neighborhood_vars[t, n] >= 0, f"ConvexHullLower_{t}_{n}")
                model.addCons(neighborhood_vars[t, n] <= coverage_tasks[n] * team_vars[t], f"ConvexHullActive_{t}_{n}")
                model.addCons(y <= team_vars[t], f"ConvexHullLink_{t}_{n}")
...
if __name__ == "__main__":
    ...
    parameters = {
        'num_teams': 200,
        'num_neighborhoods': 56,
        'min_maintenance_cost': 3000,
        'max_maintenance_cost': 3000,
        'min_security_cost': 112,
        'max_security_cost': 250,
        'min_task_capacity': 750,
        'max_task_capacity': 2000,
        'min_coverage_tasks': 17,
        'max_coverage_tasks': 45,
    }
