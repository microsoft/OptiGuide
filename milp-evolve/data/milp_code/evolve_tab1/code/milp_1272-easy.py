...
class SupplyChainNetwork:
    def __init__(self, number_of_factories, number_of_warehouses, number_of_customers, factory_costs, warehouse_costs, transport_costs, factory_capacities, customer_demands, carbon_emissions):
...
class SupplyChainOptimization:
    ...
    def generate_instance(self):
        ...
        carbon_emissions = np.random.uniform(self.min_carbon_emission, self.max_carbon_emission, (self.n_factories, self.n_warehouses, self.n_customers))
    ...
    def solve(self, instance):
        ...
        carbon_vars = {(f, w, c): model.addVar(vtype="C", name=f"Carbon_{f}_Warehouse_{w}_Customer_{c}") for f in range(n_factories) for w in range(n_warehouses) for c in range(n_customers)}
        
        # Objective: minimize the total cost including factory, warehouse, transport costs and carbon emissions penalties
        model.setObjective(
            ...
            quicksum(self.carbon_penalty * carbon_vars[f, w, c] for f in range(n_factories) for w in range(n_warehouses) for c in range(n_customers)), 
            "minimize"
        )
        ...
        # Constraints: Carbon emissions from transport
        total_transport_emission = quicksum(carbon_emissions[f, w, c] * transport_vars[f, w, c] for f in range(n_factories) for w in range(n_warehouses) for c in range(n_customers))
        model.addCons(total_transport_emission <= self.max_total_carbon_emission, "Max_Total_Carbon_Emission")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'max_warehouse_cost': 1000,
        'min_transport_cost': 42,
        'max_transport_cost': 175,
        'min_factory_capacity': 600,
        'max_factory_capacity': 2000,
        'min_customer_demand': 150,
        'max_customer_demand': 900,
        'max_warehouse_capacity': 3000,
        'min_carbon_emission': 0.66,
        'max_carbon_emission': 15,
        'carbon_penalty': 200,
        'max_total_carbon_emission': 10000,
    }
