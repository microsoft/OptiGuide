...
class CapacitatedHubLocation:
    ...
    def generate_instance(self):
        ...
        connection_costs = np.random.rand(graph.number_of_nodes, graph.number_of_nodes) * 50
        ...
        emissions = np.random.rand(graph.number_of_nodes, graph.number_of_nodes) * 10

        # Essential vs Non-essential supplies
        essential_demand = {node: (demand if np.random.rand() > 0.7 else 0) for node, demand in enumerate(demands)}
        ...
        res = {
            ...
            'emissions': emissions,
            'essential_demand': essential_demand,
        }
        return res
    ...
    def solve(self, instance):
        ...
        essential_demand = instance['essential_demand']
        ...
        total_emissions = quicksum(emission_vars[i, j] for i in graph.nodes for j in graph.nodes)
        model.addCons(total_emissions <= self.max_emissions, "TotalEmissions")

        # Essential Supply Constraints
        for node in graph.nodes:
            model.addCons(quicksum(routing_vars[node, hub] * essential_demand[node] for hub in graph.nodes) == essential_demand[node], name=f"Essential_{node}")
