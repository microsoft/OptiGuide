...
class LogisticsNetworkDesign:
    ...
    def generate_instance(self):
        ...
        transport_costs = {(d, c): np.random.uniform(self.TransportCost_LB, self.TransportCost_UB) for d in range(self.Number_of_Depots) for c in range(self.Number_of_Cities)}

        distances = {(d, c): np.linalg.norm(np.random.uniform(0, 50, 2) - np.random.uniform(0, 50, 2).astype(int)) for d in range(self.Number_of_Depots) for c in range(self.Number_of_Cities)}
    ...
    def solve(self, instance):
        ...
        segment_vars = {(d, c, s): model.addVar(vtype="C", name=f"Segment_{d}_{c}_{s}", lb=0.0) for d in range(number_of_depots) for c in range(number_of_cities) for s in range(len(transport_cost_segments)-1)}

        # Objective: minimize the total cost including depot costs, renewable utilization, variable transport costs, and city assignment costs
        model.setObjective(
            ...
            quicksum(segment_vars[d, c, s] * distances[d, c] * transport_cost_segments[s] for d in range(number_of_depots) for c in range(number_of_cities) for s in range(len(transport_cost_segments)-1)),
            "minimize"
        )
...
if __name__ == '__main__':
    seed = 42
    parameters = {
        'Number_of_Depots': 123,
        'Number_of_Cities': 168,
        'City_Cost_Lower_Bound': 2025,
        'City_Cost_Upper_Bound': 3000,
        'Min_Depot_Cost': 1263,
        'Max_Depot_Cost': 5000,
        'Min_Depot_Capacity': 885,
        'Max_Depot_Capacity': 2834,
