...
class AerospaceSupplyChainOptimization:
    ...
    def solve(self, instance):
        ...
        # New Piecewise Transport Cost Variables
        piecewise_land_intervals = range(self.piecewise_intervals)
        piecewise_air_intervals = range(self.piecewise_intervals)
        
        piecewise_vars_land = {(f, c, i): model.addVar(vtype="B", name=f"Piecewise_Facility_{f}_Component_{c}_Land_Interval_{i}") 
                               for f in range(n_facilities) for c in range(n_components) for i in piecewise_land_intervals}
        piecewise_vars_air = {(f, c, i): model.addVar(vtype="B", name=f"Piecewise_Facility_{f}_Component_{c}_Air_Interval_{i}")
                              for f in range(n_facilities) for c in range(n_components) for i in piecewise_air_intervals}
        ...
        # New Constraints: Convex Hull Formulations for Piecewise Cost Structures
        for f in range(n_facilities):
            for c in range(n_components):
                model.addCons(
                    quicksum(piecewise_vars_land[f, c, i] for i in piecewise_land_intervals) == transport_vars_land[f, c],
                    f"ConvexHull_Land_{f}_{c}"
                )
                model.addCons(
                    quicksum(piecewise_vars_air[f, c, i] for i in piecewise_air_intervals) == transport_vars_air[f, c],
                    f"ConvexHull_Air_{f}_{c}"
                )
...
if __name__ == "__main__":
    ...
    parameters = {
        ...
        'max_transport_cost': 400.0,
        'min_transport_cost_air': 350.0,
        'max_transport_cost_air': 2000.0,
        'min_inventory_cost': 112.5,
        'max_inventory_cost': 750.0,
        'min_lead_time': 3,
        'max_lead_time': 120,
        'lead_time_land': 0,
        'lead_time_air': 0,
