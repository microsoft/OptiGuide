...
class MealServiceOptimization:
    ...
    def generate_instance(self):
        ...
        energy_consumption_rates = np.random.uniform(self.min_energy_rate, self.max_energy_rate, (self.n_meals, self.n_halls))
    ...
    def solve(self, instance):
        ...
        energy_vars = {(m, h): model.addVar(vtype="C", name=f"Energy_{m}_Hall_{h}") for m in range(n_meals) for h in range(n_halls)}
        ...
        model.setObjective(
            ...
            quicksum(self.energy_cost * energy_vars[m, h] for m in range(n_meals) for h in range(n_halls)),
            "minimize"
        )
        ...
        # Constraints: Limit energy consumption
        for m in range(n_meals):
            for h in range(n_halls):
                model.addCons(energy_vars[m, h] == energy_consumption_rates[m, h] * distribution_vars[m, h], f"Energy_{m}_Hall_{h}_Usage")
                model.addCons(energy_vars[m, h] <= self.max_energy_per_hall, f"Limit_Energy_{m}_Hall_{h}")
...
if __name__ == "__main__":
    ...
    parameters = {
        ...
        'max_transport_cost': 150,
        'min_dietary_requirement': 0,
        'max_dietary_requirement': 6,
        'mean_demand': 10,
        'std_dev_demand': 0,
        'min_capacity': 150,
        'max_capacity': 1500,
        'bigM': 37,
        'diet_penalty': 112.5,
        'min_energy_rate': 0.66,
        'max_energy_rate': 20.0,
        'energy_cost': 0.75,
        'max_energy_per_hall': 50.0,
    }
