...
class LogisticsNetworkDesign:
    ...
    def generate_instance(self):
        ...
        quality_weights = np.random.uniform(0.8, 1.0, (self.Number_of_Depots, self.Number_of_Cities))
    ...
    def solve(self, instance):
        ...
        resource_vars = {(d, c): model.addVar(vtype="C", lb=0, ub=depot_capacities[d], name=f"Resource_D_{d}_C_{c}") for d in range(number_of_depots) for c in range(number_of_cities)}

        # Objective: minimize the expected total cost including depot costs, city assignment costs, and resource usage costs
        model.setObjective(
            ...
            quicksum(resource_vars[d, c] * quality_weights[d, c] for d in range(number_of_depots) for c in range(number_of_cities)) +
        ...
        # Constraints: Each city must achieve a minimum quality of service
        min_quality = 0.9
        for c in range(number_of_cities):
            model.addCons(quicksum(quality_weights[d, c] * city_vars[d, c] for d in range(number_of_depots)) >= min_quality, f"City_{c}_Quality")
        
        # Constraints: Each depot must serve at least the minimum number of cities
        min_service = 5
        for d in range(number_of_depots):
            model.addCons(quicksum(city_vars[d, c] for c in range(number_of_cities)) >= depot_vars[d] * min_service, f"Depot_{d}_MinService")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'Max_Depot_Capacity': 472,
        'No_of_Scenarios': 20,
        'Demand_Variation': 0.1,
        'Minimum_Quality': 0.52,
        'Minimum_Service': 50,
    }
