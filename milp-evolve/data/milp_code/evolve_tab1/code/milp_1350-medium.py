...
class CapacitatedHubLocation:
    ...
    def solve(self, instance):
        ...
        # Modified Capacity Constraints using Convex Hull Formulation
        for hub in graph.nodes:
            for k in range(n_commodities):
                # Introduction of new auxiliary variables for better bounding
                aux_vars = {(node, k): model.addVar(vtype="C", name=f"aux_{node}_{hub}_{k}") for node in graph.nodes}
                for node in graph.nodes:
                    model.addCons(aux_vars[(node, k)] <= commodity_demands[node, k] * routing_vars[node, hub, k])
                    model.addCons(aux_vars[(node, k)] <= capacities[hub] * hub_vars[hub])
                model.addCons(quicksum(aux_vars[(node, k)] for node in graph.nodes) <= capacities[hub])
