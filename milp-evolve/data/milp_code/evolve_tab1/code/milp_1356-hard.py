...
class ComplexCapacitatedHubLocation:
    ...
    def generate_instance(self):
        ...
        # New parameters for the complex version related to energy and emissions
        max_energy_consumption = np.random.uniform(1000, 5000, size=graph.number_of_nodes)
        emission_rates = np.random.uniform(0.1, 1.0, size=(graph.number_of_nodes, graph.number_of_nodes))
    ...
    def solve(self, instance):
        ...
        # New Constraints related to energy and emissions

        # Energy Consumption Constraints
        for node in graph.nodes:
            model.addCons(energy_vars[node] <= max_energy_consumption[node], name=f"MaxEnergy_{node}")

        # Emission Constraints
        for i in graph.nodes:
            for j in graph.nodes:
                model.addCons(emission_vars[i, j] == emission_rates[i, j] * routing_vars[i, j], name=f"Emission_{i}_{j}")

        # Objective: Minimize total costs + penalty for energy consumption and emissions
        ...
        energy_penalty = quicksum(energy_vars[node] for node in graph.nodes)
        emission_penalty = quicksum(emission_vars[i, j] for i in graph.nodes for j in graph.nodes)

        total_cost = hub_opening_cost + connection_total_cost + warehouse_fixed_cost - knapsack_profit + energy_penalty + emission_penalty
