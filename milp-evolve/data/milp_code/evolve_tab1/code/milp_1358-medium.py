...
class PharmaSupplyChainOptimization:
    ...
    def generate_instance(self):
        ...
        # Generating temperature compliance, carbon emissions, and vendor reliability data
        temp_compliance = np.random.randint(0, 2, (self.num_markets, self.num_customers))
        carbon_emissions = np.random.randint(50, 500, (self.num_markets, self.num_customers))
        vendor_reliabilities = np.random.randint(0, 2, self.num_markets)
        ...
        return {
            ...
            "vendor_reliabilities": vendor_reliabilities
        }
    ...
    def solve(self, instance):
        ...
        temp_compliance = instance['temp_compliance']
        carbon_emissions = instance['carbon_emissions']
        vendor_reliabilities = instance['vendor_reliabilities']
        ...
        temp_compliance_vars = {(m, r): model.addVar(vtype="B", name=f"TempCompliance_{m}_{r}") for m in range(num_markets) for r in range(num_customers)}
        product_waste = {r: model.addVar(vtype="C", name=f"ProductWaste_{r}") for r in range(num_customers)}
        carbon_emission_vars = {(m, r): model.addVar(vtype="C", name=f"CarbonEmission_{m}_{r}") for m in range(num_markets) for r in range(num_customers)}
        vendor_reliability_vars = {v: model.addVar(vtype="B", name=f"VendorReliability_{v}") for v in range(num_markets)}

        # Objective: minimize the total management cost including market rents, transportation costs, and penalties for violations
        model.setObjective(
            ...
            quicksum(carbon_emissions[m, r] * carbon_emission_vars[m, r] for m in range(num_markets) for r in range(num_customers)), "minimize"
        )
        ...
        # Constraints: Only open market stands can cover customers and comply with temperature requirements
        for m in range(num_markets):
            for r in range(num_customers):
                model.addCons(customer_covered[m, r] <= market_open[m], f"Market_{m}_Cover_{r}")
                model.addCons(temp_compliance_vars[m, r] == temp_compliance[m, r], f"TempCompliance_{m}_{r}")
        ...
        # Constraints: Sustainability and waste management
        for r in range(num_customers):
            model.addCons(product_waste[r] >= 0, f"ProductWaste_Non_Negative_{r}")

        for m in range(num_markets):
            for r in range(num_customers):
                model.addCons(carbon_emission_vars[m, r] >= carbon_emissions[m, r] * customer_covered[m, r], f"CarbonEmission_{m}_{r}")

        # Constraints: Vendor reliability
        for m in range(num_markets):
            model.addCons(vendor_reliability_vars[m] == vendor_reliabilities[m], f"VendorReliability_{m}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_management_cost': 1500,
        'max_management_cost': 3000,
        'min_market_coverage': 37,
        'max_market_coverage': 1000,
        'affinity': 3,
    }

    market_optimizer = PharmaSupplyChainOptimization(parameters, seed)
