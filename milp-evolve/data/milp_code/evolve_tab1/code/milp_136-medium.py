...
class EmergencyResponseMILP:
    ...
    def generate_gisp_data(self, G):
        ...
        return inventory_holding_costs, renewable_energy_costs, carbon_emissions
    ...
    def generate_instance(self):
        ...
        inventory_holding_costs, renewable_energy_costs, carbon_emissions = self.generate_gisp_data(G)
    ...
    def solve(self, instance):
        ...
        inventory_level = {f"inv_{node + 1}": model.addVar(vtype="C", name=f"inv_{node + 1}") for node in range(self.n_regions)}
        demand_level = {f"dem_{node + 1}": model.addVar(vtype="C", name=f"dem_{node + 1}") for node in range(self.n_regions)}
        renewable_energy_vars = {f"re_{i + 1}_{j + 1}": model.addVar(vtype="B", name=f"re_{i + 1}_{j + 1}") for (i, j) in edge_list}
        ...
        objective_expr += quicksum(
            ...
            for node in range(self.n_regions)
        )
        objective_expr += quicksum(
            renewable_energy_costs[(i, j)] * renewable_energy_vars[f"re_{i + 1}_{j + 1}"]
            for (i, j) in edge_list
        )
        objective_expr += quicksum(
            carbon_emissions[(i, j)] * y_vars[f"y_{i + 1}_{j + 1}"]
            for (i, j) in edge_list
        )

        # Constraints from the given MILP
        for i in range(self.n_regions):
            for k in range(self.n_demands):
        ...
        for (i, j) in edge_list:
            model.addCons(
                y_vars[f"y_{i + 1}_{j + 1}"] >= renewable_energy_vars[f"re_{i + 1}_{j + 1}"],
                name=f"RE_{i + 1}_{j + 1}"
            )
        ...
        for node in range(self.n_regions):
            ...
            model.addCons(
                inventory_level[f"inv_{node + 1}"] - demand_level[f"dem_{node + 1}"] >= 0,
