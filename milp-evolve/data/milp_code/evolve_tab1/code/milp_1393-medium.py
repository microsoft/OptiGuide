...
class UrbanPlanning:
    ...
    def generate_instance(self):
        ...
        factory_capacity = np.random.randint(self.min_capacity, self.max_capacity, self.Number_of_Zones)
        production_cost = np.random.uniform(self.min_prod_cost, self.max_prod_cost, self.Number_of_Zones)
        demand = np.random.randint(self.min_demand, self.max_demand, self.Number_of_Projects)
        transport_cost = np.random.uniform(self.min_transp_cost, self.max_transp_cost, (self.Number_of_Zones, self.Number_of_Projects))

        emissions = np.random.uniform(self.min_emissions, self.max_emissions, (self.Number_of_Zones, self.Number_of_Projects))
    ...
    def solve(self, instance):
        ...
        production_vars = {(z, p): model.addVar(vtype="C", name=f"Production_Z_{z}_P_{p}") for z in range(number_of_zones) for p in range(number_of_projects)}
        ...
        model.setObjective(
            ...
            quicksum(production_cost[z] * zone_vars[z] for z in range(number_of_zones)) +
            quicksum(transport_cost[z][p] * production_vars[z, p] for z in range(number_of_zones) for p in range(number_of_projects)) +
            quicksum(emissions[z][p] * project_vars[z, p] for z in range(number_of_zones) for p in range(number_of_projects)) +
        ...
        for z in range(number_of_zones):
            model.addCons(quicksum(production_vars[z, p] for p in range(number_of_projects)) <= factory_capacity[z] * zone_vars[z], f"Zone_{z}_Capacity")

        # New Constraints: Total production must meet demand
        for p in range(number_of_projects):
            model.addCons(quicksum(production_vars[z, p] for z in range(number_of_zones)) >= demand[p], f"Project_{p}_Demand")

        # New Constraints: Emissions limit for each zone
        max_emissions = self.Max_Emissions
        for z in range(number_of_zones):
            model.addCons(quicksum(emissions[z, p] * project_vars[z, p] for p in range(number_of_projects)) <= max_emissions * zone_vars[z], f"Zone_{z}_Emissions")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'Project_Cost_Lower_Bound': 1575,
        'Project_Cost_Upper_Bound': 3000,
        'Min_Zone_Cost': 1896,
        'Max_Zone_Cost': 5000,
        'Min_Zone_Capacity': 195,
        'Max_Zone_Capacity': 354,
        'No_of_Scenarios': 20,
        'Requirement_Variation': 0.24,
        'Min_Impact': 0.1,
        'Max_Projects': 3,
        'min_capacity': 400,
        'max_capacity': 1600,
        'min_prod_cost': 900.0,
        'max_prod_cost': 1000.0,
        'min_demand': 50,
        'max_demand': 1800,
        'min_transp_cost': 10.0,
