...
class SimplifiedDataCenterResourceAllocation:
    ...
    def generate_instance(self):
        ...
        # Introducing stochastic demands using normal distribution
        demands = np.random.normal(self.mean_task_demand, self.std_task_demand, self.n_tasks).astype(int) + 1  # using normal distribution for more variation
        ...
        efficiency_penalties = np.random.uniform(low=0.5, high=2.0, size=(self.n_centers, self.n_centers))
        
        # Semi-continuous parameters with stochastic variations
        semi_continuous_lower_bounds = np.random.randint(1, 5, (self.n_centers, self.n_centers))
        semi_continuous_upper_bounds = np.random.randint(10, 20, (self.n_centers, self.n_centers))
        semi_continuous_penalties = np.random.uniform(low=self.min_semi_continuous_penalty, high=self.max_semi_continuous_penalty, size=(self.n_centers, self.n_centers))
        ...
        return {
            ...
            "semi_continuous_lower_bounds": semi_continuous_lower_bounds,
            "semi_continuous_upper_bounds": semi_continuous_upper_bounds,
            "semi_continuous_penalties": semi_continuous_penalties,
        }
    ...
    def solve(self, instance):
        ...
        efficiency_penalties = instance['efficiency_penalties']
        semi_continuous_lower_bounds = instance['semi_continuous_lower_bounds']
        semi_continuous_upper_bounds = instance['semi_continuous_upper_bounds']
        semi_continuous_penalties = instance['semi_continuous_penalties']
        ...
        partial_usage_center_vars = {(i, j): model.addVar(vtype="C", name=f"Partial_Usage_{i}_{j}", lb=semi_continuous_lower_bounds[i, j], ub=semi_continuous_upper_bounds[i, j]) for i in range(n_centers) for j in range(n_centers)}
        ...
        model.setObjective(
            ...
            quicksum(efficiency_penalties[i, j] * flow_vars[i, j] for i in range(n_centers) for j in range(n_centers)) +
            quicksum(semi_continuous_penalties[i, j] * partial_usage_center_vars[i, j] for i in range(n_centers) for j in range(n_centers)),
            "minimize"
        )
        ...
        for i in range(n_centers):
            for j in range(n_centers):
                if i != j:
                    model.addCons(partial_usage_center_vars[i, j] == usage_vars[i, j], f"Semi_Continuous_Usage_{i}_{j}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_task_cost': 1653,
        'max_task_cost': 3000,
        'min_center_cost': 1863,
        'max_center_cost': 5000,
        'min_center_capacity': 713,
        'max_center_capacity': 1919,
        'link_probability': 0.24,
        'flow_capacity_min': 2500,
        'flow_capacity_max': 3000,
        'mean_task_demand': 1,
        'std_task_demand': 0,
        'min_semi_continuous_penalty': 0.24,
        'max_semi_continuous_penalty': 1.0,
    }
