...
class GreenEnergyWarehouseOptimization:
    ...
    def generate_instance(self):
        ...
        time_windows = np.column_stack((
            np.random.randint(start_time_window_interval[0], start_time_window_interval[1], size=graph.number_of_nodes),
            np.random.randint(end_time_window_interval[0], end_time_window_interval[1], size=graph.number_of_nodes)
        ))

        # Vehicle types and capacities
        vehicle_types = np.random.choice(["Type1", "Type2", "Type3"], size=graph.number_of_nodes, p=[0.4, 0.4, 0.2])
    ...
    def solve(self, instance):
        ...
        start_times = {node: model.addVar(vtype="C", name=f"start_time_{node}") for node in graph.nodes}
        ...
        for node in graph.nodes:
            model.addCons(start_times[node] >= time_windows[node, 0], name=f"StartAfter_{node}")
            model.addCons(start_times[node] <= time_windows[node, 1], name=f"EndBefore_{node}")

        # Arrival times based on traffic conditions
        for node in graph.nodes:
            for warehouse in graph.nodes:
                model.addCons(route_vars[node, warehouse] <= warehouse_vars[warehouse], name=f"Tightening_{node}_{warehouse}")
                model.addCons(start_times[node] + distances[node, warehouse] >= start_times[warehouse], name=f"TrafficConditions_{node}_{warehouse}")
