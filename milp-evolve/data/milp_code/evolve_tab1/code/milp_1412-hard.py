...
class GreenEnergyWarehouseOptimization:
    ...
    def solve(self, instance):
        ...
        maximal_cliques = instance['maximal_cliques']
        ...
        # Additional variables for clique constraints
        clique_vars = {clique_index: model.addVar(vtype="B", name=f"clique_{clique_index}") for clique_index in range(len(maximal_cliques))}
        ...
        for clique_index, clique in enumerate(maximal_cliques):
            model.addCons(quicksum(warehouse_vars[node] for node in clique) <= len(clique) - 1 + clique_vars[clique_index], name=f"Clique_{clique_index}")
            model.addCons(clique_vars[clique_index] <= 1, name=f"CliqueBinary_{clique_index}")
