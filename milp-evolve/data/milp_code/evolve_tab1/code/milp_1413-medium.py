...
class FleetManagement:
    ...
    def generate_instance(self):
        ...
        # Additional Drone Delivery related data generation
        time_windows = np.random.randint(30, 100, size=self.Number_of_Routes)
        battery_usage_rates = np.random.uniform(0.5, 1.5, self.Number_of_Trucks)
        weather_effects = np.random.uniform(0.8, 1.2, self.Number_of_Trucks)
        reliability_indices = np.random.uniform(0.8, 1.0, self.Number_of_Trucks)
    ...
    def solve(self, instance):
        ...
        reliability_indices = instance['reliability_indices']
        ...
        penalty_vars = {(t, r): model.addVar(vtype="C", name=f"Penalty_{t}_Route_{r}") for t in range(number_of_trucks) for r in range(number_of_routes)}
        
        # Objective: minimize the total cost including truck startup costs, route service costs, and penalties for late deliveries
        model.setObjective(
            quicksum(truck_costs[t] * truck_vars[t] for t in range(number_of_trucks)) +
            quicksum(route_costs[t, r] * route_vars[t, r] for t in range(number_of_trucks) for r in range(number_of_routes)) +
            quicksum(penalty_vars[t, r] for t in range(number_of_trucks) for r in range(number_of_routes)) +
            max_delivery_time * 10 - quicksum(reliability_indices[t] for t in range(number_of_trucks)),
            "minimize"
        )
        
        ### Introduction of new auxiliary variables for the convex hull formulation ###
        aux_vars = {(t, r): model.addVar(vtype="B", name=f"Aux_{t}_{r}") for t in range(number_of_trucks) for r in range(number_of_routes)}
        ...
        for r in range(number_of_routes):
            for t in range(number_of_trucks):
                model.addCons(route_vars[t, r] * time_windows[r] * weather_effects[t] <= time_windows[r] + penalty_vars[t, r], f"Time_Window_{t}_{r}")

        # Max delivery time constraint
        for t in range(number_of_trucks):
            for r in range(number_of_routes):
                model.addCons(route_vars[t, r] * time_windows[r] * weather_effects[t] <= max_delivery_time, f"Max_Delivery_Time_Constraint_{t}_{r}")

        # Battery usage constraints
        for t in range(number_of_trucks):
            model.addCons(quicksum(route_vars[t, r] * battery_usage_rates[t] for r in range(number_of_routes)) <= truck_vars[t] * M, f"Battery_Usage_{t}")
