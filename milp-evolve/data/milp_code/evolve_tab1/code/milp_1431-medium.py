...
class ResidentialZoneDevelopment:
    ...
    def generate_instance(self):
        ...
        flow_capacities = np.random.randint(1, 20, (self.n_markets, self.n_zones))
        flow_costs = np.random.uniform(0.5, 1.5, (self.n_markets, self.n_zones))
    ...
    def solve(self, instance):
        ...
        flow_vars = {(m, z): model.addVar(vtype="I", name=f"Flow_{m}_{z}") for m in range(n_markets) for z in range(n_zones)}
        ...
        model.setObjective(
            ...
            quicksum(flow_costs[m, z] * flow_vars[m, z] for m in range(n_markets) for z in range(n_zones)),
            "minimize"
        )

        # Constraints: Each zone demand is met by the flow from markets
        for z in range(n_zones):
            model.addCons(quicksum(flow_vars[m, z] for m in range(n_markets)) == demands[z], f"Zone_{z}_Demand")
        
        # Constraints: Only open markets can serve zones and provide flows
        for m in range(n_markets):
            for z in range(n_zones):
                model.addCons(zone_vars[m, z] <= market_vars[m], f"Market_{m}_Serve_{z}")
                model.addCons(flow_vars[m, z] <= flow_capacities[m, z] * market_vars[m], f"Flow_Capacity_{m}_{z}")

        # Constraints: Markets cannot exceed their capacities
        for m in range(n_markets):
            model.addCons(quicksum(flow_vars[m, z] for z in range(n_zones)) <= capacities[m], f"Market_{m}_Capacity")
