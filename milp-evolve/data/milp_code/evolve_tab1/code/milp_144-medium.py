...
class IndependentSet:
    ...
    def generate_instance(self):
        ...
        # Define piecewise linear parameters
        piecewise_params = {}
        for i in range(self.n_piecewise_nodes):
            node = random.randint(0, self.n_nodes - 1)
            slopes = np.random.uniform(0.5, 1.5, self.piecewise_segments)
            intercepts = np.random.uniform(0, 1, self.piecewise_segments)
            piecewise_params[node] = (slopes, intercepts)
        
        res['piecewise_params'] = piecewise_params
    ...
    def solve(self, instance):
        ...
        piecewise_params = instance['piecewise_params']
        ...
        for node, (slopes, intercepts) in piecewise_params.items():
            piecewise_var = model.addVar(vtype="C", name=f"pw_{node}")
            model.addCons(piecewise_var == quicksum(slopes[i] * var_names[node] + intercepts[i] for i in range(len(slopes))), name=f"piecewise_{node}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'n_exclusive_pairs': 900,
        'n_piecewise_nodes': 37,
        'piecewise_segments': 3,
    }
