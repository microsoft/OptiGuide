...
class MultipleKnapsack:
    ...
    def generate_instance(self):
        ...
        # New Data: Reliability and Downtime Parameters
        reliability_factors = np.random.uniform(0.95, 1.0, self.number_of_knapsacks)  # Reliability close to 1
        downtime_penalties = np.random.uniform(1, 5, self.number_of_knapsacks)  # Downtime penalties between 1 and 5

        res = {'weights': weights, 
               'profits': profits, 
               'capacities': capacities,
               'reliability_factors': reliability_factors,
               'downtime_penalties': downtime_penalties}
    ...
    def solve(self, instance):
        ...
        downtime_penalties = instance['downtime_penalties']
        ...
        # Objective: Maximize total profit considering downtime penalty
        objective_expr = quicksum((profits[i] * (j+1)) * var_names[(i, j)] for i in range(number_of_items) for j in range(number_of_knapsacks))
        downtime_cost = quicksum(downtime_penalties[j] * (1 - reliability_factors[j]) for j in range(number_of_knapsacks))
        objective_expr -= downtime_cost
