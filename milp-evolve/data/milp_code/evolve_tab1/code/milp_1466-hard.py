...
class NetworkFlowOptimization:
    ...
    def solve(self, instance):
        ...
        total_nodes = n_warehouses + n_stores

        # Decision variables
        flow_vars = {(w, s): model.addVar(vtype="C", name=f"Flow_{w}_{s}") for w in range(n_warehouses) for s in range(n_stores)}
        open_vars = {w: model.addVar(vtype="B", name=f"Open_{w}") for w in range(n_warehouses)}

        # Objective: minimize the total cost including warehouse costs, store costs, and supply costs.
        model.setObjective(
        ...
        # Constraints: Flow conservation at the warehouses
        for w in range(n_warehouses):
            model.addCons(quicksum(flow_vars[w, s] for s in range(n_stores)) <= capacities[w] * open_vars[w], f"Warehouse_{w}_Capacity")
        
        # Flow conservation at the stores: meet all demands
        for s in range(n_stores):
            model.addCons(quicksum(flow_vars[w, s] for w in range(n_warehouses)) == demands[s], f"Store_{s}_Demand")

        # Service distance constraint: only warehouses within a given distance can serve stores
        for s in range(n_stores):
...
if __name__ == '__main__':
    ...
    network_flow_optimizer = NetworkFlowOptimization(parameters, seed=seed)
    instance = network_flow_optimizer.generate_instance()
    solve_status, solve_time, objective_value = network_flow_optimizer.solve(instance)
