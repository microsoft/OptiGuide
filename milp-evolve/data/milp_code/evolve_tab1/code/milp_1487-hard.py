...
class SupplyChainOptimization:
    ...
    def generate_instance(self):
        ...
        # Generate additional adjacency node-pairs for extra constraints
        extra_adjacency_pairs = list(combinations(range(self.n_warehouses), 2))
        np.random.shuffle(extra_adjacency_pairs)
        extra_adjacency_pairs = extra_adjacency_pairs[:self.extra_adjacency_pairs]
        ...
        return {
            ...
            "graph": graph,
            "cliques": cliques,
            "extra_adjacency_pairs": extra_adjacency_pairs,
        }
    ...
    def solve(self, instance):
        ...
        extra_adjacency_pairs = instance['extra_adjacency_pairs']
        ...
        clique_open_vars = {i: model.addVar(vtype="B", name=f"Clique_{i}_Open") for i in range(len(cliques))}
        ...
        for count, clique in enumerate(cliques):
            model.addCons(quicksum(open_vars[node] for node in clique) <= clique_open_vars[count], f"Clique_{count}")

        # Introduce clique variables to ensure that at most one warehouse in each clique is opened
        for count in range(len(cliques)):
            model.addCons(clique_open_vars[count] <= 1, f"Clique_{count}_Open_Constraint")

        # Additional pairwise adjacency constraints
        for i, (u, v) in enumerate(extra_adjacency_pairs):
            model.addCons(open_vars[u] + open_vars[v] <= 1, f"Extra_Adjacency_{i}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_supply_limit': 2100,
        'max_supply_limit': 2250,
        'max_distance': 3000,
        'affinity': 77,
        'extra_adjacency_pairs': 1750,
    }
