...
class FCMCNF:
    ...
    def solve(self, instance):
        ...
        # Convex Hull formulation constraints for remote locations and logical routing
        for (i, j) in edge_list:
            model.addCons(
                is_remote_vars[f"is_remote_{i+1}_{j+1}"] <= y_vars[f"y_{i+1}_{j+1}"],
                f"ConvexHull_Remote_Location_{i+1}_{j+1}"
            )

            # Strengthened fuel consumption constraints using convex hull properties
            model.addCons(
                remote_conditions[(i, j)]['fuel_consumption'] * y_vars[f"y_{i+1}_{j+1}"] <=
                self.max_fuel_budget * z_vars[f"z_{i+1}_{j+1}"],
                f"ConvexHull_Fuel_Consumption_{i+1}_{j+1}"
            )

            # Ensuring logical routing through remote locations via convex hull approach
            model.addCons(
                z_vars[f"z_{i+1}_{j+1}"] >= y_vars[f"y_{i+1}_{j+1}"] + is_remote_vars[f"is_remote_{i+1}_{j+1}"] - 1,
                f"ConvexHull_Logical_Routing_{i+1}_{j+1}"
            )

            # Driving hours constraint using convex hull logic
            model.addCons(
                remote_conditions[(i, j)]['driving_hours'] * y_vars[f"y_{i+1}_{j+1}"] <=
                0.2 * self.regular_working_hours * z_vars[f"z_{i+1}_{j+1}"],
                f"ConvexHull_Driving_Hours_{i+1}_{j+1}"
            )

            # Big M constraints to enhance convex hull constraints
            model.addCons(
                quicksum(x_vars[f"x_{i+1}_{j+1}_{k+1}"] for k in range(self.n_commodities)) <= self.big_m_value * z_vars[f"z_{i+1}_{j+1}"],
                f"ConvexHull_BigM_{i+1}_{j+1}"
            )
