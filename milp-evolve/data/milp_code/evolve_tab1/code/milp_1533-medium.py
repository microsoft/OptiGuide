...
class HealthcareResourceOptimization:
    ...
    def generate_instance(self):
        ...
        shift_lengths = np.random.randint(self.min_shift_length, self.max_shift_length + 1, self.n_departments)
    ...
    def solve(self, instance):
        ...
        shift_adhere = {c: model.addVar(vtype="I", name=f"Shift_Adherence_{c}") for c in range(n_departments)}
        ...
        model.setObjective(
            ...
            quicksum(shift_lengths[i] * shift_adhere[i] for i in range(n_departments)),
            "minimize"
        )
        ...
        # New constraints for shift adherence using Convex Hull Formulation
        for c in range(n_departments):
            model.addCons(shift_adhere[c] >= 0, f"Shift_Positive_{c}")
            model.addCons(shift_adhere[c] <= 1, f"Shift_Less_One_{c}")
            model.addCons(
                quicksum(medical_supplies[c, s] for s in range(n_supplies)) >= self.min_shift_adherence * shift_adhere[c],
                f"Convex_Hull_Shift_{c}"
            )
