...
class HumanitarianAidDistribution:
    ...
    def solve(self, instance):
        ...
        # Clique Inequalities to prevent sub-tours
        for node_subset in combinations(graph.nodes, 3):  # Using subsets of size 3 for simplicity
            model.addCons(
                quicksum(HealthcareRouting_vars[i, j] for i, j in combinations(node_subset, 2)) <= len(node_subset) - 1,
                name=f"CliqueInequality_{node_subset}"
            )
