...
class HealthcareFacilityLocation:
    ...
    def solve(self, instance):
        ...
        M = 1e3  # A large constant for the Big M formulation
        ...
        # Delivery time window satisfaction variables
        ...
        # Healthcare center opening variables
        ...
        # Link service time with delivery window satisfaction variables using Big M formulation
        for node in graph.nodes:
            model.addCons(ServiceTime_vars[node] <= M * DeliveryWindowSatisfaction_vars[node], name=f"DeliveryWindowLink_{node}")
        ...
        model.addCons(quicksum(MedicalCenterSelection_vars[node] for node in graph.nodes) <= healthcare_center_limit, name="HealthcareCenterLimit")
