...
class ConstructionResourceAllocation:
    ...
    def generate_instance(self):
        ...
        # Create precedence constraints with given probability
        precedence_matrix = np.random.choice([0, 1], size=(self.n_projects, self.n_projects), p=[1 - self.prec_prob, self.prec_prob])
    ...
    def solve(self, instance):
        ...
        precedence_matrix = instance['precedence_matrix']
        ...
        shift_vars = {(c, s): model.addVar(vtype="B", name=f"Shift_{c}_{s}") for c in range(n_sites) for s in range(n_shifts)}
        precedence_vars = {(p1, p2): model.addVar(vtype="B", name=f"Precedence_{p1}_{p2}") for p1 in range(n_projects) for p2 in range(n_projects)}
        ...
        # New Constraints for Shifts
        for c in range(n_sites):
            for s in range(n_shifts):
                model.addCons(
                    quicksum(project_vars[c, p] for p in range(n_projects)) <= capacities[c] * shift_vars[(c, s)],
                    f"Shift_Capacity_{c}_{s}"
                )

        # New Constraints for Precedence
        for p1 in range(n_projects):
            for p2 in range(n_projects):
                if precedence_matrix[p1, p2] == 1:
                    model.addCons(
                        quicksum(project_vars[c, p1] for c in range(n_sites)) -
                        quicksum(project_vars[c, p2] for c in range(n_sites)) <= 0,
                        f"Precedence_{p1}_{p2}"
                    )
...
if __name__ == '__main__':
    ...
    parameters = {
        'n_sites': 36,
        'n_projects': 63,
        'min_project_cost': 413,
        'max_project_cost': 3000,
        'min_hiring_cost': 698,
        'max_hiring_cost': 5000,
        'min_site_capacity': 590,
        'max_site_capacity': 1077,
        'link_probability': 0.38,
        'transportation_capacity_min': 1316,
        'transportation_capacity_max': 3000,
        'battery_life_min': 0.24,
        'battery_life_max': 14.0,
