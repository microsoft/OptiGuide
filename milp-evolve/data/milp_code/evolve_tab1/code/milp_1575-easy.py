...
class CapacitatedFacilityLocation:
    ...
    def solve(self, instance):
        ...
        # Objective: minimize the total cost including emission penalties
        objective_expr = quicksum(fixed_costs[j] * open_facilities[j] for j in range(n_facilities)) + quicksum(transportation_costs[i, j] * serve[i, j] for i in range(n_customers) for j in range(n_facilities)) + quicksum(emissions[i] * serve[i, j] for i in range(n_customers) for j in range(n_facilities))
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'emission_interval': (1, 10),
        'emission_limit_interval': (50, 100)
    }
