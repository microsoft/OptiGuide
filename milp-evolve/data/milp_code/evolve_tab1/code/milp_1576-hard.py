...
class MaxSatisfiability:
    ...
    def solve(self, instance):
        ...
        # Add semi-continuous variables and new constraints
        L, U = self.semi_cont_l, self.semi_cont_u
        clause_satisfaction_lvl = {
            f"cl_s_lvl_{idx}": model.addVar(lb=0, ub=U, vtype="CONTINUOUS", name=f"cl_s_lvl_{idx}") 
            for idx in range(len(clauses))
        }

        for idx, (clause, weight) in enumerate(clauses):
            clause_var = var_names[f"cl_{idx}"]
            model.addCons(clause_satisfaction_lvl[f"cl_s_lvl_{idx}"] >= L * clause_var, name=f"L_bound_s_lvl_{idx}")
            model.addCons(clause_satisfaction_lvl[f"cl_s_lvl_{idx}"] <= U * clause_var, name=f"U_bound_s_lvl_{idx}")
