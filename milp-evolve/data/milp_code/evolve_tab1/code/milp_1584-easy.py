...
class CityHealthcareResourceOptimization:
    ...
    def get_instance(self):
        ...
        mutual_exclusivity_pairs = [(random.randint(0, self.n_hospitals - 1), random.randint(0, self.n_hospitals - 1)) for _ in range(self.n_exclusive_pairs)]
    ...
    def solve(self, instance):
        ...
        waste_vars = {h: model.addVar(vtype="C", name=f"Waste_{h}") for h in range(n_hospitals)}
        ...
        model.setObjective(
            ...
            quicksum(self.waste_penalty * waste_vars[h] for h in range(n_hospitals)),
            "minimize"
        )
        ...
        # New mutual exclusivity constraints
        for (h1, h2) in mutual_exclusivity_pairs:
            model.addCons(neighborhood_alloc[h1] + neighborhood_alloc[h2] <= 1, f"Mutual_Exclusive_{h1}_{h2}")
        
        # Waste constraints
        for h in range(n_hospitals):
            model.addCons(waste_vars[h] >= 0, f"Waste_LB_{h}")
            model.addCons(waste_vars[h] >= self.waste_factor * (1 - neighborhood_alloc[h]), f"Waste_Link_{h}")
...
if __name__ == '__main__':
    ...
    parameters = {
        'n_hospitals': 24,
        'n_vaccines': 252,
        'min_vaccine_cost': 464,
        'max_vaccine_cost': 3000,
        'min_personnel_cost': 698,
        'max_personnel_cost': 5000,
        'min_hospital_capacity': 590,
        'max_hospital_capacity': 1077,
