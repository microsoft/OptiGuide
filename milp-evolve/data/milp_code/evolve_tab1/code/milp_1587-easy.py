...
class HumanitarianAidDistribution:
    ...
    def get_instance(self):
        ...
        # Supply Chain data
        warehouse_count = self.n_warehouses
        medical_center_count = graph.number_of_nodes
        min_warehouse_cost = self.min_warehouse_cost
        max_warehouse_cost = self.max_warehouse_cost
        min_store_cost = self.min_store_cost
        max_store_cost = self.max_store_cost
        min_warehouse_cap = self.min_warehouse_cap
        max_warehouse_cap = self.max_warehouse_cap
        max_distance = self.max_distance

        warehouse_opening_costs = np.random.randint(min_warehouse_cost, max_warehouse_cost + 1, warehouse_count)
        warehouse_to_medical_center_costs = np.random.randint(min_store_cost, max_store_cost + 1, (warehouse_count, medical_center_count))
        warehouse_capacities = np.random.randint(min_warehouse_cap, max_warehouse_cap + 1, warehouse_count)
        supply_limits = np.random.uniform(self.min_supply_limit, self.max_supply_limit, warehouse_count)
        
        distances = np.random.uniform(0, max_distance, (warehouse_count, medical_center_count))
        ...
        return {
            ...
            'warehouse_opening_costs': warehouse_opening_costs,
            'warehouse_to_medical_center_costs': warehouse_to_medical_center_costs,
    ...
    def solve(self, instance):
        ...
        warehouse_opening_costs = instance['warehouse_opening_costs']
        warehouse_to_medical_center_costs = instance['warehouse_to_medical_center_costs']
        ...
        # Add warehouse and flow variables
        WarehouseOpening_vars = {w: model.addVar(vtype="B", name=f"Warehouse_{w}") for w in range(self.n_warehouses)}
        Flow_vars = {(w, mc): model.addVar(vtype="C", name=f"Flow_{w}_{mc}") for w in range(self.n_warehouses) for mc in graph.nodes}
        ...
        ### New constraints from the second MILP ###
        
        # Supply Chain Constraints
        
        # Constraints: Each medical center's demand is met by the warehouses
        for mc in graph.nodes:
            model.addCons(quicksum(Flow_vars[w, mc] for w in range(self.n_warehouses)) == critical_care_demand[mc], f"MedicalCenter_{mc}_Demand")
        
        # Constraints: Only open warehouses can supply products
        for w in range(self.n_warehouses):
            for mc in graph.nodes:
                model.addCons(Flow_vars[w, mc] <= supply_limits[w] * WarehouseOpening_vars[w], f"Warehouse_{w}_Serve_{mc}")
        
        # Constraints: Warehouses cannot exceed their capacities
        for w in range(self.n_warehouses):
            model.addCons(quicksum(Flow_vars[w, mc] for mc in graph.nodes) <= warehouse_capacities[w], f"Warehouse_{w}_Capacity")

        # New constraint: Service distance constraint (Set Covering)
        for mc in graph.nodes:
            model.addCons(quicksum(WarehouseOpening_vars[w] for w in range(self.n_warehouses) if distances[w, mc] <= self.max_distance) >= 1, f"MedicalCenter_{mc}_Covered")
        ...
        # New cost components
        warehouse_opening_cost = quicksum(WarehouseOpening_vars[w] * warehouse_opening_costs[w] for w in range(self.n_warehouses))
        warehouse_to_medical_center_cost = quicksum(Flow_vars[w, mc] * warehouse_to_medical_center_costs[w][mc] for w in range(self.n_warehouses) for mc in graph.nodes)

        total_cost = zoning_cost + transportation_cost + theft_risk_cost + renewable_energy_cost + hazardous_material_cost - auction_revenue + warehouse_opening_cost + warehouse_to_medical_center_cost
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_store_cost': 300,
        'max_store_cost': 2000,
        'min_warehouse_cap': 3000,
        'max_warehouse_cap': 3000,
        'min_supply_limit': 450,
        'max_supply_limit': 2250,
        'max_distance': 250,
    }
