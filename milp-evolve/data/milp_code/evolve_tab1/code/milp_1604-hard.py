...
class EmergencyShelterAllocation:
    ...
    def solve(self, instance):
        ...
        Evacuation_vars = {f: model.addVar(vtype="B", name=f"Evacuation_{f}") for f in range(n_locations)}
        ...
        # Constraints: Ensure evacuations are scheduled for all locations
        for l in range(n_locations):
            model.addCons(
                Evacuation_vars[l] == 1, 
                f"Location_{l}_EvacuationScheduling"
            )

        # Coverage constraint for Zero Carbon Transport for shelters within specified distance
        for l in range(n_locations):
            model.addCons(
                quicksum(EmergencyShelter_vars[e] for e in range(n_shelters) if distances[e, l] <= self.max_zero_carbon_dist) >= 1, 
                f"Location_{l}_ZeroCarbonTransport"
            )
...
if __name__ == '__main__':
    ...
    parameters = {
        'n_shelters': 420,
        'n_locations': 33,
        'min_volunteer_cost': 25,
        'max_volunteer_cost': 75,
        'min_op_cost': 1000,
        'max_op_cost': 1500,
        'min_shelter_capacity': 500,
        'max_shelter_capacity': 2500,
        'min_budget_limit': 5000,
        'max_budget_limit': 10000,
        'max_dist': 500,
        'max_zero_carbon_dist': 50,
    }
