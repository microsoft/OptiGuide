...
class HRPA:
    ...
    def solve(self, instance):
        ...
        # New flow variables
        flow_vars = {f"f_{u}_{v}": model.addVar(vtype="C", name=f"f_{u}_{v}") for u, v in G.edges}
        ...
        objective_expr -= quicksum(
            G[u][v]['compatibility_cost'] * flow_vars[f"f_{u}_{v}"]
        ...
        objective_expr -= quicksum(
            medication_compatibility[(u, v)] * flow_vars[f"f_{u}_{v}"]
            for u, v in G.edges
        )
        ...
        for u, v in G.edges:
            model.addCons(
                flow_vars[f"f_{u}_{v}"] >= compatibility_vars[f"comp_{u}_{v}"],
                name=f"Comp_Resource_{u}_{v}"
            )

        for node in G.nodes:
            model.addCons(
                quicksum(flow_vars[f"f_{u}_{node}"] for u in G.predecessors(node)) - quicksum(flow_vars[f"f_{node}_{v}"] for v in G.successors(node)) == 0,
