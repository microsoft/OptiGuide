...
class WarehouseAllocationOptimization:
    ...
    def generate_instance(self):
        ...
        # Semi-continuous variables bounds
        semi_continuous_lower_bounds = np.random.randint(50, 100, size=graph.number_of_nodes)
        semi_continuous_upper_bounds = np.random.randint(200, 300, size=graph.number_of_nodes)
    ...
    def solve(self, instance):
        ...
        semi_continuous_vars = {node: model.addVar(vtype="C", lb=0, ub=semi_continuous_upper_bounds[node], name=f"SemiContinuousWarehouse_{node}") for node in graph.nodes}
        ...
        # Budget constraints
        total_cost = quicksum(semi_continuous_vars[node] for node in graph.nodes) + \
        ...
        # Semi-continuous warehouse setup costs constraints
        for node in graph.nodes:
            model.addCons(semi_continuous_vars[node] >= semi_continuous_lower_bounds[node] * warehouse_vars[node], name=f"SemiContinuousLowerBound_{node}")
