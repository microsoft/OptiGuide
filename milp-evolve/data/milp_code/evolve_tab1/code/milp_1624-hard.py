...
class MobileHealthcareOptimization:
    ...
    def generate_instance(self):
        ...
        fuel_costs = np.random.uniform(self.min_fuel_cost, self.max_fuel_cost, (self.n_mobile_units, self.n_neighborhoods))
        delay_factors = np.random.uniform(self.min_delay_factor, self.max_delay_factor, self.n_neighborhoods)
    ...
    def solve(self, instance):
        ...
        electric_vars = {m: model.addVar(vtype="B", name=f"Electric_{m}") for m in range(n_mobile_units)}
        hybrid_vars = {m: model.addVar(vtype="B", name=f"Hybrid_{m}") for m in range(n_mobile_units)}
        combustion_vars = {m: model.addVar(vtype="B", name=f"Combustion_{m}") for m in range(n_mobile_units)}

        # Objective: minimize the total cost including operational costs, transport costs, warehouse costs, and fuel costs.
        model.setObjective(
            ...
            quicksum(fuel_costs[m, int(v.split('_')[1])] * schedule_vars[(u, v)] for (u, v) in node_pairs for m in range(n_mobile_units) if u == f'mobile_unit_{m}') +
        ...
        model.addCons(
            quicksum(electric_vars[m] + hybrid_vars[m] for m in range(n_mobile_units)) >= 0.4 * n_mobile_units,
            f"FleetComposition"
        )
        ...
        # Penalize delivery delays due to traffic in each region
        for n in range(n_neighborhoods):
            delay_factor = delay_factors[n]
            model.addCons(
                quicksum(schedule_vars[(u, f"neighborhood_{n}")] * delay_factor for u in G.predecessors(f"neighborhood_{n}")) <= neighborhood_demands[n] * (1 + delay_factor),
                f"DeliveryDelayPenalty_Neighborhood_{n}"
            )
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_operational_cost': 93,
        'max_operational_cost': 1687,
        'min_supply_limit': 675,
        'max_supply_limit': 3000,
        'min_service_level': 0.52,
        'max_service_level': 15.75,
        'max_service_distance': 125,
        'efficient_schedule_factor': 60.0,
        'heavy_load_factor': 1125,
        'city_size': 3000,
        'n_warehouses': 5,
        'min_warehouse_cost': 2,
        'max_warehouse_cost': 5000,
        'min_delivery_cost': 6,
        'max_delivery_cost': 100,
        'min_warehouse_capacity': 250,
        'max_warehouse_capacity': 3000,
        'min_fuel_cost': 0.75,
        'max_fuel_cost': 1.25,
        'min_delay_factor': 0.31,
        'max_delay_factor': 0.59,
    }
