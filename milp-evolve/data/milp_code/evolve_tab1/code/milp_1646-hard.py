...
class EVDeliveryOptimization:
    ...
    def generate_instance(self):
        ...
        for _ in range(self.num_cliques):
            size = np.random.randint(2, min(self.n_charging_stations, self.clique_max_size))
            cliques.append(np.random.choice(self.n_charging_stations, size, replace=False).tolist())
    ...
    def solve(self, instance):
        ...
        for i, clique in enumerate(cliques):
            for p1 in clique:
                for p2 in clique:
                    if p1 != p2:
                        model.addCons(ev_vars[p1] + ev_vars[p2] <= clique_vars[i], f"Clique_{i}_Edge_{p1}_{p2}")
