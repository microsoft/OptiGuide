...
class VaccineDistributionOptimization:
    ...
    def generate_instance(self):
        ...
        # New instance data from the second MILP
        ...
        delivery_times = np.random.randint(self.min_delivery_time, self.max_delivery_time + 1, (self.n_production_zones, self.n_distribution_zones))
    ...
    def solve(self, instance):
        ...
        # Additional decision variables for vaccine expiry penalties and delivery times
        expiry_penalty_vars = {d: model.addVar(vtype="C", name=f"ExpiryPenalty_{d}") for d in range(n_distribution_zones)}
        delivery_time_penalty = {(u, v): model.addVar(vtype="C", name=f"DeliveryTimePenalty_{u}_{v}") for u, v in node_pairs}

        # Objective: minimize the total cost including production zone costs, transport costs, shipment costs, inventory holding costs, penalties for expiry and delivery times.
        model.setObjective(
            ...
            quicksum(delivery_time_penalty[(u, v)] for (u, v) in node_pairs),
            "minimize"
        )
        ...
        # Constraints for equipment use
        for p in range(n_production_zones):
            model.addCons(equip_vars[p] <= vaccine_vars[p], f"Equip_Use_Constraint_{p}")

        # Constraints for patient groups and delivery times
        for p in range(n_production_zones):
            for d in range(n_distribution_zones):
                ...
                model.addCons(delivery_time_penalty[(f"production_{p}", f"distribution_{d}")] >= delivery_times[p, d] - self.max_delivery_time, f"DeliveryTimeConstraint_{p}_{d}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'n_distribution_zones': 150,
        'min_transport_cost': 2400,
        'max_transport_cost': 2700,
        'min_zone_cost': 450,
        'max_zone_cost': 2106,
        'min_production_capacity': 306,
        'max_production_capacity': 421,
        'min_budget_limit': 455,
        'max_budget_limit': 1201,
        'max_transport_distance': 708,
        'holding_cost_mean': 225.0,
        ...
        'max_expiry_days': 60,
        'min_delivery_time': 3,
        'max_delivery_time': 126,
        'equipment_use_threshold': 14,
        'patient_groups_limit': 225,
    }
