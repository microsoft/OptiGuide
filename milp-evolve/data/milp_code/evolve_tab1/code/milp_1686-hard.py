...
class DisasterResponseOptimization:
    ...
    def generate_instance(self):
        ...
        renewable_energy_capacity = np.random.randint(self.min_renewable_energy_capacity, self.max_renewable_energy_capacity + 1, self.n_charging_stations)
        weather_impact = np.random.uniform(0, 1, (self.n_charging_stations, self.n_customers))
    ...
    def solve(self, instance):
        ...
        renewable_energy_capacity = instance['renewable_energy_capacity']
        ...
        renewable_use = {p: model.addVar(vtype="C", name=f"RenewableUse_{p}") for p in range(n_charging_stations)}
        ...
        # Renewable energy use constraints
        for p in range(n_charging_stations):
            model.addCons(
                renewable_use[p] <= renewable_energy_capacity[p] * ev_vars[p],
                f"Charging_{p}_MaxRenewableEnergyCapacity"
            )

        # Weather impact on road accessibility
        for p in range(n_charging_stations):
            for d in range(n_customers):
                model.addCons(
                    distance_vars[(f"charging_{p}", f"customer_{d}")] <= (1 - weather_impact[p, d]) * ev_vars[p],
                    f"Charging_{p}_WeatherImpact_{d}"
                )
...
if __name__ == '__main__':
    seed = 42
    parameters = {
        'n_charging_stations': 270,
        'n_customers': 78,
        'min_travel_cost': 2250,
        'max_travel_cost': 3000,
        'min_energy_cost': 2250,
        'max_energy_cost': 3000,
        'min_battery_capacity': 500,
        'max_battery_capacity': 900,
        'min_emission_limit': 200,
        'max_emission_limit': 900,
        'min_renewable_energy_capacity': 1400,
        'max_renewable_energy_capacity': 2400,
    }

    optimizer = DisasterResponseOptimization(parameters, seed=seed)
