...
class EggDistributionOptimization:
    ...
    def solve(self, instance):
        ...
        # New Constraints: Ensure equipment use and handle breeding costs using Big M Formulation
        for p in range(num_farms):
            # Big M constraint for equipment use
            model.addCons(equip_vars[p] <= M * egg_production_vars[p], f"EquipUse_BigM_Constraint_{p}")
            model.addCons(equip_vars[p] >= 1 - M * (1 - egg_production_vars[p]), f"EquipUse_Reverse_BigM_Constraint {p}")
            
            # Handle breeding costs
