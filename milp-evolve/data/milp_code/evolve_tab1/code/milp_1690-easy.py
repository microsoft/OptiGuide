...
class WarehouseAllocationOptimization:
    ...
    def generate_instance(self):
        ...
        # Clique constraints generation
        cliques = list(nx.find_cliques(nx.Graph(graph.edges)))
        # Filtering out cliques of size 2 to focus on larger cliques for variability in difficulty
    ...
    def solve(self, instance):
        ...
        for clique in cliques:
            for i, j in combinations(clique, 2):
                model.addCons(delivery_vars[i, j] + delivery_vars[j, i] <= 1, name=f"Clique_{i}_{j}")
