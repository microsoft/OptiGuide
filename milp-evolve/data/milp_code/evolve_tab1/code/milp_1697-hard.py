...
class FCMCNF:
    ...
    def solve(self, instance):
        ...
        # New Big M variables
        m_vars = {(i, j, k): model.addVar(vtype="B", name=f"m_{i+1}_{j+1}_{k+1}") for (i, j) in edge_list for k in range(self.n_commodities)}
        ...
        for (i, j) in edge_list:
            ...
            for k in range(self.n_commodities):
                arc_m_expr = quicksum(x_vars[f"x_{i+1}_{j+1}_{k+1}"] for k in range(self.n_commodities))
                model.addCons(arc_m_expr <= big_m * m_vars[(i, j, k)], f"big_m_activate_{i+1}_{j+1}_{k+1}")
                model.addCons(m_vars[(i, j, k)] <= open_vars[i], f"open_activation_{i+1}_{j+1}_{k+1}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'max_region_benefit': 280,
        'min_facility_cap': 100,
        'max_facility_cap': 700,
        'big_m': 10000,
    }
