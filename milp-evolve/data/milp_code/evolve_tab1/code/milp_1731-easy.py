...
class VaccineDistributionOptimization:
    ...
    def solve(self, instance):
        ...
        # Additional Variables for Symmetry Breaking
        is_used_production = {p: model.addVar(vtype="B", name=f"IsUsedProduction_{p}") for p in range(n_production_zones)}
        ...
        model.setObjective(
            quicksum(zone_costs[p] * is_used_production[p] for p in range(n_production_zones)) +
        ...
        # Symmetry Breaking Constraint: Ensure the production zones are chosen in order
        for p in range(1, n_production_zones):
            model.addCons(
                is_used_production[p - 1] >= is_used_production[p],
                f"SymmetryBreaking_{p}"
            )

        # Linking is_used_production with vaccine_vars to ensure consistency
        for p in range(n_production_zones):
            model.addCons(
                vaccine_vars[p] == is_used_production[p],
                f"Link_VaccineVars_{p}"
            )
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_zone_cost': 450,
        'max_zone_cost': 1184,
        'min_production_capacity': 80,
        'max_production_capacity': 105,
        'min_budget_limit': 33,
        'max_budget_limit': 600,
        'max_transport_distance': 2124,
        'holding_cost_mean': 900.0,
        'holding_cost_sd': 12.5,
        'demand_mean': 1181.6,
        'demand_sd': 0.8,
    }
