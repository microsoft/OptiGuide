...
class FCMCNF:
    ...
    def generate_instance(self):
        ...
        # Generate data for facility location and flow threshold
        ...
        flow_thresholds = np.random.randint(self.min_flow_thr, self.max_flow_thr + 1, self.n_facilities)
        ...
        # Additional data for piecewise linear cost functions
        self.piecewise_points = np.random.randint(self.min_piecewise_points, self.max_piecewise_points + 1)
        piecewise_costs = { 
            (i, j): [np.random.uniform(*self.c_range) for _ in range(self.piecewise_points + 1)] 
            for (i, j) in edge_list 
        }
        res['piecewise_costs'] = piecewise_costs
    ...
    def solve(self, instance):
        ...
        # Introducing piecewise linear cost variables
        z_vars = {(i, j, s): model.addVar(vtype="C", name=f"z_{i+1}_{j+1}_{s+1}") for (i, j) in edge_list for s in range(self.piecewise_points)}
        ...
        # Adding piecewise cost to the objective function 
        objective_expr += quicksum(
            piecewise_costs[i, j][s] * z_vars[(i, j, s)]
            for (i, j) in edge_list for s in range(self.piecewise_points)
        )
        ...
        # Facility Opening based on Big M Method
        for i in range(self.n_nodes):
            flow_threshold = flow_thresholds[i]
            total_flow = quicksum(x_vars[f"x_{i+1}_{j+1}_{k+1}"] for j in outcommings[i] for k in range(self.n_commodities)) + \
                         quicksum(x_vars[f"x_{j+1}_{i+1}_{k+1}"] for j in incommings[i] for k in range(self.n_commodities))
            big_M = sum(commodities[k][2] for k in range(self.n_commodities))
            model.addCons(total_flow >= flow_threshold - (1 - open_vars[i]) * big_M, f"flow_threshold_{i+1}")

        # Piecewise linear constraints 
        for (i, j) in edge_list:
            total_flow = quicksum(x_vars[f"x_{i+1}_{j+1}_{k+1}"] for k in range(self.n_commodities))
            for s in range(self.piecewise_points):
                upper_segment = 1.0 / self.piecewise_points
                model.addCons(z_vars[(i, j, s)] <= upper_segment * total_flow, f"piecewise_{i+1}_{j+1}_{s+1}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'k_max': 1500,
        'er_prob': 0.73,
        'min_facility_cost': 100,
        'max_facility_cost': 250,
        'min_flow_thr': 225,
        'max_flow_thr': 375,
        'min_piecewise_points': 2,
        'max_piecewise_points': 70,
    }
