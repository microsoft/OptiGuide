...
class ComplexSupplyChainAndEVChargingOptimization:
    ...
    def generate_instances(self):
        ...
        weights = np.random.normal(self.weight_mean, self.weight_std, self.extra_items).astype(int)
        profits = weights + np.random.normal(self.profit_mean_shift, self.profit_std, self.extra_items).astype(int)
    ...
    def solve(self, instance):
        ...
        additional_weights = instance['additional_weights']
        additional_profits = instance['additional_profits']
        ...
        z_vars = {i: model.addVar(vtype="B", name=f"z_{i}") for i in range(len(new_item_weights))}
        ...
        ### INSERT ADDITIONAL OBJECTIVE ELEMENTS HERE 
        objective_expr += quicksum(additional_profits[i] * new_knapsack_vars[(i, j)] for i in range(len(new_item_weights)) for j in range(len(knapsack_extra_capacities)))
        ...
        for i in range(len(new_item_weights)):
            for j in range(len(knapsack_extra_capacities)):
                model.addCons(new_knapsack_vars[(i, j)] <= z_vars[i], f"BigM_constraint_1_{i}_{j}")  # If x[i][j] is 1, z[i] must be 1
                model.addCons(new_knapsack_vars[(i, j)] >= z_vars[i] - (1 - new_knapsack_vars[(i, j)]), f"BigM_constraint_2_{i}_{j}")  # If z[i] is 1, at least one x[i][j] must be 1
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'station_capacity_interval': (87, 350),
        'renewable_capacity_scale': 225.0,
        'fixed_cost_interval': (1, 5),
        'transport_cost_scale': 40.5,
        'number_of_items': 2500,
        'number_of_knapsacks': 1,
        'extra_items': 75,
        'extra_knapsacks': 70,
        'additional_renewables': 100,
        'weight_mean': 7,
        'weight_std': 125,
