...
class MixedIntegerOptimization:
    ...
    def generate_instance(self):
        ...
        # Adding knapsack data
        knapsack_capacities = np.random.randint(self.knapsack_capacity_min, self.knapsack_capacity_max, self.n_knapsacks)
        knapsack_weights = np.random.randint(self.knapsack_weight_min, self.knapsack_weight_max, (self.n_knapsacks, self.n_items))
        item_values = np.random.randint(self.item_value_min, self.item_value_max, self.n_items)

        res.update({
            'knapsack_capacities': knapsack_capacities,
            'knapsack_weights': knapsack_weights,
    ...
    def solve(self, instance):
        ...
        knapsack_weights = instance['knapsack_weights']
        item_values = instance['item_values']
        ...
        knapsack_vars = {(n, i): model.addVar(vtype="B", name=f"knapsack_{n}_{i}") for n in range(self.n_knapsacks) for i in range(self.n_items)}
        ...
        # New objective component for the knapsack problem (maximizing item values)
        objective_expr += quicksum(
            item_values[i] * knapsack_vars[(n, i)]
            for n in range(self.n_knapsacks) for i in range(self.n_items)
        )
        ...
        for n in range(self.n_knapsacks):
            model.addCons(quicksum(knapsack_weights[n, i] * knapsack_vars[(n, i)] for i in range(self.n_items)) <= knapsack_capacities[n], f"Knapsack_{n+1}_Capacity")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'n_knapsacks': 10,
        'n_items': 50,
        'knapsack_capacity_min': 100,
        'knapsack_capacity_max': 1000,
        'knapsack_weight_min': 1,
        'knapsack_weight_max': 100,
        'item_value_min': 10,
        'item_value_max': 100,
    }
