...
class EggDistributionOptimization:
    ...
    def solve(self, instance):
        ...
        # Symmetry Breaking Constraints: Ensure hubs are activated in increasing index order.
        for p in range(num_farms - 1):
            model.addCons(
                hub_activation_vars[p] >= hub_activation_vars[p + 1], 
                f"SymmetryBreaking_Hub_{p}_{p+1}"
            )
