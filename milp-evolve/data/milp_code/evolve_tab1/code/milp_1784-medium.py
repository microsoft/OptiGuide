...
class FCMCNF:
    ...
    def generate_instance(self):
        ...
        # Generate variations in fuel efficiency and emission rates
        fuel_efficiency_variations = np.random.uniform(self.min_fuel_efficiency, self.max_fuel_efficiency, self.n_facilities)
        carbon_emission_rates = np.random.uniform(self.min_emission_rate, self.max_emission_rate, self.n_facilities)
        ...
        res = {
            ...
            'fuel_efficiency_variations': fuel_efficiency_variations,
            'carbon_emission_rates': carbon_emission_rates,
        }
        return res
    ...
    def solve(self, instance):
        ...
        fuel_efficiency_variations = instance['fuel_efficiency_variations']
        ...
        # Define carbon emission variables and their constraints
        carbon_emission_vars = {s: model.addVar(vtype="C", name=f"Emission_{s}") for s in range(self.n_nodes)}
        ...
        # Adding carbon emissions to the objective function
        objective_expr += quicksum(
            carbon_emission_rates[s] * carbon_emission_vars[s]
            for s in range(self.n_nodes)
        )
        ...
        for s in range(self.n_nodes):
            emission_expr = quicksum(x_vars[f"x_{s+1}_{j+1}_{k+1}"] * fuel_efficiency_variations[s] for j in outcommings[s] for k in range(self.n_commodities))
            model.addCons(carbon_emission_vars[s] == emission_expr, f'emission_{s}')
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'max_fuel_efficiency': 2.0,
        'min_emission_rate': 10,
        'max_emission_rate': 50,
    }
