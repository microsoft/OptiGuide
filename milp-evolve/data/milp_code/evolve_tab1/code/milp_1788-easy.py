...
class FCMCNF:
    ...
    def solve(self, instance):
        ...
        # Logical conditions for hospital operations
        for h in range(self.n_nodes):
            for z in range(self.n_zones):
                model.addCons(serve_vars[h, z] <= hospital_vars[h], f"Serve_Open_Constraint_{h}_{z}")
                model.addCons(distances[h, z] * hospital_vars[h] <= self.max_distance + (M := self.max_distance * 100) * (1 - serve_vars[h, z]), f"Distance_Constraint_{h}_{z}")       
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'max_facility_cost': 937,
        'min_supply_cost': 3000,
        'max_supply_cost': 5000,
        'min_transport_time': 0.73,
        'max_transport_time': 131.25,
        'n_hospitals': 1575,
        'n_zones': 250,
        'min_transport_cost': 562,
        'max_transport_cost': 2700,
        'min_hospital_cost': 600,
        'max_hospital_cost': 1875,
        'min_bed_capacity': 562,
        'max_bed_capacity': 3000,
        'min_equipment_limit': 1215,
        'max_equipment_limit': 1425,
        'max_distance': 1181,
    }
