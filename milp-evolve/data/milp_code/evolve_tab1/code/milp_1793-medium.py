...
class EnhancedFacilityLocation:
    ...
    def generate_instance(self):
        ...
        supply_chain_costs = self.randint(self.n_facilities, self.supply_chain_cost_interval)
    ...
    def solve(self, instance):
        ...
        supply_chain_costs = instance['supply_chain_costs']
        ...
        # New variable for unmet demand penalties and supply chain preferences
        unmet_demand_penalties = {i: model.addVar(vtype="C", name=f"UnmetDemand_{i}") for i in range(n_customers)}
        preference_weights = {i: np.random.uniform(0.5, 1.5) for i in range(n_customers)}  # Adding preferences

        # Objective: minimize total cost including penalties for unmet demand and supply chain costs
        fixed_costs_expr = quicksum(fixed_costs[j] * open_facilities[j] for j in range(n_facilities))
        transportation_costs_expr = quicksum(transportation_costs[i, j] * cover[i, j] * preference_weights[i] for i in range(n_customers) for j in range(n_facilities))
        supply_chain_costs_expr = quicksum(supply_chain_costs[j] * open_facilities[j] for j in range(n_facilities))
        unmet_demand_penalties_expr = quicksum(self.penalty_coefficient * unmet_demand_penalties[i] for i in range(n_customers))

        objective_expr = fixed_costs_expr + transportation_costs_expr + supply_chain_costs_expr + unmet_demand_penalties_expr
        ...
        # New constraint: Total budget limit
        total_cost_expr = fixed_costs_expr + transportation_costs_expr + supply_chain_costs_expr
        model.addCons(total_cost_expr <= self.total_budget, "TotalBudget")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'supply_chain_cost_interval': (500, 1000),
        'big_M': 10000,
    ...
    facility_location = EnhancedFacilityLocation(parameters, seed=seed)
