...
class ElectricalLoadBalancingOptimization:
    ...
    def generate_instance(self):
        ...
        # New data
        budget_limits = np.random.uniform(self.min_budget_limit, self.max_budget_limit, self.n_generators)
        distances = np.random.uniform(0, self.max_transport_distance, (self.n_generators, self.n_loads))
        inventory_holding_costs = np.random.normal(self.holding_cost_mean, self.holding_cost_sd, self.n_generators)
        inventory_levels = np.random.normal(self.inv_mean, self.inv_sd, self.n_generators)
    ...
    def solve(self, instance):
        ...
        # New variables
        budget_vars = {g: model.addVar(vtype="C", name=f"Budget_{g}") for g in range(n_generators)}
        inventory_vars = {g: model.addVar(vtype="C", name=f"Inventory_{g}") for g in range(n_generators)}
        ...
        model.setObjective(
            ...
            quicksum(budget_vars[g] for g in range(n_generators)) +  # added budget costs
            quicksum(inventory_holding_costs[g] * inventory_vars[g] for g in range(n_generators)),  # added inventory holding costs
            "minimize"
        )
        ...
        # New constraints: Budgets cannot be exceeded
        for g in range(n_generators):
            model.addCons(budget_vars[g] <= budget_limits[g], f"Generator_{g}_BudgetConstraint")

        # New constraints: Inventory levels must be managed
        for g in range(n_generators):
            model.addCons(inventory_vars[g] >= inventory_levels[g], f"Generator_{g}_InventoryConstraint")
