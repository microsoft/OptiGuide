...
class VaccineDistributionOptimization:
    ...
    def generate_instance(self):
        ...
        return {
            ...
            "slopes": slopes,
            "n_vaccine_types": self.n_vaccine_types,
            "vaccine_demands": np.random.randint(1, 5, (self.n_vaccine_types, self.n_distribution_zones)),
            "production_capacities_vaccines": np.random.randint(self.min_production_capacity, self.max_production_capacity + 1, (self.n_production_zones, self.n_vaccine_types)),
            "transport_costs_vaccine": np.random.randint(self.min_transport_cost, self.max_transport_cost + 1, (self.n_production_zones, self.n_distribution_zones, self.n_vaccine_types))
        }
    ...
    def solve(self, instance):
        ...
        n_vaccine_types = instance['n_vaccine_types']
        vaccine_demands = instance['vaccine_demands']
        production_capacities_vaccines = instance['production_capacities_vaccines']
        transport_costs_vaccine = instance['transport_costs_vaccine']
        ...
        # Multi-commodity variables
        flow_vars = {(p, d, k): model.addVar(vtype="C", name=f"Flow_{p}_{d}_{k}") for p in range(n_production_zones) for d in range(n_distribution_zones) for k in range(n_vaccine_types)}
        ...
        # Multi-commodity constraints
        for k in range(n_vaccine_types):
            for d in range(n_distribution_zones):
                model.addCons(
                    quicksum(flow_vars[(p, d, k)] for p in range(n_production_zones)) == vaccine_demands[k, d],
                    f"Vaccine_{k}_Demand_{d}"
                )

            for p in range(n_production_zones):
                model.addCons(
                    quicksum(flow_vars[(p, d, k)] for d in range(n_distribution_zones)) <= production_capacities_vaccines[p, k],
                    f"Production_vaccine_{k}_Capacity_{p}"
                )

        for p in range(n_production_zones):
            for d in range(n_distribution_zones):
                model.addCons(
                    quicksum(flow_vars[(p, d, k)] for k in range(n_vaccine_types)) <= shipment_capacities[p, d],
                    f"Flow_Capacity_{p}_{d}"
                )
