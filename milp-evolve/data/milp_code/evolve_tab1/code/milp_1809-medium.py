...
class EmergencyResourceAllocation:
    ...
    def generate_instance(self):
        ...
        # Facility demand, capacities, and cost intervals
        customer_demand_interval = (1, 100)  # assuming 100 customers for illustration
        facility_capacity_interval = (100, 400)
        fixed_facility_cost = 2000
        facility_ratio = np.random.uniform(2, 5)
        ...
        res = {
            ...
            'customer_demand_interval': customer_demand_interval,
            'facility_capacity_interval': facility_capacity_interval,
            'fixed_facility_cost': fixed_facility_cost,
    ...
    def solve(self, instance):
        ...
        customer_demand_interval = instance['customer_demand_interval']
        facility_capacity_interval = instance['facility_capacity_interval']
        fixed_facility_cost = instance['fixed_facility_cost']
        facility_ratio = instance['facility_ratio']
        ...
        # Ensure 80% of each demand is met within a service cost limit
        penalty_cost = 250  # Arbitrary high penalty for unmet demand
        unmet_demand = {node: model.addVar(vtype="C", name=f"UnmetDemand_{node}") for node in graph.nodes}
        
        service_level = 0.8
        service_cost_limit = 3000
        for zone in graph.nodes:
            model.addCons(quicksum(evacuation_costs[zone, center] * rescue_vars[zone, center] for center in graph.nodes) <= service_cost_limit + (1 - service_level), f"ServiceLevel_{zone}")
            model.addCons(unmet_demand[zone] * penalty_cost <= service_cost_limit, f"UnmetPenalty_{zone}")
        ...
        objective = total_cost + \
                    ...
                    quicksum(unmet_demand[node] * penalty_cost for node in graph.nodes)
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'BigM': 1000000.0,
        'customer_demand_interval': (1, 100),
        'facility_capacity_interval': (100, 400),
        'fixed_facility_cost': 2000,
        'facility_ratio': 6.0,
    }
