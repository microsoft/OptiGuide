...
class DisasterResponseVRPEP:
    ...
    def solve(self, instance):
        ...
        for i in range(number_of_vehicles):
            model.addCons(
                quicksum(G.nodes[node]['demand'] * vehicle_vars[f"x_vehicle_{i}_{node}"] for node in G.nodes) <= load_vars[f"load_{i}"],
                f"VehicleLoad_{i}"
            )
        ...
        for i in range(number_of_vehicles):
            model.addCons(
                load_vars[f"load_{i}"] >= min_operational_load * quicksum(vehicle_vars[f"x_vehicle_{i}_{node}"] for node in G.nodes),
