...
class EventWorkerMarketOptimization:
    ...
    def generate_instance(self):
        ...
        # Add new customer market data
        num_customers = self.num_customers
        min_management_cost = self.min_management_cost
        max_management_cost = self.max_management_cost
        min_customer_cost = self.min_customer_cost
        max_customer_cost = self.max_customer_cost
        min_market_coverage = self.min_market_coverage
        max_market_coverage = self.max_market_coverage
        budget_constraint = self.budget_constraint

        management_costs = np.random.randint(min_management_cost, max_management_cost + 1, self.n_locations)
        customer_costs = np.random.randint(min_customer_cost, max_customer_cost + 1, (self.n_locations, num_customers))
        coverages = np.random.randint(min_market_coverage, max_market_coverage + 1, self.n_locations)
    ...
    def solve(self, instance):
        ...
        budget_constraint = instance['budget_constraint']

        model = Model("EventWorkerMarketOptimization")
        ...
        market_open = {m: model.addVar(vtype="B", name=f"MarketOpen_{m}") for m in graph.nodes}
        customer_covered = {(m, r): model.addVar(vtype="B", name=f"Market_{m}_Customer_{r}") for m in graph.nodes for r in range(len(demands))}
        ...
        # Constraints: Only open markets can cover customers
        ...
        for m in graph.nodes:
            model.addCons(quicksum(customer_covered[m, r] * demands[r] for r in range(len(demands))) <= coverages[m], f"Market_{m}_Capacity")
        
        # Constraints: Total Market Coverage meets Total Demand
        total_demand = np.sum(demands)
        model.addCons(quicksum(coverages[m] * market_open[m] for m in graph.nodes) >= total_demand, "TotalDemandCover")

        # New Constraints: Set Covering constraints to ensure each customer is covered by at least one market
        for r in range(len(demands)):
            model.addCons(quicksum(customer_covered[m, r] for m in graph.nodes) >= 1, f"Customer_{r}_Coverage")
        ...
        total_cost_with_skills_and_temporary = total_cost + \
            ...
            quicksum(management_costs[m] * market_open[m] for m in graph.nodes) + \
            quicksum(customer_costs[m, r] * customer_covered[m, r] for m in graph.nodes for r in range(len(demands)))
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_market_coverage': 45,
        'max_market_coverage': 90,
        'budget_constraint': 25000,
    }

    event_worker_market_optimization = EventWorkerMarketOptimization(parameters, seed=seed)
    instance = event_worker_market_optimization.generate_instance()
    solve_status, solve_time = event_worker_market_optimization.solve(instance)
