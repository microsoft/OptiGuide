...
class EventWorkerAllocationOptimization:
    ...
    def generate_instance(self):
        ...
        event_priorities = np.random.randint(1, 4, size=self.n_locations)
        capacity_scaling_factors = np.random.rand(n_facilities) * 2
    ...
    def solve(self, instance):
        ...
        convex_hull_vars = {(i, j, t): model.addVar(vtype="B", name=f"ConvexHull_{i}_{j}_{t}") for i in graphs[0].nodes for j in graphs[0].nodes for t in time_periods}
        ...
        # Mutual exclusivity constraints are removed to reduce redundancy
        ...
        ## Columns and Constraints for Convex Hull Formulation
        for i in graphs[0].nodes:
            for j in graphs[0].nodes:
                for t in time_periods:
                    # Convex hull constraints (adding more granularity in worker allocation relationships)
                    model.addCons(worker_vars[i, j, t] == convex_hull_vars[i, j, t] * event_vars[j], name=f"ConvexHull_WorkerAllocation_{i}_{j}_{t}")
                    model.addCons(convex_hull_vars[i, j, t] >= 0, name=f"ConvexHull_LowerBound_{i}_{j}_{t}")
                    model.addCons(convex_hull_vars[i, j, t] <= 1, name=f"ConvexHull_UpperBound_{i}_{j}_{t}")
        ...
        total_cost_with_extras = total_cost + \
            ...
            quicksum(urgent_task_vars[node] * 1000 for node in graphs[0].nodes) + \
