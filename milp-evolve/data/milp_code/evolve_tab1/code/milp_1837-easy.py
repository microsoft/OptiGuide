...
class EventWorkerAllocationOptimization:
    ...
    def generate_instance(self):
        ...
        # New shift parameters
        shifts = 3
        shift_capacity = np.random.randint(5, 10, size=shifts * self.n_locations).reshape((self.n_locations, shifts))

        # Multi-phase budget
        budget_periods = 3
        period_budgets = np.random.randint(3000, 5000, size=budget_periods).tolist()
    ...
    def solve(self, instance):
        ...
        budget_periods = instance['budget_periods']
        period_budgets = instance['period_budgets']
        ...
        # New shift variables
        shift_vars = {(node, shift): model.addVar(vtype="B", name=f"Shift_{node}_{shift}") for node in graph.nodes for shift in range(shifts)}
        period_spent_vars = {t: model.addVar(vtype="C", name=f"PeriodSpent_{t}") for t in range(budget_periods)}
        ...
        # New budget constraints on multiple periods
        for t in range(budget_periods):
            model.addCons(period_spent_vars[t] <= period_budgets[t], name=f"PeriodBudget_{t}")

        model.addCons(total_cost <= quicksum(period_spent_vars[t] for t in range(budget_periods)), name="TotalBudget")
        ...
        # New constraints for shift allocations
        for node in graph.nodes:
            model.addCons(quicksum(shift_vars[node, shift] for shift in range(shifts)) <= event_vars[node], name=f"ShiftEvent_{node}")
            for shift in range(shifts):
                model.addCons(shift_vars[node, shift] <= shift_capacity[node, shift], name=f"ShiftCapacity_{node}_{shift}")
