...
class Graph:
    ...
    def __init__(self, number_of_nodes, edges, degrees, neighbors, cliques):
    ...
    def erdos_renyi(number_of_nodes, edge_probability):
        ...
        neighbors = {node: set(G[node]) for node in G.nodes}
        return Graph(number_of_nodes, edges, degrees, neighbors, cliques)
...
class EmergencyResourceAllocation:
    ...
    def generate_instance(self):
        ...
        transport_availability = np.random.uniform(0.5, 1.5, size=len(graph.edges))
        infrastructure_levels = np.random.uniform(0.5, 1.5, size=(len(graph.edges), 2))
        slope_data = np.random.uniform(1, 10, size=(len(graph.edges), self.piece_count))
        ...
        for idx, edge in enumerate(graph.edges):
            break_points = np.linspace(0, np.max(slope_data[idx]), self.piece_count + 1)
            slopes = np.diff(break_points)
            res[f'break_points_{edge[0]}_{edge[1]}'] = break_points
            res[f'slopes_{edge[0]}_{edge[1]}'] = slopes
    ...
    def solve(self, instance):
        ...
        transport_availability = instance['transport_availability']
        slope_data = instance['slope_data']

        model = Model("EmergencyResourceAllocation")

        # Add variables
        center_vars = {node: model.addVar(vtype="B", name=f"Center_{node}") for node in graph.nodes}
        rescue_vars = {(i, j): model.addVar(vtype="B", name=f"Rescue_{i}_{j}") for i in graph.nodes for j in graph.nodes}
        ...
        service_quality_flags = {node: model.addVar(vtype="B", name=f"ServiceQuality_{node}") for node in graph.nodes}
        piece_vars = {(u, v, k): model.addVar(vtype="C", lb=0, name=f"Piece_{u}_{v}_{k}") for u, v in graph.edges for k in range(self.piece_count)}
        ...
        for node in graph.nodes:
            model.addCons(efficiency_vars[node] >= service_quality_flags[node] * service_quality_threshold, name=f"ServiceQuality_{node}")
            model.addCons(efficiency_vars[node] <= service_quality_flags[node] * BigM, name=f"ServiceQualityBigM_{node}")
            model.addCons(emergency_vars[node] + service_quality_flags[node] * service_quality_penalties[node] <= max_rescue_times[node], name=f"ServiceQualityPenalty_{node}")

        # New piecewise linear cost constraints
        for edge in graph.edges:
            u, v = edge
            model.addCons(quicksum(piece_vars[(u, v, k)] for k in range(self.piece_count)) == rescue_vars[u, v])
            for k in range(self.piece_count):
                model.addCons(
                    piece_vars[(u, v, k)] <= instance[f'break_points_{u}_{v}'][k+1] - instance[f'break_points_{u}_{v}'][k] + (1 - rescue_vars[u, v]) * BigM,
                    name=f"Piece_{u}_{v}_{k}"
                )

        # Clique Inequality Constraints for more efficient selection of centers and resource management
        for clique in graph.cliques:
            if len(clique) > 1:
                model.addCons(quicksum(center_vars[node] for node in clique) <= min(len(clique), max_centers))

        # Objective: Minimize total cost including efficiency, emergency penalties, and transport costs
        objective = total_cost + \
                    ...
                    quicksum((instance[f'slopes_{u}_{v}'][k] * piece_vars[(u, v, k)] for u, v in graph.edges for k in range(self.piece_count)))
