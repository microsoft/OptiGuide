...
class EmergencyResourceAllocation:
    ...
    def generate_instance(self):
        ...
        travel_times = np.random.randint(1, 6, size=(graph.number_of_nodes, graph.number_of_nodes))
        travel_costs = np.random.rand(graph.number_of_nodes, graph.number_of_nodes) * 10
    ...
    def solve(self, instance):
        ...
        transport_vars = {(i, j): model.addVar(vtype="C", lb=0, name=f"Transport_{i}_{j}") for i in graph.nodes for j in graph.nodes}
        ...
        for j in graph.nodes:
            model.addCons(quicksum(transport_vars[i, j] * victim_demands[i] for i in graph.nodes) <= center_capacities[j], name=f"Capacity_{j}")

        # Travel time constraints
        for i in graph.nodes:
            model.addCons(quicksum(travel_times[i][j] * rescue_vars[i, j] for j in graph.nodes) <= self.max_travel_time, name=f"TravelTime_{i}")

        # Budget constraints
        total_cost = quicksum(center_vars[node] * evacuation_center_costs[node] for node in graph.nodes) + \
                     quicksum(transport_vars[i, j] * travel_costs[i, j] for i in graph.nodes for j in graph.nodes) + \
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'rewiring_prob': 0.59,
        'max_capacity': 1312,
        'max_travel_time': 15,  # New parameter for travel time constraints
    }
