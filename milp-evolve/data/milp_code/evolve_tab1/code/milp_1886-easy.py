...
class Graph:
    ...
    def multi_layer_graph(number_of_nodes, edge_probability, layers):
        """Generate multiple layers of Erdös-Rényi graphs."""
        layer_graphs = []
...
class EventWorkerAllocationOptimization:
    ...
    def generate_instance(self):
        ...
        event_sizes = np.random.choice([1, 2, 3], size=self.n_locations, p=[0.3, 0.5, 0.2])
        ...
        overtime_rate = 1.75  # Adjusted rate for more variation
        max_overtime_hours = 50  # Increased for complexity

        max_budget = np.random.randint(5000, 20000)  # Increased upper limit for complexity
        max_work_hours = np.random.randint(100, 350, size=self.n_locations)
        min_event_workers = 5  # Increased minimum for complexity
        max_event_workers = 20  # Increased maximum for complexity

        # Facility parameters
        n_facilities = np.random.randint(3, 12)  # Increased range for complexity
        facility_capacity = np.random.randint(10, 60, size=n_facilities).tolist()
        facility_graph = nx.watts_strogatz_graph(n_facilities, 3, 0.4)  # Different facility graph for variation
        graph_edges = list(facility_graph.edges)
        
        # Added Time Periods
        time_periods = np.arange(1, 6)  # Increased number of periods for complexity
        worker_skill_matrix = np.random.randint(1, 15, size=(self.n_locations, len(time_periods)))

        # Additional data for convex hull formulation:
        training_costs = np.random.randint(150, 400, size=self.n_locations)
        temporary_facility_costs = np.random.randint(200, 500, size=n_facilities)
        temporary_facility_capacity = np.random.randint(5, 30, size=n_facilities).tolist()

        # New stochastic variables
        urgent_task_probabilities = np.random.rand(self.n_locations)  # probabilities for urgent tasks at locations

        # Introducing new components: Event priority and capacity scaling factors
        event_priorities = np.random.randint(1, 5, size=self.n_locations)  # More variation in priorities
        capacity_scaling_factors = np.random.rand(n_facilities) * 2.5  # Adjusted for complexity
        adaptive_demands = np.random.rand(self.n_locations) * 50  # Adaptive demands reflecting changing priorities
        ...
        res = {
            ...
            'urgent_task_probabilities': urgent_task_probabilities,
            'event_priorities': event_priorities,
            'capacity_scaling_factors': capacity_scaling_factors,
            'adaptive_demands': adaptive_demands
        }
        return res
    ...
    def solve(self, instance):
        ...
        urgent_task_probabilities = instance['urgent_task_probabilities']
        event_priorities = instance['event_priorities']
        capacity_scaling_factors = instance['capacity_scaling_factors']
        adaptive_demands = instance['adaptive_demands']
        ...
        urgent_task_vars = {node: model.addVar(vtype="B", name=f"UrgentTask_{node}") for node in graphs[0].nodes}
        adaptive_demand_vars = {node: model.addVar(vtype="C", name=f"AdaptiveDemand_{node}") for node in graphs[0].nodes}
        ...
        # Adaptive demands based on event priorities
        for node in graphs[0].nodes:
            model.addCons(adaptive_demand_vars[node] == event_demands[node] + adaptive_demands[node] * event_priorities[node], name=f"AdaptiveDemand_{node}")
        ...
        for node in graphs[0].nodes:
            for t in time_periods:
                ...
                model.addCons(hours_vars[node, t] <= 30, name=f'WorkMax_{node}_{t}')  # Increased max limit for complexity

        ## Additional constraints with added complexity
        # Facility capacity constraints over multiple periods
        for j in range(n_facilities):
            scaled_capacity = facility_capacity[j] * capacity_scaling_factors[j]
            model.addCons(quicksum(facility_allocation_vars[i, j] for i in graphs[0].nodes) <= scaled_capacity, name=f"FacilityCapacity_{j}")
            model.addCons(quicksum(facility_allocation_vars[i, j] + temp_facility_vars[j] * temporary_facility_capacity[j] for i in graphs[0].nodes) <= scaled_capacity + temporary_facility_capacity[j], name=f"TotalFacilityCapacity_{j}")
        ...
        # Urgent task constraints
        for node in graphs[0].nodes:
            model.addCons(urgent_task_vars[node] <= event_vars[node], name=f'UrgentTaskAssignment_{node}')
            model.addCons(quicksum(urgent_task_vars[node] * urgent_task_probabilities[node] for node in graphs[0].nodes) <= 1, name=f'UrgentTaskProbability_{node}')

        ## Objectives: now multi-period and with training costs, event priorities, and capacity scaling factors
        total_cost_with_extras = total_cost + \
            ...
            quicksum(urgent_task_vars[node] * 1500 for node in graphs[0].nodes) + \
            quicksum(event_vars[node] * training_costs[node] * event_priorities[node] for node in graphs[0].nodes)

        model.setObjective(total_cost_with_extras, "minimize")
