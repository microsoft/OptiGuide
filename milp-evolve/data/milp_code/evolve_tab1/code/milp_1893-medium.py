...
class SimplifiedEnergySupplyChainOptimization:
    ...
    def solve(self, instance):
        ...
        model = Model("SimplifiedEnergySupplyChainOptimization")
        ...
        for d in range(num_centers):
            model.addCons(
                quicksum(transmission_vars[(f"plant_{p}", f"center_{d}")] for p in range(num_plants)) >= center_demands[d], 
                f"Center_{d}_DemandCoverage"
            )

        for p in range(num_plants):
            model.addCons(
                quicksum(transmission_vars[(f"plant_{p}", f"center_{d}")] for d in range(num_centers)) <= energy_capacities[p], 
...
if __name__ == '__main__':
    ...
    parameters = {
        'num_plants': 250,
        'num_centers': 50,
        'min_transmission_cost': 2400,
        'max_transmission_cost': 2400,
        'min_gen_cost': 1000,
        'max_gen_cost': 2400,
        'min_energy_capacity': 2000,
        'max_energy_capacity': 5000,
        'max_transmission_distance': 1600,
        'min_exp_cost': 1500,
        'max_exp_cost': 7000,
    ...
    optimizer = SimplifiedEnergySupplyChainOptimization(parameters, seed=seed)
