...
class SimplifiedGISP_NDP:
    ...
    def generate_static_costs(self, G):
        for u, v in G.edges:
            ...
            G[u][v]['network_strength'] = np.random.uniform(0.5, 2.0)
    ...
    def solve(self, instance):
        ...
        for u, v in G.edges:
            # Original constraint: Conflict-free node selection
            model.addCons(
            ...
            # Logical Condition 1: If an edge is selected, at least one of its nodes must be selected
            model.addCons(
                edge_vars[f"y{u}_{v}"] <= node_vars[f"x{u}"] + node_vars[f"x{v}"],
                name=f"Logical_Cond1_{u}_{v}"
            )

            # Logical Condition 2: Edges with high network strength are only selected if below a cost threshold
            model.addCons(
                edge_vars[f"y{u}_{v}"] * G[u][v]['static_cost'] <= self.network_strength_threshold * (G[u][v]['network_strength'] <= 1.5),
                name=f"Logical_Cond2_{u}_{v}"
            )
...
if __name__ == '__main__':
    ...
    ### new parameter starts here
    parameters.update({'network_strength_threshold': 10}) 
    ### new parameter ends here 
