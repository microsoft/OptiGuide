...
class EventWorkerAllocationOptimization:
    ...
    def generate_instance(self):
        ...
        res = {
            ...
            'energy_per_allocation': np.random.randint(100, 200)
        }
        return res
    ...
    def solve(self, instance):
        ...
        energy_per_allocation = instance['energy_per_allocation']
        ...
        total_cost = quicksum(event_vars[node] * hiring_costs[node] for node in graph.nodes) + \
                     ...
                     quicksum(energy_cost * energy_vars[node] for node in graph.nodes)
        ...
        for node in graph.nodes:
            model.addCons(energy_vars[node] == quicksum(worker_vars[node, worker] * energy_per_allocation for worker in graph.nodes), name=f"EnergyAlloc_{node}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'max_overtime_hours': 2000,
        'max_budget': 8000,
        'min_event_workers': 36,
        'max_event_workers': 180,
    }
    parameters.update({
        'energy_per_allocation': 150,
        'energy_cost': 0.17,
    })
