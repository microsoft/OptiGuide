...
class ComplexSetCoverFacilityLocation:
    ...
    def generate_instance(self):
        ...
        # New data for logical conditions
        operation_status = np.random.choice([0, 1], size=self.n_facilities, p=[0.2, 0.8]) # randomly decide if a facility is initially operational
        energy_penalty_costs = np.random.randint(100, 500, size=self.n_facilities) # penalty for using more energy if not operational
    ...
    def solve(self, instance):
        ...
        for f in range(self.n_facilities):
            ...
            operation_vars[f] = model.addVar(vtype="B", name=f"Operation_{f}", obj=operation_status[f])
            energy_usage_decision_vars[f] = model.addVar(vtype="B", name=f"Energy_Usage_Decision_{f}")
        ...
        for f in range(self.n_facilities):
            model.addCons(energy_usage_decision_vars[f] <= facility_vars[f], f"Logical_Energy_Usage_Facility_{f}")
            model.addCons(energy_usage_decision_vars[f] <= operation_vars[f], f"Facility_Operation_Status_{f}")
            model.addCons(energy_usage_decision_vars[f] >= facility_vars[f] + operation_vars[f] - 1, f"Energy_Usage_Facility_Operation_{f}")
        ...
        objective_expr = quicksum(var_names[j] * c[j] for j in range(self.n_cols)) + \
                         ...
                         quicksum(energy_penalty_costs[f] * energy_usage_decision_vars[f] for f in range(self.n_facilities))
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'max_coef': 5,
        'n_crucial': 24,
        'activation_cost_low': 378,
        'activation_cost_high': 1125,
        'failure_probability_low': 0.17,
        'failure_probability_high': 0.66,
        'penalty_cost_low': 525,
        'penalty_cost_high': 560,
        'n_facilities': 7,
        'min_fixed_cost': 6,
        'max_fixed_cost': 592,
        ...
        'max_capacity': 379,
        'n_time_slots': 240,
        'initial_operational_probability': 0.59,
        'energy_penalty_cost_low': 100,
        'energy_penalty_cost_high': 2500,
    }
