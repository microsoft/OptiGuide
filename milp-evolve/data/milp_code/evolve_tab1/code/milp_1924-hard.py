...
class EmergencyResourceAllocation:
    ...
    def generate_instance(self):
        ...
        # Additional data for raw materials and waste
        raw_material_requirements = np.random.randint(1, 10, size=(graph.number_of_nodes, graph.number_of_nodes))
        max_raw_materials = np.random.randint(500, 1000)
        raw_material_costs = np.random.rand(graph.number_of_nodes, graph.number_of_nodes) * 10
        waste_penalty = np.random.rand(graph.number_of_nodes, graph.number_of_nodes)
        ...
        res = {
            ...
            'max_raw_materials': max_raw_materials,
    ...
    def solve(self, instance):
        ...
        raw_material_requirements = instance['raw_material_requirements']
        max_raw_materials = instance['max_raw_materials']
        raw_material_costs = instance['raw_material_costs']
        waste_penalty = instance['waste_penalty']
        ...
        raw_materials_used = {(i, j): model.addVar(vtype="C", name=f"RawMaterialUsage_{i}_{j}") for i in graph.nodes for j in graph.nodes}
        waste_vars = {zone: model.addVar(vtype="C", name=f"Waste_{zone}") for zone in graph.nodes}
        ...
        model.addCons(quicksum(raw_materials_used[i, j] for i in graph.nodes for j in graph.nodes) <= max_raw_materials, name="RawMaterialLimit")

        for i in graph.nodes:
            for j in graph.nodes:
                model.addCons(raw_materials_used[i, j] >= rescue_vars[i, j] * raw_material_requirements[i, j], name=f"MaterialRequirement_{i}_{j}")

        # Waste constraint
        for i in graph.nodes:
            model.addCons(waste_vars[i] >= raw_materials_used[i, j] - center_capacities[j], name=f"WasteConstraint_{i}")

        # Extended objective to minimize cost and raw material usage and waste
        objective = total_cost + quicksum(raw_material_costs[i, j] * raw_materials_used[i, j] for i in graph.nodes for j in graph.nodes) + \
                    quicksum(waste_penalty[i, j] * waste_vars[i] for i in graph.nodes for j in graph.nodes)
