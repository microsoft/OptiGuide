...
class EV_Delivery:
    ...
    def generate_instance(self):
        ...
        environmental_impact = np.random.randint(500, 2000, size=graph.number_of_nodes)
        depot_limits = np.random.randint(10000, 50000, size=graph.number_of_nodes)
    ...
    def solve(self, instance):
        ...
        depot_limits = instance['depot_limits']
        ...
        total_impact = quicksum(depot_vars[node] * environmental_impact[node] for node in graph.nodes)
        model.addCons(total_impact <= self.max_environmental_impact, name="MaxImpact")

        # Ensure depot capacity does not exceed limits
        for node in graph.nodes:
            model.addCons(quicksum(route_vars[i, node] for i in graph.nodes) <= depot_limits[node], name=f"DepotLimit_{node}")

        # Logical conditions: depots 1 and 2 cannot both be open simultaneously
        model.addCons(depot_vars[1] + depot_vars[2] <= 1, name="DepotExclusive_1_2")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'k': 0,
        'rewiring_prob': 0.24,
        'max_environmental_impact': 12000,
    }
