...
class LogisticsNetworkOptimization:
    ...
    def generate_instance(self):
        ...
        # Minimum operation levels for semi-continuous variables
        min_operation_levels = np.random.randint(20, 100, size=self.n_locations)

        # Define number of scenarios
        n_scenarios = 10

        # Stochastic data for scenarios
        scenario_delivery_demands = [delivery_demands + np.random.randint(-100, 100, size=self.n_locations) for _ in range(n_scenarios)]
        scenario_operation_costs = [operation_costs + np.random.randint(-20, 20, size=self.n_locations) for _ in range(n_scenarios)]
        scenario_fuel_costs = [fuel_costs + np.random.randint(-10, 10, size=(self.n_locations, self.n_locations)) for _ in range(n_scenarios)]
        ...
        res = {
            ...
            'M': M,
            'min_operation_levels': min_operation_levels,
            'n_scenarios': n_scenarios,
            'scenario_delivery_demands': scenario_delivery_demands,
            'scenario_operation_costs': scenario_operation_costs,
            'scenario_fuel_costs': scenario_fuel_costs,
        }
    ...
    def solve(self, instance):
        ...
        M = instance['M']
        min_operation_levels = instance['min_operation_levels']
        n_scenarios = instance['n_scenarios']
        scenario_delivery_demands = instance['scenario_delivery_demands']
        scenario_operation_costs = instance['scenario_operation_costs']
        scenario_fuel_costs = instance['scenario_fuel_costs']
        ...
        semi_continuous_vars = {node: model.addVar(vtype="C", name=f"SemiCont_{node}") for node in graph.nodes}
        ...
        # New Semi-Continuous Constraints
        for node in graph.nodes:
            model.addCons(semi_continuous_vars[node] >= min_operation_levels[node] * route_vars[node], name=f"SemiContinuousLowerBound_{node}")
            model.addCons(semi_continuous_vars[node] >= 0, name=f"SemiContinuousNonNegative_{node}")
        ...
        for scenario in range(n_scenarios):
            scenario_total_cost = quicksum(route_vars[node] * scenario_operation_costs[scenario][node] for node in graph.nodes) + \
                                  quicksum(vehicle_vars[i, j] * scenario_fuel_costs[scenario][i, j] for i in graph.nodes for j in graph.nodes) + \
                                  quicksum(supply_vars[node] * maintenance_costs[node] for node in graph.nodes)
            model.addCons(scenario_total_cost <= max_budget, name=f"ScenarioBudget_{scenario}")

        model.setObjective(quicksum(total_cost / n_scenarios for _ in range(n_scenarios)), "minimize")
