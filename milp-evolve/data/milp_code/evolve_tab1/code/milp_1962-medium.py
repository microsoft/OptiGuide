...
class LogisticsNetworkOptimization:
    ...
    def generate_instance(self):
        ...
        # Shift scheduler data - combining data from second MILP
        res['max_delay'] = np.random.randint(12, 24)
        res['travel_costs'] = np.random.uniform(10.0, 60.0, size=(self.n_locations, n_hubs))
        res['hotel_costs'] = np.random.uniform(10.0, 50.0, size=(self.n_locations, n_hubs))
        res['delay_penalties'] = np.random.uniform(20, 100, size=self.n_locations)
        return res
    ...
    def solve(self, instance):
        ...
        # Shift scheduler variables
        delay_var = model.addVar(vtype="C", name="Total_Delay")
        shift_cost_vars = {(i, hub): model.addVar(vtype="C", name=f"ShiftCost_{i}_{hub}") for i in graph.nodes for hub in range(n_hubs)}
        ...
        ### Shift scheduler constraints ###
        for i in graph.nodes:
            for hub in range(n_hubs):
                model.addCons(shift_cost_vars[i, hub] <= travel_costs[i, hub], name=f"ShiftTravelCosts_{i}_{hub}")
                model.addCons(shift_cost_vars[i, hub] <= hotel_costs[i, hub], name=f"ShiftHotelCosts_{i}_{hub}")

        model.addCons(delay_var <= max_delay, name="MaxDelay")
        total_cost += quicksum(delay_var * delay_penalties[node] for node in graph.nodes)
