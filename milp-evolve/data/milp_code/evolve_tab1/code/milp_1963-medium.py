...
class ElectricCarDeployment:
    ...
    def solve(self, instance):
        ...
        for node in graph.nodes:
            model.addCons(energy_vars[node] == quicksum(storage_vars[node] * maintenance_schedules[node, t] * energy_prices[t] for t in range(self.n_time_slots)), name=f"Energy_Consumption_{node}")

        # Storage capacity constraints
        for node in graph.nodes:
            model.addCons(storage_vars[node] <= charging_capacities[node], name=f"Storage_Capacity_{node}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'edges_to_attach': 0,
        'max_capacity': 1263,
        'failure_probability_low': 0.59,
        'failure_probability_high': 0.38,
        'n_time_slots': 2160,
        'energy_penalty_cost_low': 350,
        'energy_penalty_cost_high': 312,
    }
