...
class EventPlanningAllocation:
    ...
    def generate_instance(self):
        ...
        equipment_levels = np.random.randint(self.min_equipment_level, self.max_equipment_level, self.num_venues)
        setup_times = np.random.randint(self.min_setup_time, self.max_setup_time, self.num_venues)
    ...
    def solve(self, instance):
        ...
        equipment_levels = instance['equipment_levels']
        ...
        for v in range(self.num_venues):
            ...
            equipment_service_vars[v] = model.addVar(vtype="C", name=f"equipment_service_{v}", lb=0, ub=equipment_levels[v], obj=setup_times[v])
            for j in range(self.num_zones):
        ...
        for v in range(self.num_venues):
            ...
            model.addCons(quicksum(allocation_vars[(v, j)] * transport_costs[v][j] for j in range(self.num_zones)) <= equipment_service_vars[v], f"Venue_{v}_Service")
            for j in range(self.num_zones):
        ...
        objective_expr = quicksum(zone_vars[j] * budget[j] for j in range(self.num_zones)) + \
                         ...
                         quicksum(setup_times[v] * equipment_service_vars[v] for v in range(self.num_venues))
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'density': 0.73,
        'max_budget': 7,
        'num_essential_zones': 220,
        'equipment_cost_low': 39,
        'equipment_cost_high': 750,
        'num_venues': 135,
        'min_venue_cost': 135,
        'max_venue_cost': 592,
