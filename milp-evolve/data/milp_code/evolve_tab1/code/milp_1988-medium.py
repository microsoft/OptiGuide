...
class FleetManagement:
    ...
    def solve(self, instance):
        ...
        satisfy_demand_vars = {node: model.addVar(vtype="B", name=f"SatisfyDemand_{node}") for node in graph.nodes}
        ...
        # Logical conditions to ensure demand satisfaction
        for zone in graph.nodes:
            model.addCons(
                satisfy_demand_vars[zone] <= quicksum(vehicle_vars[center] for center in graph.nodes),
                name=f"LogicalDemand_{zone}"
            )
            model.addCons(
                quicksum(routing_vars[zone, center] for center in graph.nodes) == satisfy_demand_vars[zone],
                name=f"RoutingCondition_{zone}"
            )
        ...
        total_cost = quicksum(vehicle_vars[node] * vehicle_costs[node] for node in graph.nodes) + \
                     ...
                     quicksum((1 - satisfy_demand_vars[node]) * unmet_penalties[node] for node in graph.nodes)
