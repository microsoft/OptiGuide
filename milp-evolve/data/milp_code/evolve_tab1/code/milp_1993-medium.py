...
class EmergencyDeployment:
    ...
    def generate_instance(self):
        ...
        # New instance data
        resource_min_usage = np.random.uniform(self.resource_min_lower, self.resource_min_upper, size=graph.number_of_nodes)
        resource_max_usage = np.random.uniform(self.resource_max_lower, self.resource_max_upper, size=graph.number_of_nodes)
    ...
    def solve(self, instance):
        ...
        # New semi-continuous variables for resource usage
        resource_vars = {node: model.addVar(lb=0, ub=resource_max_usage[node], vtype="C", name=f"Resource_{node}") for node in graph.nodes}
        ...
        for group_index, set_cover_group in enumerate(set_cover_groups):
            model.addCons(quicksum(emergency_unit_vars[node] for node in set_cover_group) >= 1, name=f"SetCover_{group_index}")

        # Set packing constraints
        for group_index, set_packing_group in enumerate(set_packing_groups):
            model.addCons(quicksum(emergency_unit_vars[node] for node in set_packing_group) <= 1, name=f"SetPacking_{group_index}")

        # New resource usage constraints
        for node in graph.nodes:
            model.addCons(resource_vars[node] >= resource_min_usage[node] * emergency_unit_vars[node], name=f"MinResource_{node}")
            model.addCons(resource_vars[node] <= resource_max_usage[node] * emergency_unit_vars[node], name=f"MaxResource_{node}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'rewiring_prob': 0.52,
        'max_capacity': 1405,
        'resource_min_lower': 30,
        'resource_min_upper': 350,
        'resource_max_lower': 1000,
        'resource_max_upper': 1800,
    }
