...
class FCMCNF:
    ...
    def generate_instance(self):
        ...
        # Generate additional data for worker allocation
        worker_populations = np.random.randint(1000, 10000, size=self.n_nodes)
        hiring_costs = np.random.randint(50, 150, size=self.n_nodes)
        logistics_costs = np.random.rand(self.n_nodes, self.n_nodes) * 50
        operation_costs = np.random.randint(100, 300, size=self.n_nodes)
        overtime_costs = np.random.randint(200, 500, size=self.n_nodes)
        overtime_rate = 1.5
    ...
    def solve(self, instance):
        ...
        w_vars = {f"w_{i+1}_{j+1}": model.addVar(vtype="I", name=f"w_{i+1}_{j+1}") for (i, j) in edge_list}
        ...
        objective_expr += quicksum(
            w_vars[f"w_{i+1}_{j+1}"] * (logistics_costs[i, j] + operation_costs[j])
            for (i, j) in edge_list
        )
        ...
        for (i, j) in edge_list:
            model.addCons(y_vars[f"y_{i+1}_{j+1}"] * worker_populations[i] >= w_vars[f"w_{i+1}_{j+1}"], f"worker_alloc_{i+1}_{j+1}")
        
        # Budget constraints on worker costs
        total_worker_cost = quicksum(
            w_vars[f"w_{i+1}_{j+1}"] * (hiring_costs[i] + overtime_costs[j] * overtime_rate)
            for (i, j) in edge_list
        )
        model.addCons(total_worker_cost <= max_budget, f"budget")
