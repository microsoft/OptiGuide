...
class FleetManagement:
    ...
    def generate_instance(self):
        ...
        # New instance data to reflect the knapsack constraints
        subset_knapsack_weights = [np.random.randint(1, 200, size=graph.number_of_nodes) for _ in range(graph.number_of_nodes)]
        subset_costs = [np.random.randint(100, 300, size=graph.number_of_nodes) for _ in range(graph.number_of_nodes)]
        res['subset_knapsack_weights'] = subset_knapsack_weights
        res['subset_costs'] = subset_costs
    ...
    def solve(self, instance):
        ...
        subset_knapsack_weights = instance['subset_knapsack_weights']
        subset_costs = instance['subset_costs']
        ...
        subset_vars = {(i, j): model.addVar(vtype="B", name=f"Subset_{i}_{j}") for i in range(graph.number_of_nodes) for j in range(graph.number_of_nodes)}
        ...
        # Subset constraints to enforce knapsack problem structure
        for j in range(graph.number_of_nodes):
            model.addCons(quicksum(subset_vars[j, k] * subset_knapsack_weights[j][k] for k in range(graph.number_of_nodes)) <= vehicle_capacity, name=f"KnapsackCapacity_{j}")

        # Ensure each delivery can be served only once by a subset
        for i in range(graph.number_of_nodes):
            model.addCons(quicksum(subset_vars[j, i] for j in range(graph.number_of_nodes)) <= 1, name=f"UniqueService_{i}")
        ...
        total_cost = quicksum(vehicle_vars[node] * vehicle_costs[node] for node in graph.nodes) + \
                     ...
                     quicksum(subset_vars[i, j] * subset_costs[i][j] for i in range(graph.number_of_nodes) for j in range(graph.number_of_nodes))
