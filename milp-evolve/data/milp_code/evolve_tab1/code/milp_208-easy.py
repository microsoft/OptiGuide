...
class MultipleKnapsack:
    ...
    def generate_instance(self):
        ...
        res['index_pairs'] = [(i, j) for i in range(len(weights)) for j in range(i + 1, len(weights)) if weights[i] == weights[j] and profits[i] == profits[j]]
    ...
    def solve(self, instance):
        ...
        index_pairs = instance['index_pairs']
        ...
        # New symmetry-breaking constraints: Creating y_j to denote total weight in knapsack j
        y = {}
        for j in range(number_of_knapsacks):
            y[j] = model.addVar(vtype="C", name=f"y_{j}")
            model.addCons(y[j] == quicksum(weights[i] * var_names[(i, j)] for i in range(number_of_items)))

        # Symmetry-breaking constraint: Knapsack ordering
        for j in range(number_of_knapsacks - 1):
            model.addCons(y[j] >= y[j + 1], f"KnapsackOrdering_{j}")

        # Symmetry-breaking constraint: Item ordering
        for i, k in index_pairs:
            model.addCons(
                quicksum(var_names[(i, j)] for j in range(number_of_knapsacks)) >= quicksum(var_names[(k, j)] for j in range(number_of_knapsacks)),
                f"ItemOrdering_{i}_{k}"
            )
