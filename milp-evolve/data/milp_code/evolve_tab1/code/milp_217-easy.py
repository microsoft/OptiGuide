...
class EnhancedSetCoverWithFlow:
    ...
    def generate_instance(self):
        ...
        # objective coefficients with added diversity
        c = np.random.randint(self.max_coef, size=self.n_cols) + 1
        penalties = np.random.normal(loc=self.penalty_mean, scale=self.penalty_std, size=self.n_cols)
    ...
    def solve(self, instance):
        ...
        # Augmented composite objective
        cost_term = quicksum(var_names[j] * c[j] for j in range(self.n_cols))
        flow_term = quicksum(flow_vars[(u, v)] for u, v in flow_vars)
        penalty_term = quicksum(var_names[j] * penalties[j] for j in range(self.n_cols))
        objective_expr = cost_term - self.flow_weight * flow_term + self.penalty_weight * penalty_term
