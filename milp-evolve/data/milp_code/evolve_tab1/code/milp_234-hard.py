...
class EVChargingStationOptimization:
    ...
    def solve(self, instance):
        ...
        # Station capacity and Big M constraints
        for j in range(n_stations):
            model.addCons(quicksum(flow[i, j] for i in range(n_customers)) <= station_capacities[j], f"StationCapacity_{j}")
            for i in range(n_customers):
                model.addCons(flow[i, j] <= M * open_stations[j], f"BigM_{i}_{j}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'renewable_capacity_scale': 1687.5,
        'fixed_cost_interval': (131, 525),
        'transport_cost_scale': 303.75,
    }
