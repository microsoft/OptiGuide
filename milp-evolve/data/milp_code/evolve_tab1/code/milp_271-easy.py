...
class EVChargingStationOptimization:
    ...
    def renewable_energy_supply(self):
        return np.random.normal(self.renewable_capacity_mean, self.renewable_capacity_std, self.n_renewables)

    def stochastic_demands(self):
        return np.random.normal(self.demand_mean, self.demand_std, self.n_customers)

    def generate_instance(self):
        demands = self.stochastic_demands()
        ...
        res = {
            ...
            'renewable_capacity_std': self.renewable_capacity_std,
        }
    ...
    def solve(self, instance):
        ...
        demand_std = instance['demand_std']
        renewable_capacity_std = instance['renewable_capacity_std']
        ...
        demand_deviation = {i: model.addVar(vtype="C", name=f"DemandDeviation_{i}") for i in range(n_customers)}
        renewable_deviation = {k: model.addVar(vtype="C", name=f"RenewableDeviation_{k}") for k in range(n_renewables)}
        ...
        for i in range(n_customers):
            model.addCons(quicksum(flow[i, j] for j in range(n_stations)) == demands[i] + demand_deviation[i], f"Demand_{i}")
        ...
        for k in range(n_renewables):
            model.addCons(renewable_supply[k] <= renewable_capacities[k] - renewable_deviation[k], f"RenewableCapacity_{k}")
        ...
        # Robust constraints
        confidence_level = 1.96  # corresponding to a 95% confidence level for robustness

        for i in range(n_customers):
            model.addCons(demand_deviation[i] <= confidence_level * demand_std, f"DemandDeviationConstraint_{i}")

        for k in range(n_renewables):
            model.addCons(renewable_deviation[k] <= confidence_level * renewable_capacity_std, f"RenewableDeviationConstraint_{k}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'renewable_capacity_mean': 750.0,
        'renewable_capacity_std': 100.0,
