...
class CombinatorialAuction:
    ...
    def solve(self, instance):
        ...
        # Additional complexity using Convex Hull approach by introducing logical AND through auxiliary variables
        for (bid1, bid2) in mutual_exclusivity_pairs:
            aux_var = model.addVar(vtype="B", name=f"Aux_Bid_{bid1}_{bid2}")
            model.addCons(aux_var <= bid_vars[bid1], f"Aux1_{bid1}_{bid2}")
            model.addCons(aux_var <= bid_vars[bid2], f"Aux2_{bid1}_{bid2}")
            model.addCons(aux_var >= bid_vars[bid1] + bid_vars[bid2] - 1, f"Aux3_{bid1}_{bid2}")
