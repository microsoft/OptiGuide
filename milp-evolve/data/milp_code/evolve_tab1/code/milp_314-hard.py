...
class NDP:
    ...
    def solve(self, instance):
        ...
        installation_costs = model.addVar(vtype="C", name="installation_costs")
        maintenance_costs = model.addVar(vtype="C", name="maintenance_costs")
        
        queue_lengths = {node: model.addVar(vtype="C", name=f"queue_length_{node}") for node in G.nodes}
        waiting_times = {node: model.addVar(vtype="C", name=f"waiting_time_{node}") for node in G.nodes}
        
        voltage = {node: model.addVar(vtype="C", name=f"voltage_{node}") for node in G.nodes}
        power_demand = {node: model.addVar(vtype="C", name=f"power_demand_{node}") for node in G.nodes}
        ...
        objective_expr -= quicksum(queue_lengths[node] for node in G.nodes)
        objective_expr -= quicksum(waiting_times[node] for node in G.nodes)
        ...
        model.addCons(
            installation_costs + maintenance_costs <= self.budget,
        ...
        for node in G.nodes:
            model.addCons(
                queue_lengths[node] <= self.max_queue_length,
                name=f"QueueLength_{node}"
            )
            model.addCons(
                waiting_times[node] <= self.max_waiting_time,
                name=f"WaitingTime_{node}"
            )

        # New Power constraint
        for node in G.nodes:
            model.addCons(
                power_demand[node] <= self.max_power_capacity,
                name=f"PowerCapacity_{node}"
            )
            model.addCons(
                voltage[node] >= self.min_voltage,
                name=f"Voltage_{node}"
            )
