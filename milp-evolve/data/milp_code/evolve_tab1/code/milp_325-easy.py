...
class EnhancedCapacitatedFacilityLocation:
    ...
    def solve(self, instance):
        ...
        model = Model("EnhancedCapacitatedFacilityLocation")
        ...
        z = {(i, j): model.addVar(vtype="B", name=f"Z_{i}_{j}") for i in range(n_customers) for j in range(n_facilities)}
        ...
        # Constraints: demand must be fully served by exactly one facility
        for i in range(n_customers):
            model.addCons(quicksum(z[i, j] for j in range(n_facilities)) == 1, f"Demand_{i}")

        # Capacity constraints for convex hull formulation
        for j in range(n_facilities):
            model.addCons(quicksum(serve[i, j] * demands[i] for i in range(n_customers)) <= capacities[j], f"Capacity_{j}")
            model.addCons(quicksum(demands[i] * z[i, j] for i in range(n_customers)) <= capacities[j] * open_facilities[j],  f"CapacityConvex_{j}")

        # Linking serve and fixed costs
        for i in range(n_customers):
            for j in range(n_facilities):
                model.addCons(serve[i, j] <= z[i, j], f"ServeLink_{i}_{j}")
                model.addCons(z[i, j] <= open_facilities[j], f"OpenLink_{i}_{j}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'fixed_cost_scale_interval': (10, 11),
        'fixed_cost_cste_interval': (0, 4),
        'ratio': 15.0,
        'continuous_assignment': 0,
    }
    
    facility_location = EnhancedCapacitatedFacilityLocation(parameters, seed=seed)
