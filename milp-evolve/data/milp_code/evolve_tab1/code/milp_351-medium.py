...
class HCAN:
    ...
    def get_instance(self):
        ...
        # Added new data parameters based on GDO
        ...
        bids = [(zone, np.random.uniform(50, 200)) for zone in zones]
        mutual_exclusivity_pairs = set()
        while len(mutual_exclusivity_pairs) < self.no_exclusive_pairs:
        ...
        return {
            ...
            'bids': bids,
            'mutual_exclusivity_pairs': list(mutual_exclusivity_pairs)
        }
    ...
    def solve(self, instance):
        ...
        # New variables for enhanced constraints
        ...
        objective_expr += quicksum(market_availability[node] * nurse_shift_vars[f"NurseShift{node}"] for node in G.nodes)
        objective_expr -= quicksum(zonal_coordination[(u, v)] * congestion_vars[f"cong_{u}_{v}"] for u, v in G.edges)
        objective_expr -= quicksum(congestion_index[(u, v)] * market_vars[f"m{u}_{v}"] for u, v in G.edges)
        ...
        # Congestion management cons
        for u, v in G.edges:
            model.addCons(market_vars[f"m{u}_{v}"] >= congestion_vars[f"cong_{u}_{v}"], name=f"Cong_Market_{u}_{v}")

        # Scheduling constraints for exclusive pairs
