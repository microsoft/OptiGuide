...
class EmergencyResponseOptimization:
    ...
    def solve(self, instance):
        ...
        big_M = 1000  # "Artificial big M" constant
        ...
        for u, v in G.edges:
            model.addCons(
                response_vars[(u, v)] * big_M <= traffic_probabilities[(u, v)] * big_M,
        ...
        # Added to limit total responses per node
        for node in G.nodes:
            model.addCons(
                quicksum(response_vars[(u, v)] for u, v in G.edges if u == node) <= big_M,
                name=f"MaxResponses_{node}"
            )
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'max_penalty': 3.75,
        'max_zone_size': 25,
        'min_emergency_level': 3,
        'max_emergency_level': 17,
    }
