...
class IndependentSet:
    ...
    def generate_instance(self):
        ...
        # New instance data
        res.update({
            'inventory_levels': np.random.randint(1, 10, size=self.n_nodes),
            'transport_modes': np.random.choice(['truck', 'drone', 'boat'], size=len(graph.edges)),
            'storage_capacity': np.random.randint(20, 100, size=self.n_nodes),
            'personnel_skill': np.random.randint(1, 5, size=self.n_nodes),
            'weather_impact': np.random.randint(0, 2, size=len(graph.edges)),
            'civil_unrest_impact': np.random.randint(0, 2, size=len(graph.edges)),
        })
    ...
    def solve(self, instance):
        ...
        for node in graph.nodes:
            model.addCons(var_names[node] <= inventory_levels[node], name=f"inventory_{node}")
            model.addCons(var_names[node] <= storage_capacity[node], name=f"storage_{node}")
            model.addCons(var_names[node] <= personnel_skill[node], name=f"personnel_{node}")

        for count, edge in enumerate(graph.edges):
            model.addCons(var_names[edge[0]] + var_names[edge[1]] <= 1 + weather_impact[count],
                           name=f"weather_impact_{count}")
            model.addCons(var_names[edge[0]] + var_names[edge[1]] <= 1 + civil_unrest_impact[count],
                           name=f"civil_unrest_impact_{count}")
