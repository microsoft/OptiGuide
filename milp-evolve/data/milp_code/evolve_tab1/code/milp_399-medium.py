...
class ShippingPortOptimization:
    ...
    def get_instance(self):
        ...
        for s in range(self.no_of_scenarios):
            cargo_scenarios[s]['cargo_weight'] = {node: np.random.normal(G.nodes[node]['cargo_weight'], G.nodes[node]['cargo_weight'] * self.cargo_variation)
                                                  for node in G.nodes}
            cargo_scenarios[s]['handling_time'] = {(u, v): np.random.normal(G[u][v]['handling_time'], G[u][v]['handling_time'] * self.time_variation)
                                                   for u, v in G.edges}
            cargo_scenarios[s]['trailer_count'] = {node: np.random.normal(trailer_count[node], trailer_count[node] * self.trailer_variation)
                                                   for node in G.nodes}
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'network_prob': 0.24,
        'exclusion_rate': 0.45,
        'zero_idle_threshold': 100,
        'no_of_scenarios': 42,
        'cargo_variation': 0.38,
        'time_variation': 0.52,
