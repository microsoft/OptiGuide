...
class FactoryOptimization:
    ...
    def generate_instance(self):
        ...
        utilization_rates = np.random.uniform(0.7, 1.0, size=self.n_machines)
        energy_coefficients = np.random.uniform(0.5, 2.0, size=self.n_tasks)
        waste_coefficients = np.random.uniform(0.2, 1.5, size=self.n_tasks)
    ...
    def solve(self, instance):
        ...
        for m in range(self.n_machines):
            ...
            model.addCons(utilization_vars[m] == quicksum(machine_vars[j] for j in cols) / self.n_machines, 
                          f"utilization_{m}")
            model.addCons(utilization_vars[m] <= utilization_rates[m], f"utilization_limit_{m}")

        # Quadratic constraints
        energy_constraints = quicksum(energy_coefficients[j] * machine_vars[j] ** 2 for j in range(self.n_tasks))
        waste_constraints = quicksum(waste_coefficients[j] * machine_vars[j] ** 2 for j in range(self.n_tasks))
