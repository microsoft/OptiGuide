...
class HCAN:
    ...
    def solve(self, instance):
        ...
        equipment_use_vars = {f"EquipmentUse{node}": model.addVar(vtype="B", name=f"EquipmentUse{node}") for node in G.nodes}
        patient_group_vars = {f"PatientGroup{node}": model.addVar(vtype="I", name=f"PatientGroup{node}", lb=1, ub=5) for node in G.nodes}
        ...
        objective_expr -= quicksum(equipment_use_vars[f"EquipmentUse{node}"] * 10 for node in G.nodes)
        ...
        for node in G.nodes:
            model.addCons(
                equipment_use_vars[f"EquipmentUse{node}"] <= nurse_shift_vars[f"NurseShift{node}"],
                name=f"EquipConstraint_{node}"
            )
            model.addCons(
                quicksum(visit_segment_vars[(u, v)][segment] for u, v in G.edges if u == node or v == node for segment in range(1, piecewise_segments + 1)) <= patient_group_vars[f"PatientGroup{node}"] * 10,
                name=f"PatientGroupConstraint_{node}"
            )
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'exclusion_rate': 0.52,
        'shift_hours': 2100,
        'piecewise_segments': 81,
        'no_of_scenarios': 420,
        'patient_variation': 0.38,
        'time_variation': 0.52,
        'capacity_variation': 0.38,
        'financial_param_1': 37,
        'financial_param_2': 175,
        'travel_cost_param_1': 9.0,
        'travel_cost_param_2': 10.0,
    }
