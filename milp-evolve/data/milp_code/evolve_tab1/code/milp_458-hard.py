...
class StreamlinedTeamDeployment:
    ...
    def generate_instance(self):
        ...
        activation_costs = np.random.randint(20, 150, size=n_locations)  # simplified activation costs
    ...
    def solve(self, instance):
        ...
        model = Model("StreamlinedTeamDeployment")
        ...
        # Constraints
        for i in range(n_teams):
            # Each team assigned exactly one location
            model.addCons(quicksum(x[i, j] for j in range(n_locations)) == 1, name=f"team_assignment_{i}")

        for j in range(n_locations):
            # The total demand must not exceed the capacity if activated
        ...
        # Adding new constraint replaced using Convex Hull Formulation
        for j in range(n_locations):
            for i in range(n_teams):
                model.addCons(x[i, j] <= z[j], name=f"x_implies_z_{i}_{j}")
...
if __name__ == '__main__':
    ...
    deployment = StreamlinedTeamDeployment(parameters, seed=seed)
