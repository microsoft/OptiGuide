...
class NetworkOptimization:
    ...
    def solve(self, instance):
        ...
        # Semi-continuous variables for capacities (can be zero or within a continuous range above threshold)
        semi_continuous_threshold = 50
        NetworkNodeCap_vars = {node: model.addVar(vtype="C", name=f"NetworkNodeCap{node}", lb=0) for node in G.nodes}
        NetworkRouteCap_vars = {f"{u}_{v}": model.addVar(vtype="C", name=f"NetworkRouteCap{u}_{v}", lb=0) for u, v in G.edges}
        ...
        objective_expr -= quicksum(
            operational_cost[(u, v)] * NetworkRouteCap_vars[f"{u}_{v}"]
            for u, v in G.edges
        )
        ...
        for node in G.nodes:
            model.addCons(
                NetworkNodeCap_vars[node] >= semi_continuous_threshold * NetworkNode_vars[f"NetworkNode{node}"],
                name=f"NetworkNodeCap_{node}"
            )
        
        for u, v in G.edges:
            model.addCons(
                NetworkRouteCap_vars[f"{u}_{v}"] >= semi_continuous_threshold * NetworkRoute_vars[f"NetworkRoute{u}_{v}"],
                name=f"NetworkRouteCap_{u}_{v}"
            )
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'BandwidthVariation': 0.45,
        'NetworkCapacityVariation': 0.73,
    }
    ### given parameter code ends here
    ### new parameter code ends here
    parameters['semi_continuous_threshold'] = 50
