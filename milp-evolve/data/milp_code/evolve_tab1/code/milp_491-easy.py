...
class VehicleAllocationMILP:
    ...
    def solve(self, instance):
        ...
        charge_binary_vars = {node: model.addVar(vtype="B", name=f"ChargeRenewable{node}") for node in G.nodes}
        renewable_util_vars = {node: model.addVar(vtype="C", lb=0.0, ub=1.0, name=f"RenewableUtil{node}") for node in G.nodes}
        ...
        for node in G.nodes:
            ...
            model.addCons(
                vehicle_vars[f"Vehicle{node}"] <= charge_binary_vars[node],
            ...
            model.addCons(
                quicksum(route_vars[f"Route{u}_{v}"] for u, v in G.out_edges(node)) <= charging_capacity[node],
        ...
        # Add piecewise linear constraints for charging costs
        piecewise_points = [0, 0.5, 1.0]  # Example points for piecewise linear function
        piecewise_values = [x**2 for x in piecewise_points]  # Quadratic relationship
        
        piecewise_vars = {}
        for node in G.nodes:
            piecewise_vars[node] = {}
            for i in range(len(piecewise_points) - 1):
                segment_name = f"ChargingCost_{node}_seg{i}"
                piecewise_vars[node][i] = model.addVar(vtype="C", name=segment_name)
                slope = (piecewise_values[i+1] - piecewise_values[i]) / (piecewise_points[i+1] - piecewise_points[i])
                intercept = piecewise_values[i] - slope * piecewise_points[i]
                model.addCons(
                    piecewise_vars[node][i] == slope * renewable_util_vars[node] + intercept,
                    name=f"{segment_name}_constraint"
                )

        objective_expr += quicksum(
            piecewise_vars[node][i] for node in G.nodes for i in range(len(piecewise_points) - 1)
        )
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'shift_hours': 2520,
        'no_of_scenarios': 60,
        'demand_variation': 0.59,
        'distance_variation': 0.24,
        'capacity_variation': 0.66,
    }
