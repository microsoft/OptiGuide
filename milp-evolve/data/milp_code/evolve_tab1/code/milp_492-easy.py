...
class VehicleAllocationMILP:
    ...
    def get_instance(self):
        ...
        for _ in range(self.n_exclusive_pairs):
            fac1 = random.randint(0, len(G.nodes) - 1)
            fac2 = random.randint(0, len(G.nodes) - 1)
            if fac1 != fac2:
    ...
    def solve(self, instance):
        ...
        open_facilities = {node: model.addVar(vtype="B", name=f"Open{node}") for node in G.nodes}
        ...
        for (node1, node2) in mutual_exclusivity_pairs:
            model.addCons(open_facilities[node1] + open_facilities[node2] <= 1, f"Exclusive_{node1}_{node2}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'capacity_variation': 0.52,
        'n_exclusive_pairs': 2430,
    }
