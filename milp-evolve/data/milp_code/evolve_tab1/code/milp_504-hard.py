...
class VehicleAllocationMILP:
    ...
    def solve(self, instance):
        ...
        for i, zone in enumerate(zones):
            for j in range(len(zone)):
                for k in range(j + 1, len(zone)):
                    u, v = zone[j], zone[k]
                    model.addCons(
                        vehicle_vars[f"Vehicle{u}"] + vehicle_vars[f"Vehicle{v}"] <= 1,
        ...
        for u, v in G.edges:
            ...
            model.addCons(
                vehicle_vars[f"Vehicle{u}"] + vehicle_vars[f"Vehicle{v}"] <= 1 + route_vars[f"Route{u}_{v}"],
                name=f"VehicleFlow_{u}_{v}"
            )
            model.addCons(
                vehicle_vars[f"Vehicle{u}"] + vehicle_vars[f"Vehicle{v}"] >= 2 * route_vars[f"Route{u}_{v}"] - route_vars[f"Route{u}_{v}"],
