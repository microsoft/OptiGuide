...
class SimplifiedFCMCNF:
    ...
    def solve(self, instance):
        ...
        objective_expr += quicksum(
            adj_mat[i, j][0] * y_vars[f"y_{i+1}_{j+1}"]
        ...
        for (i, j) in edge_list:
            ...
            model.addCons(arc_expr <= adj_mat[i, j][1] * y_vars[f"y_{i+1}_{j+1}"], f"arc_{i+1}_{j+1}")
...
if __name__ == '__main__':
    ...
    simplified_fcmcnf = SimplifiedFCMCNF(parameters, seed=seed)
    instance = simplified_fcmcnf.generate_instance()
    solve_status, solve_time = simplified_fcmcnf.solve(instance)
