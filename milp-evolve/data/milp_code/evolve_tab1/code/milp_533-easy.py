...
class HCAN:
    ...
    def solve(self, instance):
        ...
        # New Convex Hull constraints and variables
        convex_hull_vars = {}
        for node in G.nodes:
            convex_hull_vars[node] = model.addVar(vtype="C", name=f"ConvexHullNode_{node}")

        for s in range(self.no_of_scenarios):
            for node in G.nodes:
                model.addCons(
                    convex_hull_vars[node] >= patient_vars[s][f"NurseShift{node}_s{s}"],
                    name=f"ConvexHullNode_{node}_s{s}_1"
                )
                model.addCons(
                    convex_hull_vars[node] <= quicksum(patient_vars[other_s][f"NurseShift{node}_s{other_s}"] for other_s in range(self.no_of_scenarios)) / self.no_of_scenarios,
                    name=f"ConvexHullNode_{node}_s{s}_2"
                )
