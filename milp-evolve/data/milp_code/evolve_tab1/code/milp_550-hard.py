...
class NetworkOptimization:
    ...
    def solve(self, instance):
        ...
        RouteRemoval_vars = {f"RouteRemoval{u}_{v}": model.addVar(vtype="B", name=f"RouteRemoval{u}_{v}") for u, v in E2}
        ...
        for u, v in E2:
            model.addCons(
                NetworkNode_vars[f"NetworkNode{u}"] + NetworkNode_vars[f"NetworkNode{v}"] - RouteRemoval_vars[f"RouteRemoval{u}_{v}"] <= 1,
                name=f"EdgeRemovalConstraint_{u}_{v}"
            )
...
if __name__ == '__main__':
    ...
    parameters = {
        'min_nodes': 13,
        'max_nodes': 803,
        'network_probability': 0.24,
        'incompatibility_rate': 0.45,
        'no_of_scenarios': 49,
        'NetworkDemandVariation': 0.17,
        'BandwidthVariation': 0.17,
        'NetworkCapacityVariation': 0.1,
        'alpha': 0.1,
    }
