...
class VehicleAllocationMILP:
    ...
    def get_instance(self):
        ...
        demand_scenarios = {node: [max(1, np.random.normal(G.nodes[node]['demand'], 20)) for _ in range(self.no_of_scenarios)] for node in G.nodes}
    ...
    def solve(self, instance):
        ...
        aux_demand_vars = {f"aux_demand_{node}_{s}": model.addVar(vtype="C", name=f"aux_demand_{node}_{s}") for node in G.nodes for s in range(self.no_of_scenarios)}
        ...
        # Adding robust constraints
        for s in range(self.no_of_scenarios):
            for node in G.nodes:
                model.addCons(
                    aux_demand_vars[f"aux_demand_{node}_{s}"] == demand_scenarios[node][s] * vehicle_vars[f"Vehicle_{node}"],
                    name=f"AuxDemand_{node}_{s}"
                )
        
        for node in G.nodes:
            model.addCons(
                quicksum(aux_demand_vars[f"aux_demand_{node}_{s}"] for s in range(self.no_of_scenarios)) / self.no_of_scenarios <= G.nodes[node]['demand'],
                name=f"NodeDemand_{node}"
            )
