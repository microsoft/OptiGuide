...
class CommunityCentersMILP:
    ...
    def get_instance(self):
        ...
        # Hazardous edges from Cooling MILP
        E2 = set()
        for edge in G.edges:
    ...
    def solve(self, instance):
        ...
        # New variables for cooling and hazard constraints
        ...
        # Integrate cooling effectiveness and hazard minimization
        objective_expr += quicksum(
            G.nodes[node]['temperature'] * cooling_node_vars[f"x{node}"]
            for node in G.nodes
        )
        objective_expr -= quicksum(
            G[u][v]['hazard_cost'] * hazard_edge_vars[f"y{u}_{v}"]
            for u, v in E2
        )
        ...
        # Cooling and hazard constraints
        for u, v in G.edges:
            if (u, v) in E2:
                model.addCons(
                    cooling_node_vars[f"x{u}"] + cooling_node_vars[f"x{v}"] - hazard_edge_vars[f"y{u}_{v}"] <= 1,
                    name=f"C_hazard_{u}_{v}"
                )
            else:
                model.addCons(
                    cooling_node_vars[f"x{u}"] + cooling_node_vars[f"x{v}"] <= 1,
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'service_hours': 1350,
        'piecewise_segments': 7,
        'no_of_scenarios': 600,
        'population_variation': 0.45,
        'time_variation': 0.45,
        'capacity_variation': 0.73,
        'financial_param1': 100,
        'financial_param2': 2000,
        'cost_param_1': 1500.0,
        'operation_capacity': 1200.0,
        'center_min_count': 225,
        'center_max_count': 375,
        'temp_param': 1.25,
        'beta': 0.38,
    }
