...
class NMD:
    ...
    def get_instance(self):
        ...
        capacity_priorities = {node: np.random.randint(1, 10) for node in G.nodes}
        ...
        return {
            ...
            'route_priorities': route_priorities,
            'capacity_priorities': capacity_priorities,
        }
    ...
    def solve(self, instance):
        ...
        capacity_priorities = instance['capacity_priorities']
        ...
        # Capacity sequence variables (SOS2)
        capacity_sequence_vars = {node: model.addVar(vtype="B", name=f"CapacitySequence_{node}") for node in G.nodes}
        ...
        for u, v in G.edges:
            model.addConsSOS1([route_priority_vars[(u, v)], route_vars[f"Route_{u}_{v}"]], name=f"RoutePrioritySOS1_{u}_{v}")

        # New Constraints to ensure ordered capacity usage (SOS2)
        for node in G.nodes:
            model.addConsSOS2([capacity_sequence_vars[node], carrier_vars[node]], name=f"CapacitySequenceSOS2_{node}")
