...
class NMD:
    ...
    def solve(self, instance):
        ...
        # Adding new variables for symmetry breaking
        carrier_priority = {node: model.addVar(vtype="B", name=f"CarrierPriority{node}") for node in G.nodes}
        facility_rank = {j: model.addVar(vtype="I", name=f"FacilityRank{j}", lb=1, ub=n_facilities) for j in range(n_facilities)}
        ...
        # Ensure symmetry breaking in carrier activation
        for i in range(1, len(G.nodes)):
            model.addCons(carrier_priority[i] >= carrier_priority[i-1], name=f"SymmetryCarrier{i}")

        # Ensure symmetry breaking in facility rank
        for j in range(1, n_facilities):
            model.addCons(facility_rank[j] >= facility_rank[j-1], name=f"SymmetryFacility{j}")
