...
class IntegratedOptimization:
    ...
    def generate_instance(self):
        ...
        importance_factor = np.random.uniform(1, 10, size=n_locations) # importance factors for locations
        ...
        # Detect cliques in the graph and add to inequalities
    ...
    def solve(self, instance):
        ...
        importance_factor = instance['importance_factor']
        ...
        # New semi-continuous variables for delivery quantities
        delivery_quantities = {(i, j): model.addVar(lb=0, ub=100, vtype="C", name=f"delivery_quantity_{i}_{j}") for i in range(n_factories) for j in range(n_locations)}
        ...
        total_cost = quicksum(production_status[i] * production_costs[i] for i in range(n_factories)) + \
                     quicksum(delivery_quantities[i, j] * delivery_costs[i, j] for i in range(n_factories) for j in range(n_locations)) + \
                     extra_cost * 1000
        ...
        for j in range(n_locations):
            model.addCons(quicksum(delivery_quantities[i, j] for i in range(n_factories)) <= importance_factor[j] * 100, name=f"importance_constraint_{j}")  # semi-continuous constraint
        ...
        # Add clique inequality constraints for more complexity
        if self.maximize_cliques:
            for count, clique in enumerate(cliques):
                if len(clique) > 1:
                    model.addCons(quicksum(station_vars[node] for node in clique) <= 1, name=f"clique_constraint_{count}")
