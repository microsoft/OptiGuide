...
class HCAN:
    ...
    def get_instance(self):
        ...
        for s in range(self.no_of_scenarios):
            care_scenarios[s]['patients'] = {node: int(np.random.normal(G.nodes[node]['patients'], G.nodes[node]['patients'] * self.patient_variation))
                                              for node in G.nodes}
            care_scenarios[s]['visit_time'] = {(u, v): np.random.normal(G[u][v]['visit_time'], G[u][v]['visit_time'] * self.time_variation)
                                               for u, v in G.edges}
            care_scenarios[s]['healthcare_cap'] = {node: int(np.random.normal(healthcare_cap[node], healthcare_cap[node] * self.capacity_variation))
                                                   for node in G.nodes}
        ...
        travel_costs_uncertainty = {(u, v): val * (1 + np.random.normal(0, self.travel_cost_variation)) for (u, v), val in travel_costs.items()}
    ...
    def solve(self, instance):
        ...
        model.addCons(
            quicksum(nurse_shift_vars[f"NurseShift{node}"] for node in G.nodes) <= self.shift_hours,
            name="OffTime_Limit"
        )
