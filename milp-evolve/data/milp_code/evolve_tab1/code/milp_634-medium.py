...
class MultipleKnapsackWithNetwork:
    ...
    def generate_instance(self):
        ...
        for u, v in G.edges:
            ...
            G[u][v]['pollution_cost'] = np.random.randint(1, 20)       # Added pollution cost
            G[u][v]['operational_cost'] = np.random.randint(1, 10)     # Added operational cost
        E_invalid = set()
        for edge in G.edges:
    ...
    def solve(self, instance):
        ...
        pollution_vars = {}  # Added pollution variables
        operational_vars = {}  # Added operational variables
        ...
        for i in range(number_of_items):
            for u, v in G.edges():
                ...
                pollution_vars[(i, u, v)] = model.addVar(vtype="B", name=f"pollution_{i}_{u}_{v}")  # Added pollution vars
                operational_vars[(i, u, v)] = model.addVar(vtype="B", name=f"operational_{i}_{u}_{v}")  # Added operational vars

        # Objective: Maximize total profit minus handling cost minus congestion cost minus noise cost minus pollution cost minus operational cost
        ...
        objective_expr -= quicksum(G[u][v]['pollution_cost'] * pollution_vars[(i, u, v)] for i in range(number_of_items) for u, v in G.edges())       # Added pollution cost
        objective_expr -= quicksum(G[u][v]['operational_cost'] * operational_vars[(i, u, v)] for i in range(number_of_items) for u, v in G.edges())       # Added operational cost
        ...
        # Equitable access constraint: Ensure that at least one item is allocated to each knapsack
        for j in range(number_of_knapsacks):
            model.addCons(
                quicksum(var_names[(i, j)] for i in range(number_of_items)) >= 1,
                f"EquitableAccess_{j}"
            )
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'exclusion_rate': 0.1,
        'max_tolls': 5,
        'equitable_access_min': 1,  # Added parameter for equitable access constraints
    }
