...
class ModifiedNMD:
    ...
    def get_instance(self):
        ...
        demographic_data = np.random.dirichlet(np.ones(5), size=n_facilities).tolist()
        ...
        co2_emissions = {(u, v): np.random.uniform(2, 20) for u, v in G.edges}
        aircraft_maintenance = {node: np.random.randint(1, 5) for node in G.nodes}
        flight_delays = {(u, v): np.random.uniform(0, 15) for u, v in G.edges}
    ...
    def solve(self, instance):
        ...
        aircraft_maintenance = instance['aircraft_maintenance']
        flight_delays = instance['flight_delays']

        model = Model("ModifiedNMD")
        ...
        co2_vars = {f"CO2_{u}_{v}": model.addVar(vtype="C", name=f"CO2_{u}_{v}") for u, v in G.edges}
        maintenance_vars = {node: model.addVar(vtype="C", name=f"Maintenance{node}") for node in G.nodes}
        delay_vars = {f"Delay_{u}_{v}": model.addVar(vtype="C", name=f"Delay_{u}_{v}") for u, v in G.edges}
        ...
        objective_expr -= quicksum(co2_vars[f"CO2_{u}_{v}"] for u, v in G.edges)
        objective_expr -= quicksum(maintenance_vars[node] * 30 for node in G.nodes)
        objective_expr -= quicksum(delay_vars[f"Delay_{u}_{v}"] for u, v in G.edges)
        ...
        model.addCons(
            quicksum(co2_emissions[(u, v)] * co2_vars[f"CO2_{u}_{v}"] for u, v in G.edges) <= self.co2_limit,
            name="CO2EmissionLimit"
        )

        for node in G.nodes:
            model.addCons(
                maintenance_vars[node] >= aircraft_maintenance[node],
                name=f"Maintenance_{node}"
            )

        for u, v in G.edges:
            model.addCons(
                delay_vars[f"Delay_{u}_{v}"] >= flight_delays[(u, v)],
                name=f"Delay_{u}_{v}"
            )
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'small_world_p': 0.24,
        'energy_limit': 10000,
        'co2_limit': 3000  # New parameter for CO2 emissions limit
    }

    nmd = ModifiedNMD(parameters, seed=seed)
