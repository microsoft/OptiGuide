...
class CapacitatedFacilityLocationWithClique:
    ...
    def generate_instance(self):
        ...
        # Segmentation points for piecewise linear function (example 3 segments)
        pwl_points = [30, 60, 90]
        pwl_costs_reduced = [5, 10, 20]

        res.update({
            'pwl_points': pwl_points,
            'pwl_costs_reduced': pwl_costs_reduced
        })
    ...
    def solve(self, instance):
        ...
        pwl_points = instance['pwl_points']
        pwl_costs_reduced = instance['pwl_costs_reduced']
        
        n_customers = len(demands)
        n_facilities = len(capacities)
        num_segments = len(pwl_points)
        ...
        # New Variables for Piecewise Linear Function Segments
        pwl_vars = {(j, k): model.addVar(vtype="C", name=f"PWL_{j}_{k}") for j in range(n_facilities) for k in range(num_segments)}
        ...
        # Piecewise Linear Function contribution to objective
        pwl_objective_expr = quicksum(pwl_vars[j, k] * pwl_costs_reduced[k] for j in range(n_facilities) for k in range(num_segments))
        
        ### given constraints and variables and objective code ends here
        ### new constraints and variables and objective code ends here
        model.setObjective(primary_objective_expr + pwl_objective_expr + (self.secondary_objective_weight * secondary_objective_expr), "minimize")
        ...
        # Clique Constraints using Clique Inequalities
        for idx, clique in enumerate(cliques):
            if len(clique) > 2:
                model.addCons(quicksum(open_facilities[j] for j in clique) <= self.clique_limit, f"Clique_Inequality_{idx}")
        ...
        for j in range(n_facilities):
            for k in range(num_segments):
                if k == 0:
                    model.addCons(pwl_vars[j, k] <= pwl_points[k], f"PWL_Var_Upper_{j}_{k}")
                else:
                    model.addCons(pwl_vars[j, k] <= pwl_points[k] - pwl_points[k-1], f"PWL_Var_Upper_{j}_{k}")
            
            model.addCons(quicksum(pwl_vars[j, k] for k in range(num_segments)) == recycled_used[j], f"Total_PWL_Sum_{j}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'pwl_points': (30, 60, 90),      # Piecewise linear segmentation points
        'pwl_costs_reduced': (5, 10, 20) # Costs for respective segments
    }
