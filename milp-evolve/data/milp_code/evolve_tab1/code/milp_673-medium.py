...
class IndependentSet:
    ...
    def generate_instance(self):
        ...
        res = {
            ...
            'hazardous_chemicals_usage': np.random.randint(1, 20, size=n_facilities),
            'sustainable_suppliers': np.random.choice([0, 1], size=n_resources, p=[0.3, 0.7]),
            'energy_usage': np.random.randint(10, 200, size=n_facilities),
            'carbon_emission': np.random.randint(50, 300, size=n_facilities),
            'carbon_emission_reduction_target': 5000
        }
    ...
    def solve(self, instance):
        ...
        hazardous_chemicals_usage = instance['hazardous_chemicals_usage']
        sustainable_suppliers = instance['sustainable_suppliers']
        energy_usage = instance['energy_usage']
        carbon_emission = instance['carbon_emission']
        carbon_emission_reduction_target = instance['carbon_emission_reduction_target']
        ...
        # Hazardous chemical usage constraint
        total_hazardous_chemical_usage = quicksum(c[j] * hazardous_chemicals_usage[j] for j in range(n_facilities))
        model.addCons(total_hazardous_chemical_usage <= self.max_hazardous_chemicals, name="hazardous_chemical_usage_limit")

        # Sustainable sourcing constraint
        total_sustainable_supply = quicksum(flow[i,j] * sustainable_suppliers[i] for i in range(n_resources) for j in range(n_facilities))
        model.addCons(total_sustainable_supply >= 0.7 * sum(resource_supply), name="sustainable_sourcing_constraint")

        # Energy usage constraint
        total_energy_usage = quicksum(c[j] * energy_usage[j] for j in range(n_facilities))
        model.addCons(total_energy_usage <= self.energy_usage_target, name="energy_usage_constraint")

        # Carbon emission constraint
        total_carbon_emission = quicksum(c[j] * carbon_emission[j] for j in range(n_facilities))
        model.addCons(total_carbon_emission <= carbon_emission_reduction_target, name="carbon_emission_constraint")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_facilities': 144,
        'max_facilities': 225,
        'min_resources_per_facility': 1,
        'max_hazardous_chemicals': 250,
        'energy_usage_target': 10000,
    }
