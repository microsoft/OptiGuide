...
class EmergencyShelterResourceAllocation:
    ...
    def generate_piecewise_cost_data(self, num_segments, min_val, max_val):
        breakpoints = np.linspace(min_val, max_val, num_segments+1)
        segment_costs = np.random.uniform(min_val, max_val, num_segments)
        return breakpoints, segment_costs
    ...
    def generate_instance(self):
        ...
        # Generate piecewise cost data
        (breakpoints_transport, costs_transport) = self.generate_piecewise_cost_data(self.num_segments_transport, self.min_cost_transport, self.max_cost_transport)
        res['breakpoints_transport'] = breakpoints_transport
        res['costs_transport'] = costs_transport
        
        # Generate set cover data (subsets of demand locations each shelter can serve)
        subsets = {j: np.random.choice(range(self.n_customers_emergency), size=random.randint(1, self.n_customers_emergency), replace=False).tolist()
    ...
    def solve(self, instance):
        ...
        breakpoints_transport = instance['breakpoints_transport']
        costs_transport = instance['costs_transport']
        ...
        # Variables for piecewise linear segments for transportation costs
        transport_segment_vars = {}
        for i in range(n_customers_emergency):
            for j in range(n_shelters):
                for k in range(self.num_segments_transport):
                    transport_segment_vars[(i, j, k)] = model.addVar(vtype="C", name=f"TransportSegment_{i}_{j}_{k}")

        # New variables for set cover (complexity added)
        open_subset = {j: model.addVar(vtype="B", name=f"OpenSubset_{j}") for j in range(n_shelters)}
        ...
        primary_objective_expr = quicksum(fixed_costs[j] * open_shelters[j] for j in range(n_shelters)) + \
                         quicksum(costs_transport[k] * transport_segment_vars[i, j, k] for i in range(n_customers_emergency) for j in range(n_shelters) for k in range(self.num_segments_transport)) + \
        ...
        for j in range(n_shelters):
            model.addCons(quicksum(medical_check[j, k] for k in range(self.n_doctors)) == open_shelters[j], f"NecessaryMedicalCheck_{j}")
        ...
        for i in range(n_customers_emergency):
            for j in range(n_shelters):
                for k in range(self.num_segments_transport):
                    if k == 0:
                        model.addCons(transport_segment_vars[i, j, k] >= 0, f"PiecewiseSegmentMin_{i}_{j}_{k}")
                        model.addCons(transport_segment_vars[i, j, k] <= quicksum(1 for m in range(self.num_segments_transport)), f"PiecewiseSegmentMax_{i}_{j}_{k}")
                    else:
                        model.addCons(transport_segment_vars[i, j, k] >= 0, f"PiecewiseSegmentMin_{i}_{j}_{k}")
                        model.addCons(transport_segment_vars[i, j, k] <= quicksum(1 for m in range(k, self.num_segments_transport)), f"PiecewiseSegmentMax_{i}_{j}_{k}")

        # Set covering constraints
        for j in range(n_shelters):
            for i in subsets[j]:
                model.addCons(quicksum(serve[i, sj] for sj in subsets[j]) >= open_subset[j], f"CoverSet_{i}_{j}")
