...
class FCMCNF_GISP:
    ...
    def generate_instance(self):
        ...
        # New: Additional Data for GISP consistency
        self.revenue = {node: np.random.randint(1, 100) for node in range(self.n_nodes)}
        self.costs = {(i, j): (adj_mat[i, j][0] + adj_mat[i, j][1]) / float(self.cost_param) for i, j in edge_list}
    ...
    def solve(self, instance):
        ...
        revenue_vars = {f"r_{i+1}": model.addVar(vtype="B", name=f"r_{i+1}") for i in range(self.n_nodes)}
        edge_vars = {f"e_{i+1}_{j+1}": model.addVar(vtype="B", name=f"e_{i+1}_{j+1}") for (i, j) in edge_list}
        piecewise_vars = {f"p_{i+1}_{j+1}": model.addVar(vtype="B", name=f"p_{i+1}_{j+1}") for (i, j) in edge_list}
        ...
        objective_expr += quicksum(
            revenue[i] * revenue_vars[f"r_{i+1}"]
            for i in range(self.n_nodes)
        )
        objective_expr -= quicksum(
            costs[i, j] * edge_vars[f"e_{i+1}_{j+1}"]
            for (i, j) in edge_list
        )
        ...
        # GISP Constraints
        # Maximal cliques constraint
        for i, clique in enumerate(cliques):
            model.addCons(
                quicksum(revenue_vars[f"r_{node+1}"] for node in clique) <= 1,
                name=f"Clique_{i+1}"
            )
        ...
        for i, j in edge_list:
            model.addCons(
                revenue_vars[f"r_{i+1}"] + revenue_vars[f"r_{j+1}"] <= 1 + piecewise_vars[f"p_{i+1}_{j+1}"],
                name=f"PL1_{i+1}_{j+1}"
            )
            model.addCons(
                revenue_vars[f"r_{i+1}"] + revenue_vars[f"r_{j+1}"] >= 2 * piecewise_vars[f"p_{i+1}_{j+1}"],
                name=f"PL2_{i+1}_{j+1}"
            )
