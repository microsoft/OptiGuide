...
class SimplifiedFacilityLocation:
    ...
    def solve(self, instance):
        ...
        # Big M constraint method for serve only from open facilities
        M = max(capacities)  # Big M should be a sufficiently large number
        for i in range(n_customers):
            for j in range(n_facilities):
                model.addCons(serve[i, j] <= M * open_facilities[j], f"BigMTightening_{i}_{j}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'ratio': 60.0,
        'big_m_constant': 600,  # M should be sufficiently large related to capacities
    }
