...
class WarehousingDeliveryOptimization:
    ...
    def generate_instance(self):
        ...
        # Seasonal weather impact matrices
        num_seasons = 4  # let's consider 4 seasons
        weather_penalties = np.random.uniform(1.0, 2.0, size=(num_seasons, num_customers, num_warehouses))
        road_usable = np.random.randint(0, 2, size=(num_seasons, num_customers, num_warehouses))
    ...
    def solve(self, instance):
        ...
        num_seasons = instance['num_seasons']
        weather_penalties = instance['weather_penalties']
        road_usable = instance['road_usable']
        ...
        # Seasonal road usability variables
        road_usable_vars = {(s, i, j): model.addVar(vtype='B', name=f'road_usable_{s}_{i}_{j}') for s in range(num_seasons) for i in range(num_customers) for j in range(num_warehouses)}
        
        # Objective function: Minimize total costs including weather impacts
        total_cost = quicksum(
            shipment[i, j] * delivery_costs[i, j] * weather_penalties[s, i, j] * road_usable[s, i, j] 
            for i in range(num_customers) for j in range(num_warehouses) for s in range(num_seasons)
        ) + quicksum(
            warehouse[j] * maintenance_costs[j] for j in range(num_warehouses)
        ) + quicksum(
            excess_time[j] * excess_penalty for j in range(num_warehouses)
        )
        ...
        # Road usability constraint based on seasonal conditions
        for s in range(num_seasons):
            for i in range(num_customers):
                for j in range(num_warehouses):
                    model.addCons(road_usable_vars[s, i, j] <= road_usable[s, i, j], name=f"seasonal_road_usable_{s}_{i}_{j}")
