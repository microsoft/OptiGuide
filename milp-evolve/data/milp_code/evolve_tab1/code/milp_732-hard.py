...
class IndependentSet:
    ...
    def generate_instance(self):
        ...
        # Introduce shortest path constraints for diversification
        distances = dict(nx.all_pairs_dijkstra_path_length(nx.Graph(graph.edges)))
    ...
    def solve(self, instance):
        ...
        for count, group in enumerate(path_constraints):
            model.addCons(quicksum(var_names[node] for node in group) <= 1, name=f"path_{count}")

        # Maximize weighted independent set
        objective_expr = quicksum(weights[node] * var_names[node] for node in graph.nodes) - quicksum(var_names[node] for node in graph.nodes) * 0.5
