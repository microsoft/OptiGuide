'''
Fixed Charge Multicommodity Network Flow (FCMCNF) with Big M Formulation
'''
import random
import time
import numpy as np
...
class FCMCNF:
    ...
    def generate_erdos_graph(self):
        ...
        M = np.max(self.d_range) * self.max_n_commodities  # Big M value calculation
    ...
    def solve(self, instance):
        ...
        for (i, j) in edge_list:
            ...
            model.addCons(arc_expr <= adj_mat[i, j][2] * y_vars[f"y_{i+1}_{j+1}"], f"capacity_{i+1}_{j+1}")
            # Big M constraint to enforce flow only if edge is active
            model.addCons(arc_expr <= adj_mat[i, j][3] * y_vars[f"y_{i+1}_{j+1}"], f"big_m_{i+1}_{j+1}")
