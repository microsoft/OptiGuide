...
class SustainableTransportOptimization:
    ...
    def get_instance(self):
        ...
        # Cost matrices with uncertainty
        transport_cost_mean = np.random.randint(20, 200, size=(num_routes, num_modes))
        environmental_impact_mean = np.random.randint(50, 300, size=(num_routes, num_modes))

        transport_cost = transport_cost_mean + np.random.normal(scale=5, size=(num_routes, num_modes))
        environmental_impact = environmental_impact_mean + np.random.normal(scale=10, size=(num_routes, num_modes))

        # Route demand with uncertainty
        route_demand_mean = np.random.randint(100, 500, size=num_routes)
        route_demand = route_demand_mean + np.random.normal(scale=20, size=num_routes)
        ...
        res = {
            ...
            'transport_cost_mean': transport_cost_mean,
            'environmental_impact_mean': environmental_impact_mean,
            'route_demand_mean': route_demand_mean
        }
    ...
    def solve(self, instance):
        ...
        transport_cost_mean = instance['transport_cost_mean']
        environmental_impact_mean = instance['environmental_impact_mean']
        route_demand_mean = instance['route_demand_mean']
        ...
        for i in range(num_routes):
            model.addCons(quicksum(AVariable_amount_transported[i, j] for j in range(num_modes)) >= route_demand_mean[i] - 20, name=f"TConstraint_route_demand_lower_{i}")
            model.addCons(quicksum(AVariable_amount_transported[i, j] for j in range(num_modes)) <= route_demand_mean[i] + 20, name=f"TConstraint_route_demand_upper_{i}")
