...
class FCMCNF_Simplified:
    ...
    def generate_cliques(self, G):
        ...
        for i in range(self.n_cliques):
            nodes = np.random.choice(G.nodes, self.clique_size, replace=False)
    ...
    def solve(self, instance):
        ...
        z_vars = {f"z_{c+1}": model.addVar(vtype="B", name=f"z_{c+1}") for c in range(len(cliques))}
        ...
        for c, clique in enumerate(cliques):
            clique_expr = quicksum(y_vars[f"y_{i+1}_{j+1}"]
                                   for i in clique for j in clique if i != j and (i, j) in edge_list)
            model.addCons(clique_expr <= (len(clique) - 1) * z_vars[f"z_{c+1}"], f"clique_{c+1}")
