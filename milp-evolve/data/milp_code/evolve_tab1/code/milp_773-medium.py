...
class CombinedResourceAllocation:
    ...
    def generate_instance(self):
        ...
        # Generate additional emergency shelter data
        emergency_capacities = self.normal_int(
            self.number_of_knapsacks, 
            self.emergency_capacity_mean,
            self.emergency_capacity_std,
            self.emergency_capacity_lower,
            self.emergency_capacity_upper
        )
        res['emergency_capacities'] = emergency_capacities
        res['shelter_limits'] = self.shelter_limits
    ...
    def solve(self, instance):
        ...
        shelter_limits = instance['shelter_limits']
        ...
        # New constraints for opening shelters and serving customers 
        for j in range(number_of_knapsacks):
            model.addCons(
                quicksum(serve[i, j] for i in range(number_of_items)) <= emergency_capacities[j] * open_shelters[j], 
                f"Emergency_Capacity_{j}"
            )

        # Limit the total number of knapsacks opened
        model.addCons(
            quicksum(open_shelters[j] for j in range(number_of_knapsacks)) <= shelter_limits, 
            "Shelter_Limit"
        )
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'fixed_cost_upper': 675,
        'ratio': 60.0,
        'additional_cost_mean': 3,
        'additional_cost_std': 1,
        'additional_cost_lower': 5,
        'additional_cost_upper': 6,
        'emergency_capacity_mean': 1000,
        'emergency_capacity_std': 50,
        'emergency_capacity_lower': 3000,
        'emergency_capacity_upper': 3000,
        'shelter_limits': 250,
    }
