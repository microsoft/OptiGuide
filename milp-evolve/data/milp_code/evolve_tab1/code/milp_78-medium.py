...
class SupplyChainNetwork:
    ...
    def generate_instance(self):
        ...
        min_supply_levels = np.random.randint(20, 50, size=supply_nodes) # New data for minimum supply levels
        surplus_penalty = np.random.randint(5, 15)  # New data for penalty per unit surplus supply
    ...
    def solve(self, instance):
        ...
        for i in range(supply_nodes):
            model.addCons(quicksum(flow_vars[(i, j)] for j in range(demand_nodes) if (i, j) in arcs) + surplus_vars[i] <= supply_capacities[i] * arc_vars[i], f"supply_capacity_{i}")

            # Minimum supply level if activated (Big M Formulation)
            M = np.max(supply_capacities)  # A large constant
