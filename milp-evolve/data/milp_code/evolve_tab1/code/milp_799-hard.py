...
class SupplyChainOptimization:
    ...
    def solve(self, instance):
        ...
        model.setObjective(quicksum(warehouse_costs[w] * warehouse_vars[w] for w in range(n_warehouses)) +
                           quicksum(delivery_cost_vars[w, c] for w in range(n_warehouses) for c in range(n_customers)), "minimize")
        ...
        # Constraints: Ensure delivery time windows
        for w in range(n_warehouses):
            for c in range(n_customers):
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_delivery_cost': 180,
        'max_delivery_cost': 1000,
