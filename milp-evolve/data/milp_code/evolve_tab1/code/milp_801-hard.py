...
class GISP:
    ...
    def solve(self, instance):
        ...
        symmetry_vars = {f"z{node}": model.addVar(vtype="B", name=f"z{node}") for node in G.nodes}
        ...
        for i in range(len(sorted_nodes) - 1):
            model.addCons(
                symmetry_vars[f"z{sorted_nodes[i]}"] >= symmetry_vars[f"z{sorted_nodes[i + 1]}"],
                name=f"Symmetry_{sorted_nodes[i]}_{sorted_nodes[i + 1]}"
            )
            model.addCons(
                node_vars[f"x{sorted_nodes[i]}"] <= symmetry_vars[f"z{sorted_nodes[i]}"],
                name=f"Symmetry_Consistency_{sorted_nodes[i]}"
            )
            model.addCons(
                node_vars[f"x{sorted_nodes[i + 1]}"] <= symmetry_vars[f"z{sorted_nodes[i + 1]}"],
                name=f"Symmetry_Consistency_{sorted_nodes[i+1]}"
            )
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_capacity': 135,
        'max_capacity': 157,
        'min_removable_edges': 165,
        'max_removable_edges': 168,
    }
