...
class CombinatorialAuction:
    ...
    def solve(self, instance):
        ...
        half_num_bids = len(bids) // 2
        for i in range(half_num_bids, len(bids)):
            model.addCons(quicksum(i * bid_vars[j] for j in range(i, len(bids))) <= i * half_num_bids, f"Symmetry_{i}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'additivity': 0.1,
        'add_item_prob': 0.73,
        'mean_exchange_rate': 0.73,
        'exchange_rate_std': 0.66,
        'mean_tariff': 0.38,
        'tariff_std': 0.31,
        'mean_esg_penalty': 0.17,
        'esg_penalty_std': 0.1,
        'mean_geopolitical_risk': 0.8,
        'geopolitical_risk_std': 0.45,
        'mean_labor_market_fluctuation': 0.17,
        'labor_market_fluctuation_std': 0.8,
        'mean_technological_disruption': 0.38,
        'technological_disruption_std': 0.66,
        'mean_logistic_cost': 0.24,
        'logistic_cost_std': 0.38,
    }
