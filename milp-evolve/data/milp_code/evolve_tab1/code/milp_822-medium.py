...
class Healthcare:
    ...
    def generate_instance(self):
        ...
        processing_times = np.random.uniform(1, 10, size=self.number_of_patients)
        storage_capacities = np.random.uniform(0, 1, size=self.number_of_patients)
        ...
        res = {'number_of_doctors': self.number_of_doctors,
               ...
               'total_storage_capacity': np.sum(capacities) / 2,
               'processing_time_limit': np.sum(processing_times) / 1.5}
...
class PatientCoverage:
    ...
    def solve(self, instance):
        ...
        total_storage_capacity = instance['total_storage_capacity']
        processing_time_limit = instance['processing_time_limit']
        ...
        for patient in range(number_of_patients):
            ...
            storage_vars[patient] = model.addVar(vtype="B", name=f"s_{patient}")
            processing_vars[patient] = model.addVar(vtype="C", lb=0, name=f"pt_{patient}")
        ...
        # Storage constraint
        model.addCons(quicksum(storage_vars[patient] * storage_capacities[patient] for patient in range(number_of_patients)) <= total_storage_capacity, name="total_storage_capacity")

        # Processing time constraint
        model.addCons(quicksum(processing_vars[patient] for patient in range(number_of_patients)) <= processing_time_limit, name="processing_time_limit")
