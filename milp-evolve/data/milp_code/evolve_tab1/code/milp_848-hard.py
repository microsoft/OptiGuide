...
class AdvancedFacilityLocation:
    ...
    def generate_instance(self):
        ...
        penalties = np.random.uniform(5, 15, self.n_neighborhoods)
        env_impact = np.random.uniform(0.1, 1.0, (self.n_facilities, self.n_neighborhoods))
    ...
    def solve(self, instance):
        ...
        env_impact = instance['env_impact']
        ...
        # Objective: maximize financial rewards from treated neighborhoods minus costs (fixed, transport) minus penalties and environmental impact
        model.setObjective(
            ...
            quicksum(penalties[n] * (1 - quicksum(allocation_vars[f, n] for f in range(n_facilities))) for n in range(n_neighborhoods)) -
            quicksum(env_impact[f][n] * allocation_vars[f, n] for f in range(n_facilities) for n in range(n_neighborhoods)),
            "maximize"
        )
        ...
        # Additional Constraints: Penalty if not all neighborhoods are assigned
        for n in range(n_neighborhoods):
            model.addCons(quicksum(allocation_vars[f, n] for f in range(n_facilities)) <= 1, f"Penalty_Constraint_{n}")
