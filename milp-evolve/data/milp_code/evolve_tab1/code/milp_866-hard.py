...
class CloudTaskScheduling:
    ...
    def generate_instance(self):
        ...
        hyper_task_server_combination = np.random.binomial(1, 0.5, (self.n_tasks, self.n_servers, self.hyper_param))  # New combinatorial data element

        res['hyper_task_server_combination'] = hyper_task_server_combination
    ...
    def solve(self, instance):
        ...
        task_server_combination = {(i, j, k): model.addVar(vtype="B", name=f"Comb_{i}_{j}_{k}") for i in range(n_tasks) for j in range(n_servers) for k in range(self.hyper_param)}
        ...
        for i in range(n_tasks):
            for j in range(n_servers):
                for k in range(self.hyper_param):
                    model.addCons(task_server_combination[i, j, k] <= hyper_task_server_combination[i, j, k], f"Comb_Hull_{i}_{j}_{k}")
                    model.addCons(quicksum(task_server_combination[i, j, k] for k in range(self.hyper_param)) == assign_task[i, j], f"Comb_Hull_Assign_{i}_{j}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'activation_cost_interval': (600, 1200),
        'ratio': 45.0,
        'hyper_param': 10,
    }
