...
class TelecomNetworkOptimization:
    ...
    def solve(self, instance):
        ...
        # Objective: minimize the total cost (hub + connection)
        model.setObjective(quicksum(hub_costs[h] * hub_vars[h] for h in range(n_hubs)) +
                           quicksum(connection_costs[h, n] * connection_vars[h, n] for h in range(n_hubs) for n in range(n_neighborhoods)), "minimize")
        ...
        for h in range(n_hubs):
            model.addCons(quicksum(connection_vars[h, n] for n in range(n_neighborhoods)) <= capacities[h], f"Hub_{h}_Capacity")
