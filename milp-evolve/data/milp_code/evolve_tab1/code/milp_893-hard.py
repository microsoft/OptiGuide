...
class CloudTaskScheduling:
    ...
    def affinity_matrix(self):
        return np.random.rand(self.n_tasks, self.n_servers)  # Random affinity between tasks and servers
    ...
    def generate_instance(self):
        ...
        task_server_affinity = self.affinity_matrix()
        ...
        res = {
            ...
            'task_server_affinity': task_server_affinity
        }
        return res
    ...
    def solve(self, instance):
        ...
        task_server_affinity = instance['task_server_affinity']
        ...
        # Server capacity limits must be respected
        ...
        # Average wait time minimized (All tasks processed within permissible limits)
        for i in range(n_tasks):
            for j in range(n_servers):
        ...
        # Clique Inequalities to mitigate the overuse of resources and ensure fair distribution
        # For simplicity, using random cliques of tasks as an example to demonstrate the concept
        clique_size = 5
        for k in range(0, n_tasks, clique_size):
            clique_tasks = range(k, min(k + clique_size, n_tasks))
            for j in range(n_servers):
                model.addCons(quicksum(assign_task[i, j] * task_server_affinity[i, j] for i in clique_tasks) <= 2, f"Clique_Inequality_{k}_{j}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'maintenance_cost_interval': (277, 1125),
