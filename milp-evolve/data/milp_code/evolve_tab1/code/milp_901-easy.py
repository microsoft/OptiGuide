...
class EnhancedCombinatorialAuction:
    ...
    def solve(self, instance):
        ...
        item_bundle_vars = {(item, i): model.addVar(vtype="B", name=f"Item_{item}_Bundle_{i}") for item in range(self.n_items) for i in range(len(bids_per_item[item]))}
        ...
        # Constraints: Each item can be in at most one bundle using Convex Hull Formulation
        for item, bid_indices in enumerate(bids_per_item):
            model.addCons(quicksum(item_bundle_vars[(item, idx)] for idx in range(len(bid_indices))) <= 1, f"Item_{item}_OneBundle")
            for idx, bid_idx in enumerate(bid_indices):
                model.addCons(bid_vars[bid_idx] <= item_bundle_vars[(item, idx)], f"LinkBid_{bid_idx}_Item_{item}")
                model.addCons(item_bundle_vars[(item, idx)] <= quicksum(bid_vars[bid_idx] for bid_idx in bid_indices), f"LinkItem_{item}_Bid_{bid_idx}Bundle_{idx}")
...
if __name__ == '__main__':
    ...
    auction = EnhancedCombinatorialAuction(parameters, seed)
