...
class EnhancedIndependentSet:
    ...
    def generate_instance(self):
        ...
        res = {'graph': graph, 'weights': weights, 'capacities': capacities, 'inequalities': inequalities, 'edge_weights': edge_weights}
        return res
    ...
    def solve(self, instance):
        ...
        model = Model("EnhancedIndependentSet")
        var_names = {}
        edge_vars = {}
        ...
        for i, (u, v) in enumerate(graph.edges):
            edge_vars[(u, v)] = model.addVar(vtype="B", name=f"edge_{u}_{v}")
        ...
        for i, (u, v) in enumerate(graph.edges):
            model.addCons(var_names[u] + var_names[v] <= edge_vars[(u, v)], name=f"cover_{u}_{v}")

        node_objective_expr = quicksum(var_names[node] * weights[node] for node in graph.nodes)
        edge_objective_expr = quicksum(edge_vars[edge] * edge_weights[i] for i, edge in enumerate(graph.edges))
        
        objective_expr = node_objective_expr + edge_objective_expr
...
if __name__ == '__main__':
    ...
    enhanced_independent_set_problem = EnhancedIndependentSet(parameters, seed=seed)
    instance = enhanced_independent_set_problem.generate_instance()
    solve_status, solve_time = enhanced_independent_set_problem.solve(instance)
