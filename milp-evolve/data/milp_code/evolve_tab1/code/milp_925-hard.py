...
class EdgeComputingOptimization:
    ...
    def generate_instance(self):
        ...
        assert self.min_power_limit > 0 and self.max_power_limit >= self.min_power_limit
        ...
        power_limits = np.random.randint(self.min_power_limit, self.max_power_limit + 1, self.n_nodes)
    ...
    def solve(self, instance):
        ...
        power_limits = instance['power_limits']
        ...
        selected_nodes = model.addVar(vtype="B", name="SelectedNodes")
        ...
        # Set covering constraint: each task must be covered by a selected node with sufficient capacity
        for t in range(n_tasks):
            model.addCons(quicksum(task_vars[n, t] for n in range(n_nodes)) >= selected_nodes, f"Task_{t}_Covered")
        
        # Nodes selected for processing tasks should also be within a certain power consumption limit
        for n in range(n_nodes):
            model.addCons(quicksum(task_demand[t] * task_vars[n, t] for t in range(n_tasks)) <= power_limits[n], f"Node_{n}_PowerLimit")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_latency': 2,
        'max_latency': 10,
        'min_power_limit': 1000,
        'max_power_limit': 5000,
    }
