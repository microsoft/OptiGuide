...
class SCND:
    ...
    def generate_demand_uncertainty(self):
        demand_uncertainties = np.random.uniform(self.demand_uncertainty_min, self.demand_uncertainty_max, size=self.n_nodes)
        return demand_uncertainties
    ...
    def generate_instance(self):
        ...
        demand_uncertainties = self.generate_demand_uncertainty()
        ...
        res = {
            ...
            'demands': demands,
            'demand_uncertainties': demand_uncertainties,
    ...
    def solve(self, instance):
        ...
        demand_uncertainties = instance['demand_uncertainties']
        ...
        for i in range(self.n_nodes):
            ...
            # Robust Demand satisfaction constraint
            model.addCons(
                quicksum(Allocation[j, i] for j in range(self.n_nodes)) >= demands[i] - demand_uncertainties[i],
                f"Demand_Satisfaction_LB_{i}"
            )
            model.addCons(
                quicksum(Allocation[j, i] for j in range(self.n_nodes)) <= demands[i] + demand_uncertainties[i],
                f"Demand_Satisfaction_UB_{i}"
            )
        ...
        for i in range(self.n_nodes):
            for j in range(self.n_nodes):
                ...
                model.addCons(
                    Batch_Size[i, j] <= self.batch_size_max,
                    f"Batch_Constraint_{i}_{j}"
                )
                # Queue time constraints
                model.addCons(
                    Queue_Time[i, j] <= self.queue_time_max,
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'env_cost_max': 25,
        'env_threshold': 500,
        'demand_uncertainty_min': 20,
        'demand_uncertainty_max': 100,
        'batch_size_max': 10,
        'queue_time_max': 5.0,
