...
class SupplyChainOptimization:
    ...
    def generate_instance(self):
        ...
        energy_costs = np.random.uniform(self.min_energy_cost, self.max_energy_cost, self.n_facilities)
    ...
    def solve(self, instance):
        ...
        facility_energy_costs_vars = {f: model.addVar(vtype="C", lb=0, name=f"FacilityEnergy_{f}") for f in range(n_facilities)}
        
        # Objective: minimize the total cost (facility opening + transport + energy costs)
        model.setObjective(
            ...
            quicksum(facility_energy_costs_vars[f] for f in range(n_facilities)),
            "minimize"
        )
        ...
        # Constraints: Energy cost per unit capacity
        for f in range(n_facilities):
            model.addCons(facility_energy_costs_vars[f] == energy_costs[f] * capacities[f] * facility_open_vars[f], f"Facility_{f}_EnergyCost")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_energy_cost': 0.45,
        'max_energy_cost': 0.77,
    }
