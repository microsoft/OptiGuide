...
class UrbanHousingManagement:
    ...
    def solve(self, instance):
        ...
        unit_backup_vars = {u: model.addVar(vtype="B", name=f"Backup_Unit_{u}") for u in range(n_units)}
        ...
        model.setObjective(quicksum(management_costs[f] * facility_vars[f] for f in range(n_facilities)) +
                           ...
                           1000 * urban_surge +
                           quicksum(surge_penalties[u] * unit_backup_vars[u] for u in range(n_units)), "minimize")
        ...
        for u in range(n_units):
            model.addCons(unit_backup_vars[u] == quicksum(unit_backup_vars[u] for f in range(n_facilities)), f"Unit_{u}_Backup")
        ...
        # New Constraints: Housing units must have a minimum quality of living
        min_quality = 0.9
        for u in range(n_units):
