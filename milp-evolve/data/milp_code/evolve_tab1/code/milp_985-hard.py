...
class UrbanHousingManagement:
    ...
    def solve(self, instance):
        ...
        unit_connection_vars = {(f, u): model.addVar(vtype="C", lb=0, ub=capacities[f], name=f"Facility_{f}_Unit_{u}") for f in range(n_facilities) for u in range(n_units)}
        
        # Objective: minimize the total costs (management + infrastructure)
        model.setObjective(quicksum(management_costs[f] * facility_vars[f] for f in range(n_facilities)) +
                           quicksum(infrastructure_costs[f, u] * unit_connection_vars[f, u] for f in range(n_facilities) for u in range(n_units)), "minimize")
        ...
        for f in range(n_facilities):
            for u in range(n_units):
                model.addCons(unit_connection_vars[f, u] <= facility_vars[f] * capacities[f], f"Facility_{f}_Connection_{u}")
        
        # Constraints: Facilities cannot exceed their capacity
        for f in range(n_facilities):
            model.addCons(quicksum(unit_connection_vars[f, u] for u in range(n_units)) <= capacities[f], f"Facility_{f}_Capacity")
        
        # Constraints: Housing units must have a minimum quality of living
        min_quality = 0.9
        for u in range(n_units):
        ...
        # Constraint: Facilities must serve at least a minimum number of housing units
        min_service = 5
        for f in range(n_facilities):
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_infrastructure_cost': 90,
        'max_infrastructure_cost': 175,
        'min_capacity': 500,
        'max_capacity': 3000,
    }
