...
class EnhancedFacilityCovering:
    ...
    def solve(self, instance):
        ...
        model = Model("EnhancedFacilityCovering")
        ...
        # Constraints: Each neighborhood must be covered by at least one facility
        for n in range(n_neighborhoods):
            model.addCons(quicksum(allocation_vars[f, n] for f in range(n_facilities)) >= 1, f"Neighborhood_{n}_Covering")

        # Constraints: Only open facilities can cover neighborhoods
        for f in range(n_facilities):
            for n in range(n_neighborhoods):
                model.addCons(allocation_vars[f, n] <= facility_vars[f], f"Facility_{f}_Cover_{n}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'stddev_transport_cost': 632,
        'min_transport_cost': 759,
        'max_transport_cost': 2529,
        'min_capacity': 1044,
        'max_capacity': 1518,
    }

    covering_optimizer = EnhancedFacilityCovering(parameters, seed=42)
    instance = covering_optimizer.generate_instance()
    solve_status, solve_time, objective_value = covering_optimizer.solve(instance)
