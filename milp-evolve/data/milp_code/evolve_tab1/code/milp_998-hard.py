...
class AdvancedFacilityLocation:
    ...
    def solve(self, instance):
        ...
        tier_vars = {f: model.addVar(vtype="I", name=f"Tier_{f}", lb=1, ub=3) for f in range(n_facilities)}
        ...
        model.setObjective(
            ...
            quicksum(1000 * tier_vars[f] for f in range(n_facilities)) +
            quicksum(100 * quicksum(allocation_vars[f, n] for n in range(n_neighborhoods)) for f in range(n_facilities)),
            "maximize"
        )
        ...
        for f in range(n_facilities):
            for t in range(1, 3):
                model.addCons(tier_vars[f] * quicksum(allocation_vars[f, n] for n in range(n_neighborhoods)) <= capacities[f] * t, f"Tier_Capacity_{f}_{t}")
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'min_fixed_cost': 1500,
        'max_fixed_cost': 1581,
        'min_transport_cost': 243,
        'max_transport_cost': 490,
        'min_capacity': 238,
        'max_capacity': 1771,
    }
