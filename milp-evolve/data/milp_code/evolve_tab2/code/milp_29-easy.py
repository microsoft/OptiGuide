...
class JobScheduling:
    ...
    def solve(self, instance):
        ...
        aux_allocation = {}  # Auxiliary machine allocation binary variables
        ...
        for i in range(self.n_jobs):
            ...
            aux_allocation[i] = model.addVar(f"aux_alloc_{i}", vtype="B", lb=0, ub=1)  # Auxiliary allocation decision
        ...
        # New machine capacity constraints
        for j in range(self.n_machines):
            model.addCons(
                quicksum((processing_times[i] * (1 - aux_allocation[i])) for i in range(self.n_jobs) if machine_assignment[i] == j) 
                <= machine_capacity[j], f"Machine_Capacity_{j}"
            )

        # New objective: Minimize a combination of makespan, risk cost and aux allocation cost
        overtime_penalty = 50
        objective_expr = end_time + self.risk_weight * quicksum(risk_cost[i] for i in range(self.n_jobs)) + \
                         overtime_penalty * quicksum(aux_allocation[i] for i in range(self.n_jobs))
