...
class OptiGuideAgent(AssistantAgent):
    ...
    def __init__(self,
        ...
        self.set_sourcecode(source_code)
        ...
        self._origin_execution_result = _run_with_exec(source_code,
                                                       self._solver_software, self.visual_callback)
    ...
    def set_sourcecode(self, source_code: str):
        """Set the source code to run."""
        self.end_code = source_code.find("STOP HERE")

        if self.end_code > 0:
            self.end_code += len("STOP HERE")
    ...
    def generate_reply(
        ...
        if sender not in [self._writer, self._safeguard]:
            ...
            writer_sys_msg = (WRITER_SYSTEM_MSG.format(
                ...
                source_code=self._source_code[:self.end_code],
            ...
            safeguard_sys_msg = SAFEGUARD_SYSTEM_MSG.format(
                source_code=self._source_code[:self.end_code]) + user_chat_history
    ...
    def _generate_reply_to_writer(self, sender):
        ...
        if safe_msg.find("DANGER") < 0:
            ...
            src_code = _insert_code(self._source_code[:self.end_code], code,
                                    self._solver_software)

            if self.code_callback:
                self.code_callback(src_code + self._source_code[self.end_code:])

            execution_rst = _run_with_exec(src_code + self._source_code[self.end_code:], self._solver_software, self.visual_callback)
...
def _run_with_exec(src_code: str,
                   solver_software: str, visual_callback) -> Union[str, Exception]:
    ...
    try:
        if visual_callback:
            visual_callback(locals_dict['instance_graph'])
        print(locals_dict['instance_graph'])
...
def _insert_code(src_code: str, new_lines: str, solver_software: str) -> str:
    ...
    if solver_software == "gurobi":
        if new_lines.find("addConstr") >= 0:
            return _replace(src_code, CONSTRAINT_CODE_STR, CONSTRAINT_CODE_STR + "\n" + new_lines)
    elif solver_software == "pyomo":
        if new_lines.find("Constraint") >= 0:
            return _replace(src_code, CONSTRAINT_CODE_STR, CONSTRAINT_CODE_STR + "\n" + new_lines)

    return _replace(src_code, DATA_CODE_STR, DATA_CODE_STR + "\n" + new_lines)
